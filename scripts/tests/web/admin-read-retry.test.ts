...
import { createImageMetadataSession, reconcileImageMetadataSession } from "../../../packages/web/src/components/image/editor/image-metadata-session.ts";
...
test(`[Web/后台保存] 写响应挂起后限时进入快照确认，保留冻结意图和新草稿 / ${phase}`, async (t) => {
  ...
  const authority = ids.map(id => editableImage(id, { title: "submitted", tags: ["submitted-tag"] }));
  const snapshotGate = Promise.withResolvers<void>();
  let writes = 0, snapshots = 0, invalidations = 0;
  ...
  const { clock } = createReadHarness(t, async (input, init) => {
    ...
    if (String(input).endsWith("/images/update")) {
      ...
      return stalledResponse({ ...init, signal: init?.signal ?? new AbortController().signal }, phase);
    }
  ...
  function Harness() {
    operations = useImageMetadataOperations({ initialIds: ids, onSaved: async () => { invalidations++; } });
    return null;
  }
  try {
    await React.act(async () => root.render(React.createElement(Harness)));
    let pending!: ReturnType<typeof operations.save>;
    await React.act(async () => { pending = operations.save(updates, ids); });
    await React.act(async () => { await clock.advanceBy(29_999); await settleRequests(); });
    assert.equal(snapshots, 0);
    await React.act(async () => { await clock.advanceBy(1); await settleRequests(); });
    ...
    await React.act(async () => { outcome = await pending; });
    assert.ok(outcome!);
    assert.equal(outcome!.report.responseReceived, false);
    assert.equal(outcome!.report.updated, 2);
    assert.deepEqual(outcome!.attempt.items[0], { id: ids[0], title: "submitted", tags: ["submitted-tag"] });
    const reconciled = reconcileImageMetadataSession(session, outcome!.attempt, outcome!.authoritativeItems!);
...
test(`[Web/后台保存] 未知写入迟于首次确认完成，人工确认交接最新列表和词表 / ${confirmation}${automatic ? " auto" : ""}`, async (t) => {
  ...
  const after = editableImage(imageId, { title: "committed", tags: ["new-tag"], brightness: "light" });
  let committed = false;
  let writes = 0, snapshots = 0, listReads = 0, vocabularyReads = 0;
  const { client, clock } = createReadHarness(t, async (input, init) => {
    if (String(input).endsWith("/logs/client-errors")) return Response.json({ ok: true });
    if (String(input).endsWith("/images/update")) {
      writes++;
    ...
    vocabularyReads++;
    return Response.json({ themes: [], authors: [], tags: committed ? [{ slug: "new-tag", display_name: "new-tag" }] : [] });
  }, true);
  const list = new QueryObserver(client, {
    queryKey: [...queryKeys.adminImages, "current-page"],
    staleTime: Infinity,
    queryFn: async () => { listReads++; return { items: [committed ? after : before] }; }
  });
  ...
  try {
    await React.act(async () => root.render(React.createElement(Harness)));
    let pending!: ReturnType<typeof operations.save>;
    await React.act(async () => { pending = operations.save([{
      id: imageId, title: "committed", tags: ["new-tag"], brightness: automatic ? "auto" : "light"
    }], [imageId]); });
    await React.act(async () => { await clock.advanceBy(30_000); await settleRequests(); });
    for (let attempt = 0; attempt < 4; attempt++) {
      await React.act(async () => { await clock.advanceBy(30_000); await settleRequests(); });
      if (attempt < 3) await React.act(async () => { await clock.advanceBy(500 * 2 ** attempt); await settleRequests(); });
    }
    ...
    if (automatic) {
      ...
      Object.assign(session.drafts[imageId]!, { title: "committed", tags: ["new-tag"], brightness: "auto" });
      const reconciled = reconcileImageMetadataSession(session, outcome.attempt, outcome.authoritativeItems);
      assert.equal(reconciled.drafts[imageId]!.brightness, "auto", "刷新父页面不能清除未确认指令草稿");
    }
    ...
    assert.deepEqual(vocabulary.getCurrentResult().data?.tags.map(tag => tag.slug), ["new-tag"]);

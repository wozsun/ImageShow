...
import { adminPermissions, sortOrderMin, sortOrderMax } from "../../../packages/shared/src/browser.ts";
...
async function sortingPage(t: TestContext, kind: Kind) {
  ...
  const { VocabularyAdmin } = await import("../../../packages/web/src/pages/admin/VocabularyAdmin.tsx");
  ...
  const { storageBackendS3FormSettings } = await import("../../../packages/web/src/pages/admin/storage/storage-backend-form.ts");
  ...
  const original = (kind === "storage" ? ["local", "a", "b", "c"] : ["a", "b", "c"]).map((slug, index) => ({
    slug, sort_order: 100 - index, display_name: slug, image_count: 0,
    link: "", derived_identity: null, type: slug === "local" ? "local" : "s3", enabled: true, is_default: slug === "local",
    content_md5: null, s3: { ...storageBackendS3FormSettings(), secret_access_key_configured: true },
    ingestion_session_count: 0, cleanup_job_count: 0, failed_cleanup_job_count: 0,
    exhausted_cleanup_job_count: 0, deletion: slug === "local"
      ? { action: "blocked", blockers: ["local"] } : { action: "delete", blockers: [] }
  }));
  const response = (values: Record<string, number> = {}) => {
    const rows = original.map((item) => ({ ...item, sort_order: values[item.slug] ?? item.sort_order }));
    rows.sort((a, b) => Number(b.slug === "local") - Number(a.slug === "local")
      || b.sort_order - a.sort_order || a.slug.localeCompare(b.slug));
  ...
  client.setQueryData(queryKeys.me, { authenticated: true, username: "sort-tester", role: "super",
    permissions: Object.values(adminPermissions), preferences: {}, csrf: "sort-test" });
  const projections = kind === "storage"
    ? [queryKeys.storageOptions] : [queryKeys.galleryFacets, queryKeys.galleryStats, queryKeys.ingestionVocabulary];
  ...
  await h.render(h.React.createElement(QueryClientProvider, { client },
    h.React.createElement(MemoryRouter, { initialEntries: [`/admin/${kind}`] },
      h.React.createElement(AuthSessionProvider, null,
        h.React.createElement(ActionFeedbackProvider, null,
          kind === "storage" ? h.React.createElement(StorageSettings)
            : h.React.createElement(VocabularyAdmin, { kind }))))));
  const input = (slug: string) => {
    const node = h.document.querySelector<HTMLInputElement>(`.sort-order-control input[aria-label$=" ${slug}排序值"]`);
    assert.ok(node, `排序输入 ${slug}`);
    return node;
  };
  const edit = async (slug: string, value: string) => h.React.act(async () => { inputText(h.window, input(slug), value); });
  const emit = async (node: EventTarget, event: string, properties: Record<string, unknown> = {}) => {
    await h.React.act(async () => { dispatchDomEvent(h.window, node, event, properties); });
    await h.flush();
  };
...
for (const kind of ["tags", "themes", "authors", "storage"] as const) {
  test(`[Web/后台排序] ${kind} 草稿、控件内焦点与连续单项保存保持其他卡片可编辑`, async (t) => {
    ...
    assert.deepEqual([...h.document.querySelectorAll<HTMLInputElement>(".sort-order-control input")]
      .map((node) => node.value), ["1001", kind === "storage" ? "97" : "98", "-20"]);
    assert.equal(h.input("b").disabled, false);
    assert.equal(h.pending.length, 4, "每项一次写入及所属列表回读");
    for (const projection of h.projections) assert.equal(h.client.getQueryState(projection)?.isInvalidated, true);
    assert.equal(h.client.getQueryState(queryKeys.adminImages)?.isInvalidated, false, "排序只失效相关投影");
  });
}
...
test("[Web/后台排序] 作者资料保存与排序回读重叠时采用同时包含两次修改的列表", async (t) => {
  ...
  await h.React.act(async () => { inputText(h.window, name, "updated"); });
...
test("[Web/后台排序] 作者同值顺序保持服务器排列，无法确定的新位置由列表回读", async (t) => {
  ...
  await h.React.act(async () => { h.client.setQueryData(h.key, { items: [aa, ab] }); });
  await h.flush();
  const order = () => [...h.document.querySelectorAll<HTMLInputElement>(".entity-card .entity-slug")]
  ...
  await h.React.act(async () => {
    inputText(h.window, h.document.querySelector<HTMLInputElement>(".entity-create-slug")!, created.slug);
  });
...
test("[Web/后台排序] 整数输入边界、权威更新与卸载遵循草稿和单次提交契约", async (t) => {
  ...
  for (const value of ["", "1.5", "1e3", "NaN", String(sortOrderMax + 1), String(sortOrderMin - 1)]) {
    await edit(value);
    await key("Enter");

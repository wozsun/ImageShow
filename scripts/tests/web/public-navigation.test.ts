  ...
  await h.render(
    h.React.createElement(
      ...
      h.React.createElement(
        ...
        h.React.createElement(
          MemoryRouter,
          { initialEntries: ["/embed/home"] },
          h.React.createElement(NavigationProbe),
          h.React.createElement(AppRoutes)
        )
      )
    )
  );
...
test("[Web/公开导航] 首页页脚按配置展示备案和受限 HTML，安全处理链接及空项", async (t) => {
  ...
  assert.equal(
    h.document.querySelector("footer a")?.getAttribute("href"),
    "https://beian.miit.gov.cn/"
  );
  await render({
    icp: "示例ICP备123号",
    mps: "示例公网安备12345678901234号",
    footer: "完整页脚文字"
  });
  assert.equal(
    h.document.querySelector("footer")?.textContent,
    "示例ICP备123号|示例公网安备12345678901234号完整页脚文字"
  );
  assert.equal(
    h.document.querySelectorAll("footer a")[1]?.getAttribute("href"),
  ...
  await render({
    footer:
      'Powered by <a href="https://example.com/project?q=1&amp;x=2" onclick="alert(1)" style="color:red" target="_self">ImageShow</a><br>更多说明 &amp; 文字'
  });
  const link = h.document.querySelector("footer a")!;
  assert.equal(link.textContent, "ImageShow");
  assert.deepEqual(
    Object.fromEntries(
      Array.from(link.attributes, (attribute) => [attribute.name, attribute.value])
    ),
    {
  ...
  await render({
    footer:
      '<script>alert(1)</script><style>body{display:none}</style><img src="https://example.com/image" onerror="alert(1)"><iframe src="https://example.com/"></iframe><svg><a href="https://example.com/">SVG</a></svg><template><a href="https://example.com/">隐藏</a></template>安全正文'
  });
  assert.equal(h.document.querySelector("footer")?.textContent, "安全正文");
  assert.deepEqual(
    Array.from(h.document.querySelector("footer")!.querySelectorAll("*"), (node) => node.localName),
    ["p"]
  );
  ...
  assert.equal(
    ...
    null,
    "嵌入首页未配置 footer 时不保留备案或页脚空间"
  );
  await render(
    { ...registrations, footer: '自定义 <a href="https://example.com/">链接</a><br>第二行' },
    true
  );
  ...
  assert.equal(
    h.document.querySelector("footer")?.textContent,
    "示例ICP备123号|示例公网安备12345678901234号普通页内容"
  );
    ...
    await t.test(failure, async (t) => {
      ...
      function Harness() {
        ...
        return route.error
          ? h.React.createElement(PublicFilterErrorState, {
              ...
              onClear: () =>
                route.updateSearchParams((params) => {
                  params.delete("tag");
                  return params;
                })
            })
      ...
      await h.render(
        h.React.createElement(
          ...
          h.React.createElement(
            MemoryRouter,
            { initialEntries: ["/gallery?tag=new-tag&theme=null"] },
      ...
      const retry = () =>
        [...h.document.querySelectorAll("button")].find(
              ...
              new Response(
                new ReadableStream({
                  start(controller) {
                    ...
                    h.pending[0].signal!.addEventListener(
                      "abort",
                      () => controller.error(h.pending[0].signal!.reason),
                      { once: true }
                    );
                  }
                })
      ...
      await h.respond(2, {
        themes: [],
        authors: [],
        tags: [{ slug: "new-tag", display_name: "新标签" }]
      });
    ...
    await t.test(path, async (t) => {
      ...
      t.after(
        installProperties(h.window, {
          ...
          scrollX: 0,
          scrollY: 0,
          location: new URL(`https://img.example${path}`),
          scrollTo() {},
          getComputedStyle: () => ({
            paddingLeft: "0px",
            paddingRight: "0px",
      ...
      client.setQueryData(queryKeys.galleryFacets, { themes: [], tags: [], authors: [] });
      client.setQueryData(queryKeys.siteConfig, {
        site: appConfig.runtimeDefaults.site,
        embed: { enabled: true }
      });
      ...
      const page = show
        ? h.React.createElement(ShowPage, {
            embedded,
            settings: {
              enabled: true,
              autoplay: false,
        ...
        h.React.createElement(
          ...
          h.React.createElement(
            ...
            h.React.createElement(
              AuthSessionProvider,
              null,
              h.React.createElement(LocationProbe),
      ...
      const imageRequests = () =>
        h.pending.filter((request) => request.path.startsWith("/api/images?"));
      ...
      await h.respond(h.pending.indexOf(facets[0]), {
        themes: [],
        authors: [],
        tags: [
          { slug: "new-tag", display_name: "新标签" },
          { slug: "other", display_name: "另一个标签" }
        ]
      });
      await h.flush();
      ...
      for (const label of ["主题", "作者"]) {
        const clear = [...h.document.querySelectorAll("button")].find(
          (button) => button.textContent === `清空${label}条件`
        )!;
        assert.ok(clear);
        await h.React.act(async () => clear.click());
        await h.flush();
        if (label === "主题") {
      ...
      assert.equal(recovered.get("device"), "pc");
      assert.equal(recovered.get("brightness"), "light");
...
test("[Web/公开导航] 显示名解析后的标签条件仍受规范串预算约束", () => {
  ...
  const filters = galleryFiltersFromSearchParams(
    new URLSearchParams({
      tag:
        "all:" +
        tags
...
test("[Web/公开导航] 公开图库筛选与随机图链接使用同一当前参数契约", () => {
  for (const theme of ["null", "!null", "none", "!none"]) {
    ...
    assert.equal(
      imageBrowseApiSearchParams(selected, "latest", { view: "gallery" }).get("theme"),
      theme
    );
  }
  const filters = galleryFiltersFromSearchParams(
  ...
  assert.deepEqual(galleryFiltersFromSearchParams(new URLSearchParams("device=all")), {
    device: "",
    brightness: "",
    theme: "",
    tag: "",
  ...
  assert.throws(
    () =>
      galleryFiltersFromSearchParams(
        new URLSearchParams("theme=stage,!editorial&tag=valid,INVALID_VALUE")
      ),

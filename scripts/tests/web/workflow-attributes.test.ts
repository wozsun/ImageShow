...
import {
  ...
  type ImageAttributeClearPlan
} from "../../../packages/web/src/lib/image-draft.ts";
...
test("[Web/主题] 模糊候选键盘选择提交真实 slug，直接输入仍可新建", async (t) => {
  ...
  await h.React.act(async () => {
    dispatchDomEvent(window, input, "keydown", { key: "ArrowDown" });
  });
  await h.React.act(async () => {
    dispatchDomEvent(window, input, "keydown", { key: "Enter" });
  });
  assert.deepEqual(values, ["hangzhou"]);
  await h.React.act(async () => {
    inputText(window, input, "new-place");
  });
  await h.React.act(async () => {
    dispatchDomEvent(window, input, "keydown", { key: "Enter" });
  });
...
test("[Web/主题] 保留值输入按留空处理且不提示新建主题", async (t) => {
  ...
  const input = h.document.querySelector<HTMLInputElement>('input[aria-label="主题"]')!;
  await h.React.act(async () => {
    inputText(h.window as unknown as Window, input, " NULL ");
  });
  assert.equal(input.hasAttribute("data-new-slug"), false);
  await h.React.act(async () => {
    dispatchDomEvent(h.window as unknown as Window, input, "focusout");
  });
  assert.deepEqual(values, [""]);
...
test("[Web/批量属性] 显式清空只更新指定分类，普通应用保留空输入并追加去重标签", () => {
  ...
  for (const field of Object.keys(cases) as ClearableImageAttribute[]) {
    ...
    const result = reduceIngestionQueue(
      { jobs, page: 1 },
      {
        type: "clear-attribute",
        field,
        attempts: new Map(
          jobs.filter((job) => job.id !== "new").map((job) => [job.id, "attempt-1"])
        )
      }
    );
  ...
  assert.deepEqual(
    mergeCommonImageAttributes(draft, {
      ...empty,
      theme: "new-theme",
      tags: [draft.tags[0]!, "new-tag", "new-tag"]
    }),
...
test("[Web/批量属性] 确认期间的本地接管按原身份清空，已覆盖任务与重试实例不扩大范围", async (t) => {
  ...
  const jobs = [
    ...
    "signing",
    "signed-handoff"
  ].map((id, batchPosition) => ingestionJob({ id, kind: "upload", batchPosition }));
  ...
  await React.act(async () => {
    ...
    queue.updateJob("signing", {
      ...
      imageId: "01900000-0000-7000-8000-000000000012"
    });
    ...
    queue.producerApi.bindServerJob(
      "covered",
      {
        ...
        imageId: "01900000-0000-7000-8000-000000000002",
        serverAccepted: true
      },
      undefined,
      10
    ...
    queue.producerApi.bindServerJob(
      ...
      {
        sessionId: "I".repeat(43),
        imageId: "01900000-0000-7000-8000-000000000099",
        serverAccepted: true
      },
      undefined,
      12
...
test("[Web/批量属性] 清空冻结水位，部分失败只重试失败身份，未知响应复用请求号", async (t) => {
  ...
  const server = {
    ...
    actionWatermark: "original-watermark",
    connectionGeneration: 1,
    lastAcceptedOrder: 10,
  ...
  const succeeded = {
    ...
    image_id: "01900000-0000-7000-8000-000000000003"
  };
  ...
  function Probe() {
    ...
    workflow = useIngestionQueueSubmitActions({
      ...
      onDone() {},
      captureServerAction: () => ({ required: false, frozen: null })
    });
    return null;
  }
  ...
  await harness.respond(
    0,
    response([
      { ...succeeded, status: "changed" },
      { ...failed, status: "failed", message: "暂时失败" }
    ])
  );
  ...
  await React.act(async () => {
    uncertainResult = uncertain.apply().catch((error: unknown) => error);
  });
  await harness.flush();
  ...
  await harness.render(React.createElement(Probe));
  await React.act(async () => {
    await assert.rejects(stale.apply(), /队列连接已变化/);
  });
...
for (const failure of [
  {
    label: "服务端错误",
    status: 503,
    body: '{"ok":false,"code":"response_lost","error":"response lost"}'
  },
  ...
  test(`[Web/批量属性] 后续页${failure.label}时续传原游标，保留此前逐项结果且不重复通知`, async (t) => {
    ...
    const server = {
      status: "ready",
      actionScope: "P".repeat(32),
      actionWatermark: "frozen-pages",
      connectionGeneration: 1,
      recoverAuthority: async () => {}
    };
    const hold = { current: false };
    ...
    function Probe() {
      actions = useIngestionQueueActions("upload", server as never, hold, () => {});
      return null;
    }
    ...
    const options = {
      onBatchResult: (result: IngestionQueueActionResultDto) =>
        notifications.push(...result.items.map((item) => item.session_id))
    };
    let initial!: ReturnType<typeof actions.run>;
    await React.act(async () => {
      initial = actions.run(frozen, undefined, options);
    });
    await harness.flush();
    const knownFailure = {
      ...
      message: "retry this identity"
    };
    await harness.respond(0, {
      ok: true,
      processed: 1,
      changed: 0,
      failed: 1,
    ...
    await React.act(async () => {
      resumed = actions.run(frozen, undefined, options);
    });
    await harness.flush();
    assert.deepEqual(body(3), body(1), "人工重试必须从服务端最后可重放的页继续");
    const second = {
      session_id: "S".repeat(43),
      image_id: "01900000-0000-7000-8000-000000000002",
      status: "changed" as const
    };
    const third = {
      session_id: "T".repeat(43),
      image_id: "01900000-0000-7000-8000-000000000003",
      status: "changed" as const
    };
    await harness.respond(3, {
      ok: true,
      processed: 1,
      changed: 1,
      failed: 0,
    ...
    assert.deepEqual(await resumed, {
      ...
      changed: 2,
      failed: 1,
      items: [knownFailure, second, third]
    });
...
test("[Web/批量属性] 移动浮层拥有菜单与确认框，按层关闭且确认不触发父表单保存", async (t) => {
  ...
  const { WorkflowAttributeActions } =
    await import("../../../packages/web/src/components/form/WorkflowAttributeActions.tsx");
  ...
  restorers.push(
    ...
    installProperties(document, {
      ...
      removeEventListener(
        ...
        if (
          ...
          keyCaptures.delete(listener);
        else removeListener(type, listener, options);
      }
    }),
    installPropertyDescriptors(document, {
      activeElement: { configurable: true, get: () => activeElement }
    }),
    ...
    installProperties(globalThis, {
      ...
      matchMedia: (media: string) => ({
        media,
        matches: media.includes("max-width") || media.includes("prefers-reduced-motion"),
        addEventListener() {},
        removeEventListener() {}
      })
    })
  ...
  const send = async (target: EventTarget, type: string, properties = {}) => {
    ...
    await React.act(async () => {
      dispatchDomEvent(window, target, type, {
        ...(type === "touchstart"
          ? { touches: Object.assign(touches, { item: (index: number) => touches[index] ?? null }) }
          : {}),
        ...properties
      });
    });

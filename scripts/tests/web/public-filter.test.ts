...
import { createPublicFilterDraft, publicDraftFilters, publicFilterChips, groupPublicFilterChips, resolvePublicFilterTag, createTagSelection } from "../../../packages/web/src/lib/gallery/public-filter-draft.ts";
...
test("[Web/公开筛选] 主题和作者共享 32 项及字符边界，拒绝混合条件和无效片段", () => {
  ...
  for (const field of ["theme", "author"] as const) {
    for (const mode of ["include", "exclude"] as const) {
      ...
      assert.equal(galleryFiltersFromSearchParams(new URLSearchParams({ [field]: value }))[field], value);
    ...
    for (const value of ["a,!b", "a,invalid_slug", "!", "a,", "x".repeat(33)]) {
      assert.throws(() => galleryFiltersFromSearchParams(new URLSearchParams({ [field]: value })), { name: "GallerySelectorError", field });
    }
    assert.throws(() => gallerySelectorValue(field, Array.from({ length: 32 }, (_, i) => `s${String(i).padStart(2, "0")}${"x".repeat(29)}`)), /长度限制/);
  }
});
...
test("[Web/公开筛选] 取消标签使重复组拆开超额时仍可继续移除，恢复后再统计应用", async (t) => {
  ...
  Object.assign(h.window, { scrollY: 0, scrollTo() {}, getComputedStyle: () => ({ paddingRight: "0px", getPropertyValue: () => "16px" }) });
  const client = new QueryClient({ defaultOptions: { queries: { retry: false, gcTime: Infinity } } });
  t.after(() => client.clear());
  const options = Array.from({ length: 17 }, (_, i) => ({ slug: `t${i}`, display_name: `标签${i}`, image_count: 1 }));
  const group = "all:" + options.map(option => option.slug).sort().join(",");
  const filters = { ...emptyGalleryFilters, tag: [group, group] };
  const stats: GalleryStatsDto = { total_images: 1, matching_images: 1, themes: [], authors: [],
    tags: options, devices: [], brightnesses: [], categories: [], tag_groups: [{ tag: group, image_count: 1 }, { tag: group, image_count: 1 }] };
  ...
  await h.render(h.React.createElement(QueryClientProvider, { client }, h.React.createElement(PublicFilterDialog, {
    filters, unresolvedTags: [], facets: { themes: [], authors: [], tags: options }, facetsLoading: false,
    facetsError: null, retryVocabulary() {}, returnFocusRef: { current: null }, onClose() {}, onApply() {}, view: "gallery"
  })));
  const card = (slug: string) => h.document.querySelector<HTMLButtonElement>(`[data-filter-section="tag"] [data-filter-option="${slug}"]`)!;
  ...
  assert.match(h.document.querySelector('.public-filter-notice[role="alert"]')!.textContent!, /去重后最多 32/);
...
test("[Web/公开筛选] 无效链接逐字段恢复，弹窗取消不丢条件且分享保持禁用", async (t) => {
  ...
  function Probe() {
    route = useImageBrowseRoute(); dialog = usePublicFilterDialog(route);
    return route.error ? h.React.createElement(PublicFilterErrorState, {
      error: route.error, onClear: field => route.updateFilter(field, ""), onRetry: route.retryVocabulary
    }) : null;
  }
  const original = "theme=city%2C%21forest&author=artist%2C%21other&tag=all%3A&device=pc&brightness=dark";
  await h.render(h.React.createElement(QueryClientProvider, { client },
    h.React.createElement(MemoryRouter, { initialEntries: ["/gallery?" + original] }, h.React.createElement(Probe))));
  assert.equal(route.ready, false); assert.equal(route.getPageUrl("latest"), null); assert.equal(route.randomLink.url, null);
  await h.React.act(async () => dialog.open());
  const session = dialog.session!;
  const draft = createPublicFilterDraft(session.filters, session.unresolvedTags, session.unresolvedSelectors);
  ...
  for (const field of ["theme", "author", "tag"] as const) {
    const button = [...h.document.querySelectorAll("button")].find(button => button.textContent === `清空${{ theme: "主题", author: "作者", tag: "标签" }[field]}条件`)!;
  ...
  assert.ok(route.getPageUrl("latest")); assert.ok(route.randomLink.url);
  assert.equal(h.pending.length, 0);
  await h.React.act(async () => route.updateFilter("theme", "city,!forest"));
  await h.React.act(async () => route.clearFilters());
  assert.equal(route.ready, true); assert.equal(route.params.toString(), "");
});
...
test("[Web/公开筛选] 弹窗可逐项清除损坏条件，所有错误解除前不能应用或统计", async (t) => {
  ...
  t.after(() => client.clear());
  await h.render(h.React.createElement(QueryClientProvider, { client }, h.React.createElement(PublicFilterDialog, {
    filters: { ...emptyGalleryFilters, device: "pc" }, unresolvedTags: ["all:"],
    unresolvedSelectors: { theme: ["city,!forest"], author: ["artist,!other"] },
    facets, facetsLoading: false, facetsError: null, retryVocabulary() {},
    returnFocusRef: { current: null }, onClose() {}, onApply() {}, view: "gallery"
  })));
  const apply = () => h.document.querySelector<HTMLButtonElement>(".public-filter-apply")!;
  const clear = async (label: string) => h.React.act(async () => {
    const button = [...h.document.querySelectorAll("button")].find(button => button.textContent === `清除${label}条件`)!;
    assert.ok(button); button.click();
  });
  assert.equal(apply().disabled, true);
  await clear("主题"); assert.equal(apply().disabled, true);
  assert.match(h.document.body.textContent!, /artist,!other/);
  await clear("作者"); assert.equal(apply().disabled, true);
  assert.equal(h.pending.filter(request => request.path.includes("device=pc")).length, 0);
  await clear("标签"); assert.equal(apply().disabled, false);
...
test("[Web/首页] 第 33 个主题或作者被拒绝，移除已有项后可以重新选择", async (t) => {
  const h = await createConfigStreamHarness(t);
  const options = Array.from({ length: 33 }, (_, i) => ({ slug: `s${i}`, display_name: `选项${i}`, image_count: 1 }));
  const stats: GalleryStatsDto = { total_images: 33, matching_images: 33, themes: options, authors: options.map(option => ({ ...option, link: "" })),
    tags: [], devices: [], brightnesses: [], categories: [] };
  let selected = emptyGalleryFilters;
  function Probe() {
    const [filters, change] = h.React.useState({ ...emptyGalleryFilters,
      theme: options.slice(0, 32).map(o => o.slug).join(","), author: options.slice(0, 32).map(o => o.slug).join(",") });
    selected = filters;
    return h.React.createElement(HomeCatalog, { catalogRef: { current: null }, armed: false, filters, stats,
      isPending: false, isError: false, isRefreshing: false, availabilityUnverified: false,
      onFiltersChange: change, tagMode: "any", onTagModeChange() {}, onRetry() {}, onCatalogIntent() {} });
  }
  await h.render(h.React.createElement(Probe));
  for (const field of ["theme", "author"] as const) {
    const buttons = () => [...h.document.querySelectorAll<HTMLButtonElement>(`.home-${field}-options button`)];
...
test("[Web/公开筛选] 弹窗拒绝第 33 项且保留当前可应用的包含或排除条件", async (t) => {
  for (const field of ["theme", "author"] as const) for (const exclude of [false, true]) {
    await t.test(`${field}/${exclude ? "exclude" : "include"}`, async (t) => {
      const h = await createConfigStreamHarness(t);
      Object.assign(h.window, { scrollY: 0, scrollTo() {}, getComputedStyle: () => ({ paddingRight: "0px", getPropertyValue: () => "16px" }) });
      const client = new QueryClient({ defaultOptions: { queries: { retry: false, gcTime: Infinity } } });
      t.after(() => client.clear());
      const options = Array.from({ length: 33 }, (_, i) => ({ slug: `s${i}`, display_name: `选项${i}`, image_count: 1, link: "" }));
      const filters = { ...emptyGalleryFilters, [field]: options.slice(0, 32).map(o => `${exclude ? "!" : ""}${o.slug}`).join(",") };
      const stats: GalleryStatsDto = { total_images: 33, matching_images: 1, themes: options, authors: options,
        tags: [], devices: [], brightnesses: [], categories: [] };
      client.setQueryData([...queryKeys.galleryStats, ""], stats);
      client.setQueryData([...queryKeys.galleryStats, galleryStatsSearch(filters)], stats);
      await h.render(h.React.createElement(QueryClientProvider, { client }, h.React.createElement(PublicFilterDialog, {
        filters, unresolvedTags: [], facets: { themes: options, authors: options, tags: [] }, facetsLoading: false,
        facetsError: null, retryVocabulary() {}, returnFocusRef: { current: null }, onClose() {}, onApply() {}, view: "gallery"
      })));
      const card = () => h.document.querySelector<HTMLButtonElement>(`[data-filter-section="${field}"] [data-filter-option="s32"]`)!;
      ...
      assert.match(h.document.body.textContent!, /最多选择 32 项/);
      assert.equal(h.document.querySelector(".public-filter-selected-count b")?.textContent, "32");
      assert.equal(h.document.querySelector<HTMLButtonElement>(".public-filter-apply")!.disabled, false);
...
test("[Web/公开筛选] 目录解析隔离未知标签，清空可恢复，组数和总预算有界", () => {
  ...
  assert.throws(() => createTagSelection([Array.from({ length: 33 }, (_, i) => `night-${i}`).join(",")]), /32/);
  assert.throws(() => publicDraftFilters({ ...createPublicFilterDraft(), theme: { mode: "include", selected: Array.from({length:40}, (_, i) => `theme-${i}`) } }, createTagSelection()), /最多选择 32 项/);
});
...
test("[Web/公开筛选] 统计与列表匹配语义相同，统计保留组边界和当前且组", () => {
  for (const ua of ["Mozilla/5.0 (iPhone; CPU iPhone OS 18_0 like Mac OS X)", "Mozilla/5.0 (Windows NT 10.0; Win64; x64)", ""]) {
    ...
    assert.deepEqual(parseGalleryTagFilter(list.getAll("tag")).expression, parseGalleryTagFilter(stats.getAll("tag")).expression);
    assert.deepEqual(stats.getAll("tag"), filters.tag);
    list.delete("tag"); stats.delete("tag");
    ...
    assert.equal(new URLSearchParams(galleryStatsSearch({ ...filters, tag: "all:night" }, ua)).get("tag_scope"), "1");
    assert.equal(new URLSearchParams(galleryStatsSearch({ ...filters, tag: "all:night" }, ua, null)).get("tag_scope"), null);
  }
});

test("[Web/公开筛选] 已选栏按非空组展示或且，跨组重复标签去重计数", () => {
  const draft = createPublicFilterDraft({ ...emptyGalleryFilters, theme: "!city", tag: ["night,rain", "all:night,rain"] });
  ...
  assert.deepEqual(groupPublicFilterChips(chips, selection).filter(chip => chip.section === "tag")
    .map(chip => [chip.groupId, chip.label]), [[1, "夜景 / 雨景"], [2, "夜景 & 雨景"]]);
  const withEmptyGroup = { ...selection, groups: [...selection.groups, { id: 3, mode: "any" as const, selected: [] }], activeId: 3 };
  assert.deepEqual(groupPublicFilterChips(chips, withEmptyGroup), groupPublicFilterChips(chips, selection));
});
...
test("[Web/公开筛选] 应用等待滚动解锁且只写入一次，取消与路由返回撤销旧草稿", async (t) => {
  ...
  function Probe() {
    ...
    dialog = usePublicFilterDialog({ filters: emptyGalleryFilters, ready: true, params: new URLSearchParams(), unresolvedSelectors: {}, applyFilters: filters => { applied.push(filters); } });

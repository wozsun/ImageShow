  ...
  function Navigation() {
    ...
    return h.React.createElement(
      ...
      /^\/(?:embed\/)?(?:gallery|show)\/?$/i.test(pathname)
        ? h.React.createElement(ImageControls)
        : null
    );
  }
...
test("[Web/公开视口] 嵌入安全区校验父窗口、实例、来源和尺寸，全屏及卸载恢复原生值", async (t) => {
  ...
  const restoreCrypto = installProperties(globalThis, {
    crypto: {
  ...
  const send = (
    ...
    h.window.dispatchEvent(
      Object.assign(new Event("message"), {
        source,
        origin,
        data: { ...ready, type: "insets", insets, ...data }
      })
    );
  };
  const bottom = () =>
    h.document.documentElement.style.getPropertyValue("--embed-safe-area-bottom");
  for (const invalid of [
    { version: 2 },
    { bridgeId: "stale" },
    { insets: { ...insets, bottom: -1 } },
    { insets: { ...insets, bottom: Infinity } },
    { insets: { ...insets, top: 301 } },
    { insets: {} }
  ]) {
    send(invalid);
    assert.ok(!bottom());
...
test("[Web/公开视口] 原生安全区按订阅上报、合并变化，断开和重挂释放旧实例", async (t) => {
  ...
  const parent = {
    postMessage: (data: Record<string, unknown>, origin: string) => messages.push({ data, origin })
  };
  const native = { top: 23, right: 17, bottom: 31, left: 11 };
  const observers = new Set<{ notify: () => void; targets: Set<HTMLElement> }>();
  const restoreObserver = installProperties(globalThis, {
    ResizeObserver: class {
      ...
      observe(target: HTMLElement) {
        ...
        const measure = (css: string) =>
          native[
            css.match(/safe-area-inset-(top|right|bottom|left)/)?.[1] as keyof typeof native
          ] ?? 0;
        target.getBoundingClientRect = () =>
          ({
            width: measure(target.style.width),
            height: measure(target.style.height)
          }) as DOMRect;
      }
      disconnect() {
  ...
  for (const extra of [{ version: 2 }, { channel: "other" }, { bridgeId: "stale" }])
    send("connect", extra);
  send("connect", {}, {});
...
test("[Web/公开视口] 菜单在公开页安全边界内定位，后台保留现有可视视口边界", async (t) => {
  ...
  const restoreStyle = installProperties(globalThis, {
    getComputedStyle: () => ({
...
test("[Web/公开视口] 已打开菜单随安全区单独变化重排，关闭后释放观察资源", async (t) => {
  ...
  const restore = installProperties(globalThis, {
    getComputedStyle: () => ({
      getPropertyValue: (name: string) => (name.endsWith("bottom") ? `${bottom}px` : "0px")
    }),
    ResizeObserver: class {
  ...
  anchor.getBoundingClientRect = () =>
    ({ left: 40, right: 140, width: 100, top: 70, bottom: 114 }) as DOMRect;
  const triggerRef = { current: anchor };
  function Menu() {
    menu = useAnchoredMenu({
      triggerRef,

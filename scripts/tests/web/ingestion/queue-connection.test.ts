...
test("[Web/内容接入] Server 内容接入队列 Hook 在重连与任意分页时只保留一个 SSE 和当前页基线", async (t) => {
  ...
  try {
    ...
    function Probe(
      ...
      const view = useServerIngestionQueue({
        ...
        onCompletedIngestions: (entries) => {
          probeCompletedObservations.push(...entries.map(({ pair }) => pair.image_id));
        },
    ...
    let recoverOwnerAfterSuccessfulAction:
      ((result: IngestionQueueActionResultDto) => Promise<void>) | undefined;
    let projectOwnerCompletedCleanupBatch:
      ((result: IngestionQueueActionResultDto) => number) | undefined;
    ...
    let bindLoadingAcceptedOwner: ((requestGeneration: number) => void) | undefined;
    ...
    const crossGenerationCompletedImageId = "00000000-0000-7019-8000-00000000008e";
    const crossGenerationPresentSessionId = "P".repeat(43);
    const crossGenerationPresentImageId = "00000000-0000-7018-8000-00000000008e";
    ...
    const offPageCompletionSessionIds = Array.from({ length: 23 }, (_, index) =>
      String(index).padStart(43, "C")
    );
    const offPageCompletionImageIds = Array.from(
      { length: 23 },
      (_, index) => `019f8457-063a-7${index.toString(16).padStart(3, "0")}` + "-b580-00000000008e"
    );
    const compactChunkFailureSessionIds = Array.from({ length: 102 }, (_, index) =>
      index.toString(36).padStart(43, "Z")
    );
    const compactChunkFailureImageIds = Array.from(
      { length: 102 },
      (_, index) => `019f8457-063a-7${index.toString(16).padStart(3, "0")}` + "-c580-00000000008e"
    );
    function OwnerProbe(props: Readonly<{ displayed: boolean }>) {
      ...
      markOwnerCompletedBrowserOwned = (imageId) => {
        const job = queue.jobsRef.current.find((item) => item.imageId === imageId);
        assert.ok(job);
      ...
      prepareOffPageCompletionBatch = () => {
        queue.appendJobs(
          offPageCompletionImageIds.map((imageId, index) =>
            ingestionJob({
      ...
      bindOwnerHandoff = (revision) => {
        ...
        queue.producerApi.bindServerJob(
          placeholder.id,
          {
      ...
      bindUnknownCompletedHandoff = () => {
        const placeholder = queue.jobsRef.current.find((job) => job.id === unknownPlaceholderId);
      ...
      bindCrossGenerationCompletedCoverage = () => {
        ...
        const placeholders = Array.from({ length: 61 }, (_, index) =>
          ingestionJob({
            id:
              index === 40
      ...
      prepareVisibleReadyRelease = () => {
        ...
        visibleReadyReleaseBeforeIds = queue.jobsRef.current.flatMap((job) =>
          job.imageId &&
          visibleReadyReleaseImageIds.includes(
            job.imageId as (typeof visibleReadyReleaseImageIds)[number]
          )
            ? [job.imageId]
            : []
        );
      ...
      return React.createElement(
        ...
        JSON.stringify({
          ...
          reconnectDone: queue.jobsRef.current.filter(
            (job) =>
              job.status === "done" &&
              (job.sessionId === reconnectVisibleSessionId ||
                job.sessionId === reconnectOffPageSessionId)
          ).length,
          offPageCompletionOwners: queue.jobsRef.current.filter(
            (job) => job.imageId && offPageCompletionImageIds.includes(job.imageId)
          ).length,
          offPageCompletionDone: queue.jobsRef.current.filter(
            (job) =>
              job.status === "done" &&
              job.imageId &&
              offPageCompletionImageIds.includes(job.imageId)
          ).length,
          compactChunkHydrated: queue.jobsRef.current.filter(
            (job) =>
    ...
    const unsubscribeQueryEvents = queryClient.getQueryCache().subscribe((event) => {
      if (
        event.type === "updated" &&
        event.action.type === "invalidate" &&
        JSON.stringify(event.query.queryKey) === JSON.stringify(queryKeys.adminImages)
      )
    ...
    try {
      ...
      await runQueueScenario(
        ...
        async () => {
          const emptyView = () =>
            JSON.parse(emptyContainer.textContent || "{}") as {
              status: string;
              generation: number;
              revision: number | null;
              total: number | null;
          ...
          try {
            await React.act(async () => {
              emptyRoot.render(
                withQueryClient(React.createElement(Probe, { enabled: true, offset: 0 }))
              );
              await Promise.resolve();
            });
            ...
            await settleUntil(() => emptyView().status === "ready" && emptyView().total === 0);
      ...
      await runQueueScenario(
        ...
        async () => {
          await React.act(async () => {
            root.render(withQueryClient(React.createElement(Probe, { enabled: true, offset: 40 })));
            await Promise.resolve();
          });
          assert.equal(ControlledEventSource.all.length, 1);
          assert.equal(ControlledEventSource.active.size, 1);
          ...
          await settleUntil(() => view().watermark === "watermark-completed-before-cleanup");
          ...
          await settleUntil(() => view().watermark === "watermark-post-cleanup-stable");
          requests.splice(postCleanupRefreshStart, 1);

          const offPageCompletedImageId = "00000000-0000-7093-8000-00000000008e";
          await React.act(async () => {
            source.emit("mutation", {
              type: "mutation",
              queue: "upload",
          ...
          await React.act(async () => {
            root.render(withQueryClient(React.createElement(Probe, { enabled: true, offset: 20 })));
            await Promise.resolve();
          });
          ...
          await settleUntil(() => view().watermark === "watermark-coalesced-first");
          await React.act(async () => {
            await new Promise((resolve) => setTimeout(resolve, 0));
          });
          assert.equal(requests[coalescedRefreshStart]!.aborted, false);
          ...
          await settleUntil(() => view().watermark === "watermark-after-unknown-coverage");
          ...
          await settleUntil(() => view().watermark === "watermark-reverted-failure-recovered");
          ...
          await settleUntil(() => view().watermark === "watermark-current-filter");
          await React.act(async () => {
            root.render(withQueryClient(React.createElement(Probe, { enabled: true, offset: 20 })));
            await Promise.resolve();
          });
          ...
          assert.equal(
            new Headers(trailingReconnectRequest.init.headers).get(ingestionActionScopeHeader),
            "scope-five",
          ...
          await React.act(async () => {
            requests[automaticRecoveryStart]!.resolve(
              new Response(
                JSON.stringify({
                  ok: false,
          ...
          await React.act(async () => {
            requests[mutationRecoveryStart]!.resolve(
              new Response(
                JSON.stringify({
                  ok: false,
          ...
          await React.act(async () => {
            ...
            requests[boundedFailureStart]!.resolve(
              new Response(
                JSON.stringify({
                  ok: false,
          ...
          await React.act(async () => {
            ...
            requests[boundedFailureStart + 1]!.resolve(
              new Response(
                JSON.stringify({
                  ok: false,
          ...
          assert.equal(view().items[0]?.status, "ready");
          await React.act(async () => {
            requests[retainedRefreshStart]!.resolve(
              new Response(
                JSON.stringify({
                  ok: false,
          ...
          await React.act(async () => {
            root.render(
              withQueryClient(React.createElement(Probe, { enabled: false, offset: 40 }))
            );
            await Promise.resolve();
          });
      ...
      await runQueueScenario(
        ...
        async () => {
          ...
          await React.act(async () => {
            ownerRoot.render(withQueryClient(React.createElement(OwnerProbe, { displayed: true })));
            await Promise.resolve();
          });
          await settleUntil(() => ControlledEventSource.all.length === ownerSourceStart + 1);
          ...
          const ownerView = () =>
            JSON.parse(ownerContainer.textContent || "{}") as {
              status: string;
              generation: number;
              revision: number | null;
              page: number;
          ...
          assert.equal(
            new URL(
              requests[ownerRequestStart + 1]!.url,
          ...
          await settleUntil(() => ownerView().status === "ready" && ownerView().page === 3);
          ...
          assert.equal(
            new URL(
              requests[ownerRequestStart + 3]!.url,
              "https://imageshow.test"
            ).searchParams.get("offset"),
            "0"
          );
          ...
          assert.equal(
            ownerView().visible.filter((job) => job.id === unknownImageId).length,
            0,
          ...
          await settleUntil(() =>
            Boolean(pendingRequest(ingestionStatusPath, unknownRequestStart))
          );
          await React.act(async () => {
          ...
          const oldGenerationStatusCount = requests
            .slice(unknownRequestStart)
            .filter(
              (request) =>
                !request.aborted &&
          ...
          await settleUntil(() => Boolean(pendingRequest(ingestionSnapshotPath, coldRequestStart)));
          await React.act(async () => {
          ...
          await settleUntil(() => Boolean(pendingRequest(ingestionStatusPath, coldRequestStart)));
          await React.act(async () => {
          ...
          await settleUntil(() =>
            Boolean(pendingRequest(ingestionSnapshotPath, coverageBaselineStart))
          );
          await React.act(async () => {
          ...
          await settleUntil(() =>
            Boolean(pendingRequest(ingestionSnapshotPath, coveredRequestStart))
          );
          await React.act(async () => {
          ...
          await settleUntil(() =>
            Boolean(pendingRequest(ingestionStatusPath, coveredRequestStart))
          );
          await React.act(async () => {
          ...
          await settleUntil(() =>
            Boolean(pendingRequest(ingestionStatusPath, crossGenerationRequestStart))
          );
          assert.equal(
            requests
          ...
          await React.act(async () => {
            respond(pendingRequest(ingestionStatusPath, crossGenerationRequestStart)!, {
              items: [
                {
                  session_id: crossGenerationSessionId,
                  image_id: crossGenerationImageId,
          ...
          await settleUntil(() =>
            Boolean(pendingRequest(ingestionStatusPath, completedCoverageStart))
          );
          assert.equal(
          ...
          await settleUntil(() =>
            Boolean(pendingRequest(ingestionStatusPath, presentCoverageStart))
          );
          assert.equal(
            requests
          ...
          await settleUntil(() => !ownerView().pendingHandoff && ownerView().reconnectDone === 2);
          assert.equal(
            requests
              .slice(reconnectAcceptedStart)
              .filter(
                (request) =>
                  !request.aborted &&
                  new URL(request.url, "https://imageshow.test").pathname === ingestionStatusPath
          ...
          await settleUntil(() =>
            Boolean(pendingRequest(ingestionStatusPath, loadingAcceptedStart))
          );
          ...
          const completedSnapshotImageId = "00000000-0000-7044-8000-00000000008e";
          ...
          await settleUntil(() => Boolean(pendingRequest(ingestionSnapshotPath)));
          ...
          await React.act(async () => {
            respond(repeatedCompletedSnapshot, {
              ...
              revision: 116,
              last_accepted_order: 11,
              offset: Number(repeatedCompletedSnapshotUrl.searchParams.get("offset")),
              limit: Number(repeatedCompletedSnapshotUrl.searchParams.get("limit")),
              total: 2,
              unfinished: 1,
              waiting: 0,
              running: 0,
          ...
          await settleUntil(() => ownerView().status === "ready");
          await settleUntil(() => Boolean(pendingRequest(ingestionStatusPath, renderGapStart)));
          await React.act(async () => {
          ...
          await settleUntil(() => ownerView().status === "ready");
          assert.equal(
            ownerView().visible.some((job) => job.id === visibleReadyReleaseTargetImageId),
            false,
          ...
          await React.act(async () => {
            respond(singleReleaseSnapshot, {
              ...
              items: remainingReadyReleaseImageIds.map((imageId) => ({
                ...
                session_id:
                  visibleReadyReleaseSessionIds[visibleReadyReleaseImageIds.indexOf(imageId)],
          ...
          await settleUntil(() => ownerView().status === "ready" && ownerView().page === 2);
          ...
          await React.act(async () => {
            absentReleaseFailedProof.resolve(
              new Response(
                JSON.stringify({
                  ok: false,
          ...
          await React.act(async () => {
            respond(absentReleaseProof, {
              ...
              failed: 0,
              items: boundaryItems
                .filter((item) => item.image_id !== absentReleaseImageId)
                .map((item) => ({
                  ...item,
          ...
          for (let attempt = 0; attempt < 4; attempt += 1) {
            ...
            await React.act(async () => {
              failedRecoverySnapshot.resolve(
                new Response(
                  JSON.stringify({
                    ok: false,
          ...
          await React.act(async () => {
            ownerRoot.render(
              withQueryClient(React.createElement(OwnerProbe, { displayed: false }))
            );
            await Promise.resolve();
          });
          ...
          const clearedCompletedImageId = "00000000-0000-7046-8000-00000000008e";
          const retainedCompletedSessionId = "O".repeat(43);
          const retainedCompletedImageId = "00000000-0000-7047-8000-00000000008e";
          const browserCompletedSessionId = "R".repeat(43);
          const browserCompletedImageId = "00000000-0000-7048-8000-00000000008e";
          ...
          await React.act(async () => {
            respond(completedCleanupBaseline, {
              ...
              offset: Number(completedCleanupBaselineUrl.searchParams.get("offset")),
              limit: Number(completedCleanupBaselineUrl.searchParams.get("limit")),
              total: 3,
              unfinished: 0,
              waiting: 0,
              running: 0,
          ...
          assert.ok(ownerView().visible.some((job) => job.id === browserCompletedImageId));
          ...
          await React.act(async () => {
            completedCleanupRecovery = recoverOwnerAfterSuccessfulAction!(completedCleanupResult);
            await Promise.resolve();
          });
          ...
          await React.act(async () => {
            respond(completedCleanupRecoverySnapshot, {
              ...
              offset: Number(completedCleanupRecoveryUrl.searchParams.get("offset")),
              limit: Number(completedCleanupRecoveryUrl.searchParams.get("limit")),
              total: 1,
              unfinished: 0,
              waiting: 0,
              running: 0,
                ...
                session:
                  index === 22
                    ? {
                        ...
                        version: 6,
                        progress_seq: 0,
                        last_semantic_revision: offPageCompletionBaseRevision + index + 1,
                        accepted_order: 20 + index
                      }
                    : {
                        session_id: offPageCompletionSessionIds[index],
              ...
              items: [
                {
                  ...
                  redis_status: "completed",
                  redis_version: 6,
                  redis_last_semantic_revision: offPageCompletionBaseRevision + 23
                }
              ]
            });
            await Promise.resolve();
          ...
          await React.act(async () => {
            respond(secondPageSnapshot, {
              ...
              items: offPageCompletionImageIds.slice(20).map((completedImageId, offset) => ({
                ...
                version: 6,
                progress_seq: 0,
                last_semantic_revision: offPageCompletionBaseRevision + 21 + offset,
          ...
          await React.act(async () => {
            respond(compactChunkBaselineRequest, {
              ...
              items: [
                ...
                ...compactChunkFailureImageIds
                  .slice(0, 20)
                  .map((_imageId, index) => compactChunkActiveItem(index))
              ],
          ...
          const compactCompletedStatus = (index: number) => ({
            ...
            redis_version: 6,
            redis_last_semantic_revision: compactChunkBaselineRevision + index + 1
          });
          const compactCompletedMutation = (index: number) => {
            ...
            return {
              ...
              session: {
                ...
                version: 6,
                progress_seq: 0,
                last_semantic_revision: compactChunkBaselineRevision + completedCount,
                accepted_order: 43 + index
              },
          ...
          const compactChunkFirstPairs = JSON.parse(String(compactChunkFirstRequest.init.body))
          ...
          const compactChunkRequests = requests
          ...
          assert.deepEqual(JSON.parse(String(compactChunkFailedSecondRequest.init.body)).items, [
            {
          ...
          await React.act(async () => {
            compactChunkFailedSecondRequest.resolve(
              new Response(
                JSON.stringify({
                  ok: false,
          ...
          await settleUntil(() =>
            ownerView().serverNotice.includes("controlled compact second chunk failure")
          );
          assert.equal(
            adminImageInvalidations,
          ...
          assert.deepEqual(
            (
              JSON.parse(String(compactChunkRetryFirstRequest.init.body)).items as Array<{
                image_id: string;
              }>
          ...
          await settleUntil(
            () => adminImageInvalidations >= invalidationsBeforeCompactChunkFailure + 1
          );
          assert.equal(
            adminImageInvalidations,

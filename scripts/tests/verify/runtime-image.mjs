...
expectedRuntimeConfig.log.level = runtimeSeeds.LOG_LEVEL;
...
// Keep both required character classes deterministic; base64url randomness
// does not guarantee that a short sample contains a digit.
const adminPassword = `Verify1-${randomBytes(18).toString("base64url")}`;
...
try {
  ...
  if (!interruptedSignal) {
    ...
    const logText = logs ? resultText(logs) : "";
    if (logText) {

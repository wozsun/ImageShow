...
const phases = {
  ...
  build: [
    ...
    ["Web build contract", "node", ["scripts/tests/verify/check-web-chunks.mjs"]]
  ],
  runtime: [
    [
      "Server acceptance",
    ...
    [
      ...
      "node",
      ["scripts/tests/verify/runtime-image.mjs"],
      { cooperativeShutdown: true }
    ]
  ]
};
...
if (!mode || !["source", "build", "runtime", "release"].includes(mode)) {
  throw new Error("Usage: node scripts/tests/verify/run.mjs <source|build|runtime|release>");
}

...
test("[Server/标签] 词项预算按规范表达式计数并独立约束原始输入", () => {
  ...
  assert.throws(
    () => parseTagFilter([Array.from({ length: 33 }, (_, i) => `t${i}`).join(",")]),
    /32/
  );
...
test("[Server/标签] 有限集合运算与图片资格一致且重叠分支只产生一个成员", () => {
  ...
  const execute = (expression: TagExpression, withOtherFilters: boolean) => {
    ...
    const tags = new Map(
      [...slugs, "empty"].map((slug) => [
        slug,
        source(
          slug,
          images.filter((item) => item.tags.includes(slug)).map(({ id }) => id)
        )
      ])
    );
    let sequence = 0;
    const execution = readyImageFilterOperations(
      {
        ...
        positive: withOtherFilters
          ? [
              [
                source(
                  "axis",
                  images.filter(({ id }) => id % 2 === 0).map(({ id }) => id)
                )
              ]
            ]
          : [],
    ...
    for (const withOtherFilters of [false, true]) {
      const expected = images
        .filter(
          (item) =>
            (!expression ||
              expression.anyOf.some((clause) => clause.every((tag) => item.tags.includes(tag)))) &&
            (!withOtherFilters || (item.id % 2 === 0 && item.id !== 2 && item.id !== 4))
        )
        .map(({ id }) => id);
      assert.deepEqual(
        execute(expression, withOtherFilters),
        expected,
        JSON.stringify({ values, withOtherFilters })
      );
    }
  }
  assert.equal(
...
test("[Server/筛选] 完整设备候选复用属性索引，亮度和组合条件保留独立语义", () => {
  for (const device of ["pc", "mb"] as const) {
    ...
    assert.equal(
      resolveDirectReadyImageFilterKey(createImageFilterPlan({ devices: [device] })),
      key
    );
    for (const brightness of ["dark", "light"] as const) {
      assert.equal(
        resolveDirectReadyImageFilterKey(
          createImageFilterPlan({ devices: [device], brightnesses: [brightness] })
        ),
    ...
    assert.equal(
      resolveDirectReadyImageFilterKey(
        createImageFilterPlan({ devices: [device], tag: { anyOf: [["tag-a"]] } })
      ),
      null
    );
  }
  assert.equal(
    resolveDirectReadyImageFilterKey(createImageFilterPlan({})),
    READY_IMAGE_ALL_INDEX_KEY
  );
  assert.equal(
    resolveDirectReadyImageFilterKey(createImageFilterPlan({ brightnesses: ["light"] })),
    null
  );
  for (const value of ["auto", "all", "other", "pc:light", ""]) {
    assert.equal(
      readyImageAttributeIndexSpec(`imageshow:cache:images:derived:index:device:${value}`),
      null
    );
  }
});
...
test("[Server/筛选] 统计维度与成员工作量在预算内准入，超限拒绝", () => {
  ...
  assert.equal(
    assessReadyImageStatsWork({
      dynamicDimensions: policy.maxDynamicStatsDimensions,
      intersections: []
    }).admitted,
    true
  );
  assert.equal(
    assessReadyImageStatsWork({
      dynamicDimensions: policy.maxDynamicStatsDimensions + 1,
      intersections: []
    }).admitted,
    false
  );
  assert.equal(
    assessReadyImageStatsWork({
      dynamicDimensions: 1,
      intersections: [
        { baseCount: policy.maxCardinalitySourceMembersPerOperation, candidateCount: 1 }
      ]
    }).admitted,
    false
  );
  ...
  assert.equal(
    assessReadyImageFilterWork({ itemCount: count * 4, positive: [[count, count]], exclusions: [] })
      .admitted,
    true
  );
  assert.equal(
  ...
  assert.equal(
    assessReadyImageStatsWork({
      dynamicDimensions: 1,
      intersections: [{ baseCount: -1, candidateCount: 1 }]
    }).admitted,
    false
  );
});
...
test("[Server/筛选] 构建槽位有界，大任务独占且重复释放不扩大并发", () => {
  ...
  const small = {
    ...
    totalSourceMembers: 2,
    peakSourceMembers: 2,
    totalExpectedMembers: 1,
    peakExpectedMembers: 1,
    peakOperands: 2
  };
  for (const [acquire, limit, large] of [
    [
      tryAcquireReadyImageFilterBuildSlot,
      policy.maxConcurrentFilterBuilds,
      { ...small, totalSourceMembers: policy.largeFilterSourceMembers }
    ],
    [
      tryAcquireReadyImageStatsBuildSlot,
      policy.maxConcurrentStatsBuilds,
      { ...small, totalExpectedMembers: policy.largeStatsExpectedMembers }
    ]
  ] as const) {
    ...
    try {
      ...
      assert.equal(
        acquire(large),
        null,
        "one large task may remain active alongside smaller tasks"
      );
    } finally {

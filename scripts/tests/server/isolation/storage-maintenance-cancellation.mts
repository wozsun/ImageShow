...
await runIntegrationScenario(async (runtime) => {
  ...
  try {
    ...
    await removeDriverObject(access.driver, "full", incompleteKey);
  }
  ...
  for (const firstFinished of ["repair", "removal"] as const) {
    ...
    for (const key of orphanKeys)
      await access.driver.writeBuffer("full", key, Buffer.from("orphan"), "image/webp");
    ...
    try {
      await settleWithin(
        Promise.race([
          Promise.all([repairStarted.promise, removalStarted.promise]),
          pending.then(() => assert.fail("both resource pools must start"))
        ])
      );
      const source = (
        await pool.query<ImageStorageMigrationRecord>(
          "SELECT id, ext, storage_slug, md5, image_size, thumbnail_size FROM metadata WHERE id=$1",
          [image.id]
        )
      ).rows[0]!;
      ...
      stop.abort(reason);
      if (firstFinished === "repair") {
        repairGate.resolve();
        await settleWithin(repairFinished.promise);
      } else {
        removalGate.resolve();
        await settleWithin(removalFinished.promise);
      }
      await waitForStorageLockWait(pool, true);
      ...
      for (const key of orphanKeys)
        assert.equal(await access.driver.exists("full", key), !startedKeys.includes(key));
    } finally {
      stop.abort(reason);
      repairGate.resolve();
      removalGate.resolve();

...
await runIntegrationScenario(async (runtime) => {
  ...
  const { getAdminImageSnapshots } = await import("../../../../packages/server/src/images/read-models/admin-images.ts");
  const { withPublicDatabaseRead } = await import("../../../../packages/server/src/core/database/public-fallback.ts");
  const { imageUpdateLockRequests } = await import("../../../../packages/server/src/images/image-update-lock.ts");
  ...
  const restoreQuery = interceptSqlQueries(runtime.databasePools.pool, (sql, _values, query) => {
    if (sql.includes("WHERE id = ANY($1::uuid[])") && sql.includes("AND status = 'ready'")) snapshotSelects++;
    return query();
  });
  const waiting = async () => Number((await runtime.databasePools.pool.query(
  ...
  try {
    ...
    const abandoned = controllers.map((controller) => fetch(origin + "/api/admin/images/snapshot", {
      method: "POST", headers, body: JSON.stringify({ ids: [image.id] }), signal: controller.signal
    ...
    await assert.rejects(getAdminImageSnapshots([image.id], preAborted.signal), /cancel before checkout/);
    assert.equal(snapshotSelects, 0);

    const live = fetch(origin + "/api/admin/images/snapshot", {
      method: "POST", headers, body: JSON.stringify({ ids: [image.id] })
    });
    await waitForCount(1);
    await writer.query("UPDATE metadata SET title='authoritative-after-lock' WHERE id=$1", [image.id]);
    ...
    const read = withPublicDatabaseRead(readAbort.signal, ({ reader }) => reader.query(
      "SELECT pg_sleep(30) /* cancelled-public-read */"
    ));
    const readRejected = assert.rejects(read, (error) => error === readReason);
    const runningRead = async () => Number((await runtime.databasePools.pool.query(
      "SELECT count(*)::int AS count FROM pg_stat_activity WHERE datname=current_database() "
        + "AND state='active' AND query=$1", ["SELECT pg_sleep(30) /* cancelled-public-read */"]
    )).rows[0].count);
    ...
    const update = await fetch(origin + "/api/admin/images/update", {
      method: "POST", headers,
      body: JSON.stringify({ items: [{ id: image.id, title: "  normalized title  ", description: "\n  first\n second  \n", source: " example.com/post ", original: " https://example.com/image.jpg " }] })
    });
    assert.equal(update.status, 200, await update.clone().text());
    const normalized = await getAdminImageSnapshots([image.id]);
    assert.deepEqual(Object.fromEntries(["title", "description", "source", "original"].map((field) => [field, normalized.items[0]![field as "title" | "description" | "source" | "original"]])), {
      title: "normalized title", description: "first\n second", source: "https://example.com/post", original: "https://example.com/image.jpg"
    });
    console.log(JSON.stringify({ scenario: "image-snapshot-cancellation", cancelled: 4, waitingAfterAbort: 0, abandonedReads: 0, cancelledPublicQueries: 1, liveSnapshot: snapshot.items[0]?.title, normalized: true }));
  } finally {

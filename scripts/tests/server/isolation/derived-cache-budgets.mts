...
await runIntegrationScenario(async ({ redisClient: { redis } }) => {
  ...
  const { registerReadyImageDerivedResultUnchecked: register } =
    await import("../../../../packages/server/src/images/ready-cache/derived/registry.ts");
  const {
  ...
  const oneMiB = JSON.stringify({
    value:
      "界" +
      "a".repeat(policy.maxStatsResultBytes - Buffer.byteLength(JSON.stringify({ value: "界" })))
  });
  ...
  assert.equal(
    await register({ key: firstFilter, kind: "filter", count: 0, itemCount: 1000 }),
    true
  );
  ...
  assert.equal(
    await redis.exists(firstStats, firstFilter, firstMeta),
    0,
    "signature eviction retires its stats and filter metadata together"
  );
  ...
  const members = Array.from({ length: itemCount }, (_, i) => [
    String(i),
    `synthetic-member-${i}`
  ]).flat();
  const keys: string[] = [];

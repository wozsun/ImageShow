...
await runIntegrationScenario(async (runtime) => {
  ...
  const { resolveImageFilterPlan, createImageFilterPlan } =
    await import("../../../../packages/server/src/images/filter-plan.ts");
  const { sampleReadyImages, readReadyImageCursorPage } =
  ...
  await pool.query(
    "INSERT INTO theme(slug, display_name) VALUES ('matrix-city','城市'),('matrix-nature','自然'),('matrix-empty','空主题')"
  );
  await pool.query(
    "INSERT INTO author(slug, display_name) VALUES ('matrix-alice','作者甲'),('matrix-bob','作者乙'),('matrix-empty','空作者')"
  );
  for (const [index, slug] of allTags.entries()) {
    await pool.query("INSERT INTO tag(slug, display_name) VALUES ($1,$2)", [
      slug,
      ["城市", "夜景", "雨景", "森林", "空标签", "matrix-a", "A&B+C#%"][index] ?? slug
    ]);
  }
  ...
  const { parseReadyImageGlobalStats } =
    await import("../../../../packages/server/src/images/ready-cache/counts/model.ts");
  const { readReadyImageCountSnapshot } =
    await import("../../../../packages/server/src/images/ready-cache/counts/query.ts");
  const { resolveGalleryStatsPlan } =
    await import("../../../../packages/server/src/images/read-models/gallery-stats-plan.ts");
  const { READY_IMAGE_STATS_KEY } =
  ...
  const assertEmpty = (stats: Awaited<ReturnType<typeof getPublicGalleryStats>>) => {
    ...
    assert.deepEqual(stats.devices, [
      { device: "pc", image_count: 0 },
      { device: "mb", image_count: 0 }
    ]);
    assert.deepEqual(stats.brightnesses, [
      { brightness: "dark", image_count: 0 },
      { brightness: "light", image_count: 0 }
    ]);
  };
  ...
  for (const [index, row] of rows.entries()) {
    ...
    for (const tag of members)
      await pool.query("INSERT INTO image_tag(image_id,tag_slug) VALUES ($1,$2)", [row.id, tag]);
  }
  await vocab.refreshEntityVocabularies(["tag", "theme", "author"]);
  ...
  cases.push(
    ...
    {
      tags: ["all:matrix-a,matrix-b", "all:matrix-b,matrix-a"],
      match: (row) => (row.mask & 3) === 3
    },
    {
      tags: Array(9).fill("all:matrix-a,matrix-b,matrix-c,matrix-d"),
      match: (row) => (row.mask & 15) === 15
    },
    {
      tags: ["all:matrix-a,matrix-b", "matrix-c"],
      match: (row) => (row.mask & 3) === 3 || Boolean(row.mask & 4)
    },
    {
      tags: ["all:matrix-a,matrix-b", "all:matrix-c,matrix-d"],
      match: (row) => (row.mask & 3) === 3 || (row.mask & 12) === 12
    },
  ...
  for (const device of ["pc", "mb"])
    for (const brightness of ["dark", "light"]) {
      cases.push({
        tags: ["all:matrix-a,matrix-b"],
        axis: { device, brightness, theme: "!matrix-city", author: "matrix-alice,matrix-bob" },
        match: (row) =>
          (row.mask & 3) === 3 &&
          row.device === device &&
          row.brightness === brightness &&
          row.theme !== "matrix-city" &&
          row.author !== null
      });
    }
  cases.push({
  ...
  const groupMatches = (row: Row, value: string) => {
    ...
    const hasTag = (slug: string) =>
      tags.includes(slug) && Boolean(row.mask & (1 << tags.indexOf(slug)));
  ...
  try {
    ...
    for (const backend of ["PostgreSQL", "Redis"]) {
      if (backend === "Redis") {
        ...
        const context = {
          ...
          globalStats: parseReadyImageGlobalStats(
            await runtime.redisClient.redis.hgetall(READY_IMAGE_STATS_KEY),
            rows.length
          )
        };
        ...
        for (const query of [
          ...
          assert.deepEqual(
            mismatch.snapshot,
            full.snapshot,
            "revision mismatch discards all carried membership"
          );
        ...
        try {
          ...
          assert.equal(
            nextRead.selects,
            8,
            "the next transaction falls back under its newer revision"
          );
        } finally {
      ...
      for (const groups of statsGroups)
        for (const axes of statsAxes) {
          const scopes = [
            undefined,
            ...groups.flatMap((group, index) => (group.startsWith("all:") ? [index + 1] : []))
          ];
          for (const scope of scopes) {
            ...
            const expectedMatching = rows.filter(
              (row) => axisMatches(row, axes) && matchesTags(row)
            ).length;
            const expectedGroups = groups.map((tag) => ({
              tag,
              image_count: rows.filter((row) => axisMatches(row, axes) && groupMatches(row, tag))
                .length
            }));
            ...
            for (const slug of tags) {
              const expected = rows.filter(
                (row) =>
                  axisMatches(row, axes) &&
                  groupMatches(row, slug) &&
                  (scope === undefined || groupMatches(row, groups[scope - 1]!))
              ).length;
              assert.equal(
                body.tags.find((item: { slug: string }) => item.slug === slug)?.image_count,
                expected,
                `${label}: ${slug}`
              );
            }
            for (const [field, values] of Object.entries({
              ...
              for (const value of values) {
                const actual = body[field === "brightness" ? "brightnesses" : `${field}s`].find(
                  (item: Record<string, unknown>) =>
                    item[field === "theme" || field === "author" ? "slug" : field] === value
                )?.image_count;
                const expected = rows.filter(
                  (row) =>
                    axisMatches(row, axes, field) &&
                    matchesTags(row) &&
                    axisMatches(row, { [field]: value })
                ).length;
                assert.equal(actual, expected, `${label}: ${field} ${value}`);
              }
            if (backend === "Redis") {
              const { plan, tagCounts } = await resolveGalleryStatsPlan(
              ...
              if (cached.cached) {
                ...
                assert.deepEqual(
                  cached.value.tagGroups,
                  expectedGroups.map((group) => group.image_count),
                  label
                );
                ...
                for (const field of ["themes", "tags", "authors"] as const) {
                  assert.deepEqual(
                    cached.value[field],
                    Object.fromEntries(
                      body[field].map((item: { slug: string; image_count: number }) => [
                        item.slug,
                        item.image_count
                      ])
                    ),
                ...
                assert.deepEqual(
                  cached.value.devices,
                  Object.fromEntries(
                    body.devices.map((item: { device: string; image_count: number }) => [
                      item.device,
                      item.image_count
                    ])
                  ),
                ...
                assert.deepEqual(
                  cached.value.brightnesses,
                  Object.fromEntries(
                    body.brightnesses.map((item: { brightness: string; image_count: number }) => [
                      item.brightness,
                      item.image_count
                    ])
                  ),
                  `${label}: Redis brightnesses`
                );
              }
            }
            statsCases += 1;
      ...
      for (const query of [
        "tag=matrix-a&tag_scope=1",
        "tag=all:matrix-a&tag_scope=2",
        "tag_scope=1",
        "tag=all:matrix-a&tag_scope=0"
      ]) {
        assert.equal(
          (await get(`/api/gallery-stats?${query}`)).status,
          400,
          `${backend}: invalid scope ${query}`
        );
      }
      ...
      for (const entry of cases) {
        ...
        if (backend === "PostgreSQL") postgresStats.set(query.toString(), body);
          ...
          assert.deepEqual(
            body,
            postgresStats.get(query.toString()),
            `${label}: complete statistics DTO`
          );
    ...
    for (const entry of [
      ...
      {
        tags: [`all:${budgetTags.join(",")}`, "matrix-c"],
        expected: rows.filter((row, index) => index === 0 || Boolean(row.mask & 4))
      }
    ]) {
    ...
    assert.equal(
      (await updateImages([{ id: changed.id, tags: ["matrix-a", "matrix-b"] }])).updated,
      1
    );
    ...
    assert.equal(
      (
        await updateImages([
          { id: changed.id, tags: ["matrix-empty"], theme: "matrix-empty", author: "matrix-empty" }
        ])
      ).updated,
      1
    );
    ...
    await pool.query("DELETE FROM theme WHERE slug=ANY($1::text[])", [
      ["matrix-city", "matrix-nature", "matrix-empty"]
    ]);
    await pool.query("DELETE FROM author WHERE slug=ANY($1::text[])", [
      ["matrix-alice", "matrix-bob", "matrix-empty"]
    ]);
  }
});

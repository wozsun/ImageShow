...
try {
  await runIntegrationScenario(async (runtime) => {
    ...
    const { ReadyImageCacheCoordinator } =
      await import("../../../../packages/server/src/images/ready-cache/coordinator-machine.ts");
    const { probeRedisOperationalState, getRedisOperationalState } =
      await import("../../../../packages/server/src/core/runtime-availability.ts");
    const { rebuildReadyImageCache } =
    ...
    try {
      ...
      assert.deepEqual(
        await readReadyImageCacheMeta(),
        originalMeta,
        "valid Redis core survives database outage"
      );
      ...
      process.stdout.write(
        JSON.stringify({
          recovery_ms: Math.round(performance.now() - restoredAt),
      ...
      assert.deepEqual(
        ...
        redisBefore,
        "recovery does not require a new Redis event"
      );
      assert.deepEqual(
        await readReadyImageCacheMeta(),
        originalMeta,
        "healthy cache is validated without a full rebuild"
      );
      assert.equal(
        (
          await pool.query(
            "SELECT count(*)::int AS count FROM background_job WHERE type='cache.rebuild'"
          )
        ).rows[0].count,
        0
      );
      assert.equal(
        (await pool.query("SELECT title FROM metadata WHERE id=$1", [imageId])).rows[0].title,
        "Preserved image"
      );
      assert.deepEqual(await coordinator.withRead(async () => "cached"), {
        acquired: true,
        value: "cached"
      });
    } finally {

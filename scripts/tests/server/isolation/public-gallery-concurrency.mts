...
await runIntegrationScenario(async (runtime) => {
  ...
  for (const endpoint of ["gallery-facets", "gallery-stats"]) {
    ...
    const responses = Promise.all(signals.map((signal) => (
      fetch(`http://127.0.0.1:${address.port}/api/${endpoint}`, { signal }).then(async (response) => ({
    ...
    try {
      ...
      process.stdout.write(JSON.stringify({ endpoint, requests: result.length, checkouts,
      ...
      assert.deepEqual((result[0]!.value as { tags: unknown }).tags, [
        { slug: "shared-tag", display_name: "Shared tag", ...(endpoint === "gallery-stats" ? { image_count: 0 } : {}) }
      ]);

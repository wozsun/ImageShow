...
await runIntegrationScenario(async (runtime) => {
  ...
  const { updateStorageBackend } = await import("../../../../packages/server/src/storage/backends/update.ts");
  const { reloadRuntimeConfigFromDisk } = await import("../../../../packages/server/src/config/runtime-config-store.ts");
  ...
  await updateRuntimeConfig({
    ...
    embed: { enabled: false, allowed_origins: ["https://portal.example.test", "https://*.trusted.example.test"] },
  ...
  const request = (path: string, headers: Record<string, string> = {}, method = "GET", host = "images.example.test") => (
    app.request(`http://internal.test${path}`, {
      method, headers: { Host: host, "X-Forwarded-Proto": "https", "X-Real-IP": "192.0.2.1", ...headers }
    })
  );
  ...
  const expectLimited = async (response: Response) => {
    ...
    assert.ok(Number(response.headers.get("Retry-After")) <= getRuntimeConfig().security.random_window_seconds);
  ...
  for (const referer of ["https://images.example.test/gallery", "https://sub.images.example.test/", "https://portal.example.test/article?q=1", "https://a.b.trusted.example.test/"]) {
  ...
  const concurrent = await Promise.all(Array.from({ length: 8 }, () => request(random(), { "X-Real-IP": "192.0.2.4" })));
  ...
  for (let index = 0; index < 3; index++) await expectStatus(await request(random(), { "X-Real-IP": "192.0.2.5" }), 200);
  ...
  await expectStatus(await request(random(), { Referer: "https://replacement.example.test/page" }), 200);
  await updateRuntimeConfig({ embed: { allowed_origins: ["https://portal.example.test", "https://*.trusted.example.test"] } });
  ...
  try {
    for (const prefix of ["full", "thumbs"]) {
      ...
      for (const [method, headers, expected] of [
        ...
        const denied = await request(path, { ...headers, Referer: "https://attacker.example.test/" }, method);
        ...
        assert.equal(openRead.mock.callCount(), beforeDenied, "rejected requests never open image objects");
      }
    }
    await expectStatus(await request(`/images/original/${id}`, { Referer: "https://portal.example.test/" }), 401);
    await updateStorageBackend("local", { public_base_url: "https://media.example.test/pictures" });
    const redirected = await request(`/images/full/${key}`, { Referer: "https://images.example.test/" });
    assert.equal(redirected.status, 302);
    assert.equal(redirected.headers.get("Vary"), "Referer");
    assert.equal(redirected.headers.get("Location"), `https://media.example.test/pictures/full/${key}`);
    await redirected.arrayBuffer();
    for (const prefix of ["full", "thumbs"]) {
      const path = `/pictures/${prefix}/${key}`;
      for (const referer of ["", "https://portal.example.test/path", "https://child.images.example.test/"]) {
      ...
      const beforeDenied = openRead.mock.callCount();
      await expectStatus(await request(path, { Referer: "https://attacker.example.test/", Range: "bytes=0-2" }, "GET", "media.example.test"), 403);
      await expectStatus(await request(path, { Referer: "https://attacker.example.test/" }, "HEAD", "media.example.test"), 403);
      assert.equal(openRead.mock.callCount(), beforeDenied);
      await expectStatus(await request(path, { Origin: "https://portal.example.test", "Access-Control-Request-Method": "GET" }, "OPTIONS", "media.example.test"), 204);
    }
  } finally {

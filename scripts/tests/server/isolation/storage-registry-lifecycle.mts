...
await runIntegrationScenario(async (runtime) => {
  ...
  const objectAccess = await import("../../../../packages/server/src/storage/objects/access.ts");
  {
    ...
    const request = (
      ...
      app.request(`http://internal.example.test${path}`, {
        method,
    ...
    try {
      ...
      const { registerSettingsRoutes } =
        await import("../../../../packages/server/src/routes/settings.ts");
      const { handleApiError } =
        await import("../../../../packages/server/src/core/http/responses.ts");
      ...
      try {
        ...
        const reload = await settingsApp.request(
          "http://main.example.test/api/admin/settings/reload",
          { method: "POST" }
        );
      ...
      assert.equal(
        (
          await request("images.example.test", "/static/test.js", "OPTIONS", {
            "Access-Control-Request-Method": "GET"
          })
        ).status,
        204
      ...
      const { resourceHostBoundary } =
        await import("../../../../packages/server/src/routes/resource-host.ts");
      const sharedResources = new Hono();
      sharedResources.use(
        "*",
        resourceHostBoundary(
          () => true,
          async (context, path) => context.text(path ?? "")
        )
      );
      for (const [localRoot, assetRoot] of [
        ["", "/full"],
        ["", "/thumbs"],
        ["/assets", ""]
      ]) {
        ...
        const asset = await sharedResources.request(`http://internal.test${assetRoot}/entry.js`, {
        ...
        assert.deepEqual(
          ...
          bytes,
          "actual image object keys keep local serving priority"
        );
      }
      await backendUpdate.updateStorageBackend("local", {
        public_base_url: "https://images.example.test/pictures"
      ...
      const restoreSql = interceptSqlQueries(database.pool, async () => {
        throw new Error("hot local read must not query SQL");
      });
      runtime.redisClient.redis.hget = (() => {
        throw new Error("local origin must not query image projection");
      }) as typeof originalHget;
      try {
        const hot = await request("images.example.test", undefined, "GET", {
          "If-None-Match": etag
        });
        assert.equal(hot.status, 304);
      } finally {
        restoreSql();
        runtime.redisClient.redis.hget = originalHget;
      }
      await backendUpdate.updateStorageBackend("local", {
      ...
      const encodedResponse = await request(
        "new-images.example.test",
        new URL(encodedUrl).pathname
      );
  ...
  try {
    registry.invalidateStorageBackendRegistry();
    const pendingRegistryAccess = registry.resolveStorageAccess(registryBackend);
  ...
  try {
    ...
    await database.pool.query(
      "UPDATE storage_backend SET config=config-'capabilities' WHERE slug='capability'"
    );
    registry.invalidateStorageBackendRegistry();
    ...
    assert.equal(
      (await registry.resolveStorageAccess("capability")).driver,
      first.driver,
    ...
    await assert.rejects(
      mutations.createStorageBackend({
        slug: "rejected",
    ...
    try {
      ...
      release.resolve();
      await Promise.allSettled([checking, outcome]);
      s3.state.beforeRequest = undefined;
    }

    const { registerStorageRoutes } =
      await import("../../../../packages/server/src/routes/storage.ts");
    const { handleApiError } =
      await import("../../../../packages/server/src/core/http/responses.ts");
    ...
    app.use("/api/admin/*", async (context, next) => {
      context.set("session", {
        id: "storage-route",
        username: "integration-admin",
        role: context.req.header("x-role") === "super" ? "super" : "image",
        csrf: "test"
      });
      await next();
    });
    registerStorageRoutes(app as unknown as Hono);
    const post = (path: string, body: unknown, role: "super" | "image") =>
      app.request(`/api/admin/storage/${path}`, {
        method: "POST",
    ...
    assert.equal(
      (await post("backends/local", { public_base_url: "https://images.example.test" }, "image"))
        .status,
      403
    );
    assert.equal(
      (await post("backends/local", { public_base_url: "http://images.example.test" }, "super"))
        .status,
      400
    );
    assert.equal(
      (await post("backends/local", { public_base_url: "https://images.example.test" }, "super"))
        .status,
      200
    );
    ...
    try {
      ...
      const beforeSorting = (
        await database.pool.query(
          "SELECT * FROM storage_backend WHERE slug <> 'capability' ORDER BY slug"
        )
      ).rows;
      const sortPath = "backends/capability/sort-order";
      assert.equal((await post(sortPath, { sort_order: 5 }, "image")).status, 403);
      assert.equal(
        (await post("backends/local/sort-order", { sort_order: 5 }, "super")).status,
        400
      );
      assert.equal(
        (await post("backends/missing-sort/sort-order", { sort_order: 5 }, "super")).status,
        404
      );
      ...
      for (const [sort_order, expected] of [
        ...
        assert.deepEqual(
          (await listStorageBackendOptions()).map((item) => item.slug),
          items.map((item) => item.slug)
        );
      }
      assert.deepEqual(
    ...
    assert.equal(
      (await post("backends/capability", { s3: { capabilities: { content_md5: true } } }, "super"))
        .status,
      400
    );
    ...
    for (const baseline of [
      ...
      assert.deepEqual(
        (
          await database.pool.query(
            "SELECT slug, sort_order FROM storage_backend WHERE slug<>$1 ORDER BY slug",
            [slug]
          )
        ).rows,
        before

...
await runIntegrationScenario(async (runtime) => {
  ...
  await assert.rejects(
    driver.openRead("full", "stream.bin", undefined, { signal: AbortSignal.abort(reason) }),
    (error) => error === reason
  );
  const controller = new AbortController();
  const opened = await driver.openRead("full", "stream.bin", undefined, {
  ...
  try {
    ...
    await assert.rejects(
      driver.readBuffer("full", "oversized.bin"),
      (error) => error instanceof Error && "code" in error && error.code === "object_too_large"
    );
  } finally {
  ...
  for (const root of [join(storageRoot, "full"), storageRoot]) {
    ...
    if (root.endsWith("full"))
      assert.deepEqual(await collectStorageKeyListing(driver.listKeys("full")), {
        complete: true,
        count: 0,
        keys: []
      });
    ...
    try {
      await assert.rejects(
        root === storageRoot
          ...
          : collectStorageKeyListing(driver.listKeys("full")),

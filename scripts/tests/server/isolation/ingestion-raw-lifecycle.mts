...
await runIntegrationScenario(async (runtime) => {
  ...
  const { prepareIngestionSessionSnapshot } =
    await import("../../../../packages/server/src/images/ingestion/workers/prepare-session.ts");
  const { withNormalizationAdmission } =
    await import("../../../../packages/server/src/images/normalization-admission.ts");
  const { cancelIngestionSessions } =
    await import("../../../../packages/server/src/images/ingestion/cancel/coordinator.ts");
  ...
  try {
    ...
    const service = new IngestionSessionService(
    ...
    const intents = (
      await service.createUploadIntents(
        ...
        Array.from({ length: 3 }, (_, index) => ({
          ...fixture.serviceDraft,
          idempotency_key: "raw-lock-" + index,
          batch_key: batchKey,
    ...
    const startUpload = (index: number) => {
      const pending = receiveUploadIntentBody(
        service,
        "raw-lock-owner",
        intents[index]!.credential,
    ...
    try {
      ...
      for (const [index, intent] of intents.entries()) {
        if (index + 1 < intents.length) {
          ...
          assert.deepEqual(
            claimOrder,
            intents.slice(0, index + 1).map((item) => item.session_id),
        ...
        await assert.rejects(
          settleWithin(pendingUploads[index]!),
          (error) =>
            error === reason ||
        ...
        assert.equal(
          requiredValue(await repository.readUploadIntent("raw-lock-owner", intent.session_id))
            .execution_token,
          ""
        );
        assert.deepEqual(
        ...
        if (index + 1 < intents.length) {
          await settleWithin(
            Promise.race([
              ...
              pendingUploads[index + 1]!.then(() =>
                assert.fail("queued raw request ended before claim")
              )
            ])
          );
        }
      ...
      assert.deepEqual(
        claimOrder,
        intents.map((item) => item.session_id)
      );
    } finally {
    ...
    const sharedLockWaiters = async () =>
      Number(
        (
          await runtime.databasePools.pool.query<{ count: number }>(
    ...
    assert.equal(
      await sharedLockWaiters(),
      0,
      "cancelled raw requests must drain their advisory lock waits"
    );
    const { withStorageLocationReadLock } =
    ...
    const received = activeSession(
      await settleWithin(
        receiveUploadIntentBody(
          service,
          "raw-lock-owner",
          intent.credential,
          new Response(body).body
    ...
    const createRawSession = async (label: string, status: "preparing" | "failed") => {
      ...
      const template = {
        ...
        owner: "raw-session-owner",
        session_id: sessionId,
        image_id: randomUuidV7At(new Date(now)),
    ...
    const normalizationBlocker = withNormalizationAdmission(
    ...
    try {
      ...
      const observedRepository = repositoryWithOverrides(repository, {
        updateProgress: async (...args) => {
          progress.push(args[2].phase);
          return repository.updateProgress(...args);
        }
      });
      pendingPrepare = prepareIngestionSessionSnapshot(
        observedRepository,
        preparing.session,
        prepareStop.signal,
        {
      ...
      blockerGate.resolve();
      await normalizationBlocker;
      await settleWithin(
        Promise.race([
          transcodeEntered.promise,
          pendingPrepare.then(() => assert.fail("transcode must start"))
        ])
      );
      ...
      await removeIngestionRaw(preparing.session, preparing.session.raw_generation);
      assert.deepEqual(
        await readFile(preparing.path),
        body,
        "in-flight transcode retains the raw lease after cancellation"
      );
    ...
    const cancellation = await cancelIngestionSessions(
      repository,
      new IngestionIrreversibleCoordinator(),
      retired.session.owner,
      [{ ...retired.session, expected_version: retired.session.version }],
      () => undefined,
      {},
      {
    ...
    await runtime.runtimeConfigStore.replaceRuntimeConfig(previous);
  }
  ...
  const cursorPaths = Array.from({ length: 9 }, (_, index) =>
    raw.ingestionRawPath(cursorPair, randomUuidV7At(new Date(cursorNow + index)))
  );
  ...
  const restoreBudget = installProperties(appConfig.ingestionRuntime, {
    orphanCleanupMaxTempEntriesPerCycle: 4
  });
  ...
  try {
    ...
    for (let cycle = 0; cycle < 30 && removed === 0; cycle += 1) {
      ...
      const report = await scanner.cleanupIngestionTempOrphans({
        keep,
        fileCutoff: cursorNow,
        partCutoff: cursorNow,
        signal: slice.signal
    ...
    assert.ok(
      slicedCycles > 1,
      "repeated time slices must preserve forward progress past retained files"
    );
    ...
    assert.equal(
      removed,
      1,
      "the bounded cursor eventually reaches the orphan after retained files"
    );
    await assert.rejects(stat(tail), { code: "ENOENT" });

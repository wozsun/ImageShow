...
await runIntegrationScenario(async ({ databasePools: { pool } }) => {
  ...
  const { retryExhaustedMoveCleanupJobs } =
    await import("../../../../packages/server/src/storage/cleanup/repository.ts");
  const expiredAge =
    Math.max(
  ...
  for (const extraPayload of [{}, { retain_exhausted: true }]) {
    ...
    const retained = histories.find(
      (item) => item.type === "move.cleanup" && item.status === "failed"
    )!;
    assert.deepEqual(
      (await jobs.cleanupBackgroundJobHistory()).sort((a, b) => a.status.localeCompare(b.status)),
      [
    ...
    assert.deepEqual(
      (await pool.query("SELECT id,payload FROM background_job")).rows,
    ...
    try {
      ...
      assert.equal(
        ...
        60,
        "延后期限与领取使用同一数据库时钟，不受应用时钟偏移影响"
      );
      assert.equal(
        await jobs.claimBackgroundJob("move.cleanup"),
        undefined,
        "延后期限到达前不可领取"
      );
      ...
      assert.equal(
        ...
        appConfig.backgroundJob.retryBackoffSeconds[0],
        "失败退避同样由数据库时钟计算"
      );
  ...
  try {
    for (const type of ["cache.rebuild", "move.cleanup"] as const)
      for (const previousStatus of ["succeeded", "failed"]) {
        ...
        cleanup = undefined;
        const renewed = (
          await pool.query("SELECT status, target_id, payload FROM background_job WHERE id=$1", [
            renewedId
          ])
        ).rows[0];
        assert.deepEqual(
          ...
          {
            status: "pending",
            target_id: "renewed-target",
            payload: { marker: "renewed-payload" }
          },
        ...
        assert.deepEqual(
          await jobs.cleanupBackgroundJobHistory(),
          [],
          "新 pending 任务不属于历史清理对象"
        );

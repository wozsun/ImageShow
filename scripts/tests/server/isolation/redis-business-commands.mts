...
type ArrayCommandName =
  | "imageshowReserveWindows"
  | "imageshowSampleReadyImageCoreIndex"
  | "imageshowSampleReadyImageDerivedIndex";
type BusinessCommands = Omit<ReadyImageRedisClient, ArrayCommandName> &
  Record<ArrayCommandName, (...args: Array<string | number>) => Promise<unknown[]>>;
type BusinessRedis = Redis & BusinessCommands;
type BusinessPipeline = ReturnType<Redis["pipeline"]> &
  Record<
    keyof BusinessCommands,
    (...args: Array<string | number>) => ReturnType<Redis["pipeline"]>
  >;
...
await runIntegrationScenario(async () => {
  ...
  const redisWindowLimit =
    await import("../../../../packages/server/src/core/redis/window-limit.ts");
  const readyCacheRedisClient =
    await import("../../../../packages/server/src/images/ready-cache/redis/client.ts");
  ...
  const concurrentReservations = await Promise.all(
    Array.from({ length: 8 }, (_, index) =>
      businessRedis.imageshowReserveWindows(
        "1",
  ...
  assert.ok(
    concurrentReservations.every((result) => JSON.stringify(result.slice(0, 3)) === "[1,1,1]")
  );
  ...
  assert.equal(
    await businessRedis.imageshowTouchReadyImageIndexedResult(
      indexedKey,
      indexedMetaKey,
      ...indexedRegistry,
      indexedKey,
  ...
  assert.deepEqual(
    await businessRedis.imageshowStoreReadyImageFilterSet(
      "3",
      filterSourceA,
      filterSourceB,
      filterDestination,
  ...
  const missingCoreSample = await businessRedis.imageshowSampleReadyImageCoreIndex(
    ...coreSampleArguments
  );

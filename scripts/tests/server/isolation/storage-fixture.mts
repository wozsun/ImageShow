...
export function controlledStorageDriver(overrides: Partial<StorageDriver>): StorageDriver {
  const unexpected = (): never => {
    throw new Error("unexpected storage fixture operation");
  };
  return {
    exists: unexpected,
    openRead: unexpected,
    readBuffer: unexpected,
    writeBuffer: unexpected,
    writeStream: unexpected,
    removeObjects: unexpected,
    serverCopySource: unexpected,
    supportsServerCopySource: unexpected,
    copyFromServerSource: unexpected,
    listKeys: unexpected,
    selfTest: unexpected,
    pruneEmptyDirs: unexpected,
    ...overrides
  };
}

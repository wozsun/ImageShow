...
await runIntegrationScenario(async (runtime) => {
  ...
  await runtime.databasePools.pool.query(
    ...
    [JSON.stringify({ endpoint: "https://objects.example.com", region: "test-region", bucket: "synthetic-bucket",
      access_key_id: "synthetic-key", secret_access_key: "synthetic-secret", root_path: "gallery",
      public_base_url: "", force_path_style: true })]
  );
  ...
  mock.method(LocalBackend.prototype, "openRead", async function(this: InstanceType<typeof LocalBackend>, ...args: Parameters<typeof openLocal>) {
  ...
  mock.method(S3Client.prototype, "send", async (command: unknown) => {
    ...
    assert.equal(command.input.Range, undefined, "random response does not promise cross-request ranges");
  ...
  try {
    ...
    for (const method of ["GET", "HEAD"]) {
      ...
      for (const [condition, status] of [[etag, 206], ['W/' + etag, 200], ['"unrelated"', 200]] as const) {
        const ranged = await app.request(localPath, { method, headers: { Range: "bytes=2-6", "If-Range": condition } });
    ...
    for (const size of ["full", "thumb"]) {
      ...
      assert.ok(body.items.every(item => ("object_url" in item) === (size === "full") && ("thumb_url" in item) === (size === "thumb")));
    }
    ...
    assert.equal(new Set(seedIds).size, 1, "size preserves seeded choice across cold PostgreSQL and warm Redis");
    await runtime.databasePools.pool.query("UPDATE storage_backend SET config=config || $1::jsonb WHERE slug='synthetic-s3'",
    ...
    for (const size of ["full", "thumb"] as const) {
      ...
      assert.equal(response.headers.get("location"), `https://public.example.com/gallery/${size === "thumb" ? "thumbs" : "full"}/${storageObjectKey(ids[1]!, size === "thumb" ? "webp" : "jpg")}`);
    }
  } finally {
    mock.restoreAll();

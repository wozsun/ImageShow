...
await runIntegrationScenario(async (runtime) => {
  const { mergeS3Settings } =
    await import("../../../../packages/server/src/storage/backends/config.ts");
  ...
  const foregroundCleanupReceipt = (
    await database.pool.query(
  ...
  const adoptedCleanupJob = (
    await database.pool.query(
      "UPDATE background_job SET status='running', execution_token=$2 " + "WHERE id=$1 RETURNING *",
  ...
  const originalCleanupRemoveObjects = localAccess.driver.removeObjects.bind(localAccess.driver);
  ...
  const persistedGuardDeadline = (
    await database.pool.query(
      "SELECT payload->>'confirm_absent_after' AS deadline " + "FROM background_job WHERE id=$1",
      [guardedLatePublishJob.id]
    )
  ).rows[0]?.deadline;
  assert.ok(
  ...
  const admittedCleanupReason = new Error("injected move cleanup cancellation after delete");
  const originalAdmittedCleanupRemove = localAccess.driver.removeObjects.bind(localAccess.driver);

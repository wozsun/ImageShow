...
await runIntegrationScenario(async (runtime) => {
  ...
  try {
    ...
    const { limitProtectedAdminRequestBody } =
      await import("../../../../packages/server/src/core/http/request-body-limit.ts");
    ...
    const backend = async (slug: string) =>
      (
        await runtime.databasePools.pool.query("SELECT config FROM storage_backend WHERE slug=$1", [
          slug
        ])
      ).rows[0] ?? null;
    ...
    rejectedPackage.storage_backends.push({
      ...rejectedPackage.storage_backends[0]!,
      slug: "probe-failure",
      s3: { ...rejectedPackage.storage_backends[0]!.s3, root_path: "/fail" }
    });
    s3.state.putError = (request) =>
    ...
    for (const mode of ["success", "rolled_back", "committed", "unknown"] as const) {
      ...
      const names: string[] = [];
      const remove = store.onRuntimeConfigChange(() =>
        names.push(store.getRuntimeConfig().site.header_name)
      );
      try {
        ...
        if (mode === "rolled_back") {
          ...
          assert.equal(
            (await backend(slug))?.config.secret_access_key ?? null,
          ...
          (error: unknown) =>
            phase === "restore"
              ? error instanceof ApiError &&
                ...
                typeof error.details === "object" &&
                error.details !== null &&
                "restore_error" in error.details &&
    ...
    try {
      ...
      let settled = false;
      reload = store.reloadRuntimeConfigFromDisk().then(() => {
        settled = true;
      });
      await setImmediate();
    ...
    const removeFailing = store.onRuntimeConfigChange(() => {
    ...
    const request = (path: string, body?: unknown, role = "super", csrf = "config-csrf") =>
      app.request(`http://imageshow.test/api/admin/advanced-config/${path}`, {
    ...
    try {
      ...
      assert.deepEqual(store.getRuntimeConfig(), baseline);
      assert.equal(
        (await backend("route-existing"))?.config.secret_access_key,
        s3.settings.secret_access_key
      );
      assert.deepEqual(
        (await backend("route-existing"))?.config.capabilities,
        { content_md5: true },
        "导入须按本次探測结果保存能力"
      );
    } finally {
      await reset("route-existing");
    }
    const imported = await request("import", {
      package: { config: { site: { header_name: "Imported Through Route" } } },
      slug_mappings: {}
    });

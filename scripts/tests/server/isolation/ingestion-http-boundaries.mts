...
await runIntegrationScenario(async (runtime) => {
  ...
  const { openIngestionActionScope } = await import("../../../../packages/server/src/images/ingestion/queue/action-scope.ts");
  const { createIngestionSessionId } = await import("../../../../packages/server/src/images/ingestion/sessions/identity.ts");
  ...
  try {
    ...
    scope = openIngestionActionScope({ id: cookie.slice(cookie.indexOf("=") + 1), username: "integration-admin" }, "import", () => {});
    const headers = {
      "content-type": "application/json", origin, cookie,
      "x-csrf-token": csrf_token, [ingestionActionScopeHeader]: scope.id
    };
    ...
    for (let index = 0; index <= 3600; index++) {
      ...
      const template = {
        ...fixture.importTemplate, owner: "integration-admin", session_id: sessionId,
        image_id: randomUuidV7(), image_time: new Date().toISOString()
      };
      const now = Date.now();
      const session = activeSession((await fixture.productionIngestionRepository.acceptImportSession({
        ...template, semantic_hash: ingestionSessionSemanticHash(template)
      }, fixture.displayOrderKey(sessionId, index, now), now)).session);
    ...
    for (const chunked of [false, true]) {
      const snapshot = await fetch(origin + ingestionSnapshotPath + "?queue=import&limit=1", {
        method: "POST", headers, body: JSON.stringify({ exclude_items: [], include_items: [] })
      });
      ...
      const body = Buffer.from(JSON.stringify({
        queue: "import", action: "apply_metadata", action_request_id: randomUuidV7(),
        action_watermark, items: pairs.slice(0, 3600), metadata: { title }
      }));
      ...
      const result = await fetch(origin + ingestionActionPath, { method: "POST", headers, ...init });
      ...
      const stored = await fixture.productionIngestionRepository.readSessions("integration-admin", pairs);
      ...
      assert.equal(activeSession(outside).metadata.title, fixture.importTemplate.metadata.title,
        "精确集合之外的任务不受批量属性修改影响");
    }
    ...
    const probe = async (path: string, expected: number, authenticated: boolean, csrf: string, declared: boolean) => {
      let reads = 0;
      ...
      const request = new Request(origin + path, {
        method: "POST", headers: {
          ...
          cookie: authenticated ? cookie : "", "x-csrf-token": csrf,
      ...
      try {
        ...
        if (expected === 413) assert.equal((await response.json() as { code: string }).code, "request_body_too_large");
    ...
    console.log(JSON.stringify({ scenario: "ingestion-http-boundaries", preciseItems: 3600,
      actionSizes, transports: ["content-length", "chunked"], authBeforeRead: true, bounded: true }));
  } finally {
    scope?.close();

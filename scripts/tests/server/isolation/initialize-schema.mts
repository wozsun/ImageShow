...
databasePools.pool.on("connect", (client: { query: (...args: unknown[]) => Promise<unknown> }) => {
...
try {
  ...
  if (maximumConcurrentQueries !== 1) {
    throw new Error(
      "startup must execute one SQL query at a time per client: " + maximumConcurrentQueries
    );
  }
} finally {

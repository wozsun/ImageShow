...
import { formatLogContext, safeLogText, safeLogValue } from "../../../packages/shared/src/browser.ts";
...
test("[Server/日志] 脱敏凭据与命名空间并保留恢复定位和异常链", () => {
  ...
  const error = Object.assign(new Error("open failed at C:\\private folder\\source.jpg"), {
    code: "ENOENT", path: "C:\\private folder\\source.jpg",
  ...
  const context = {
    image_id: "00000000-0000-7000-8000-000000000001", transaction_id: "42",
    source_backend: "local", target_backend: "s3",
    candidates: [{ backend: "s3", prefix: "full", key: "01/example.webp", namespace_identity: secret }],
    session_id: secret, password: secret, secret_access_key: secret, authorization: `Bearer ${secret}`,
  ...
  assert.deepEqual(JSON.parse(formatLogContext(result)).original_error.stack, result.original_error.stack);
  assert.equal(context.session_id, secret);
  assert.equal(formatLogContext(new Error(`secret_access_key="${secret}"`)).includes(secret), false);
  for (const protocol of ["http", "https", "redis", "rediss", "postgres", "postgresql", "s3"]) {
    assert.equal(safeLogText(`request ${protocol}://user:${secret}@store.invalid/private?token=${secret} failed`),
...
test("[Server/日志] 启动配置解析与校验失败经过清洗并有界退出", async () => {
  ...
  const invalidConfig = { ...appConfig.runtimeDefaults, log: { ...appConfig.runtimeDefaults.log, level: "leak-marker" } };
  for (const content of ["leak-marker", JSON.stringify(invalidConfig)]) {
    writeFileSync(join(directory, "config.json"), content);
    const result = await runProcess(process.execPath, [
      resolve("node_modules/tsx/dist/cli.mjs"), "--tsconfig", resolve("packages/server/tsconfig.check.json"),
      resolve("packages/server/src/index.ts")
    ], {
      env: { ...process.env, NODE_ENV: "development", IMAGESHOW_DEVELOPMENT_DATA_DIRECTORY: directory },
...
test("[Server/日志] 循环、控制字符、访问器与大对象保持有界有效 JSON", () => {
  let accessorReads = 0;
  const object: Record<string, unknown> = { ok: 1, text: "before\n[ERROR] forged\r\u001b[31m\u2028after" };
  object.self = object;
  Object.defineProperty(object, "danger", { enumerable: true, get() { accessorReads++; throw new Error("synthetic"); } });
  object.toJSON = () => { throw new Error("must not serialize source"); };
  ...
  Object.defineProperty(error, "stack", { get() { throw new Error("synthetic"); } });
  assert.equal((safeLogValue(error) as Record<string, unknown>).message, "synthetic");
  assert.equal(formatLogContext({ name: "SyntaxError", message: "synthetic-private-value" }).includes("synthetic-private-value"), false);
});

test("[Server/日志] HTTP 日志生成服务端关联号、记录路由模板并统一文件与控制台输出", async (t) => {
  const lines: string[] = [];
  const capture = (chunk: unknown) => { lines.push(String(chunk)); return true; };
  ...
  t.after(() => { restoreOut(); restoreErr(); configureRuntimeLogger(() => appConfig.runtimeDefaults.log); });
  const app = new Hono();
  app.onError((error, c) => handleApiError(c, error));
  app.use("*", auditAdminMutation);
  app.post("/synthetic/:id", () => { throw new Error("password=synthetic-private-value\nforged"); });
  registerAdminLogRoutes(app);
  const response = await app.request("http://example.invalid/synthetic/private-path?token=synthetic-private-value", {
    method: "POST", headers: { "x-request-id": "external-id" }
  });
  ...
  assert.deepEqual(contexts.map((context) => context.request_id), [requestId, requestId]);
  assert.deepEqual(contexts.map((context) => context.route), ["/synthetic/:id", "/synthetic/:id"]);
  assert.equal(lines.join("").includes("synthetic-private-value"), false);
  assert.equal(lines.join("").includes("private-path"), false);
  assert.equal(lines.every((line) => line.split("\n").length === 2), true);
  assert.ok(readFileSync(join(runtimePaths.logDirectory, "app.log"), "utf8").endsWith(lines.at(-1)!));
  lines.length = 0;
  const report = await app.request("http://example.invalid/api/admin/logs/client-errors", {
    method: "POST", headers: { "content-type": "application/json" },
    body: JSON.stringify({ context: "synthetic.operation", error: { name: "SyntaxError", message: "synthetic-private-value" }, metadata: { token: "synthetic-private-value", image_id: "synthetic-image" } })
  });

...
test("[Server/发布] 完整版本检查拒绝任一 manifest、lockfile、分支或标签不一致", async () => {
  ...
  const run = (args: string[], refType = "branch", refName = "dev") =>
    execFileSync(process.execPath, [join(directory, script), ...args], {
      encoding: "utf8",
      stdio: "pipe",
  ...
  assert.match(
    run(["--tag", `v${version}`], "tag", `v${version}`),
    /manifests and lockfile agree/u
  );
  ...
  assert.throws(
    () => run(["--branch", "dev"], "branch", "main"),
    /branch main does not match dev/u
  );

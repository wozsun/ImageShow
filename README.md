...
反向代理的请求体上限不能低于应用内任一对应设置；当前最大的固定边界是 JSONL
解析的 128 MiB，示例使用 `client_max_body_size 128m`。若把单文件上传上限提高到
128 MiB 以上，还需同步提高代理值。否则请求会在到达应用鉴权和校验逻辑前被代理
返回 413。不要把应用 HTTP 端口直接暴露公网；`X-Forwarded-Proto` 缺失会导致

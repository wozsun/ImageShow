...
- local、S3 兼容对象存储和 WebDAV 多后端并存；支持单图、批量和整后端迁移。
...
当前生产拓扑只支持一个 ImageShow 应用实例。应用应只监听回环或私有网络，由可信反向
代理终止 HTTPS，并覆盖客户端传入的 `Host` 与 `X-Forwarded-*` 头。示例按 Nginx 1.30
stable 线编写；2026-08-10 对照

...
- 公开首页、双模式 WebGL 展映、瀑布流画廊、图片详情与可选无导航嵌入页。展映与画廊
  ...
  公开与后台详情中的当前展示图保留
  浏览器原生右键与长按保存能力；详情直链未就绪时标题保留纯文本，不提供空地址链接。
  画廊卡片换源后重新加载，并清除旧地址的失败状态。未登录访客的公开“原图”按钮由
...
- 主站 `/random` 随机图 API，以及集中承载本地媒体、缩略图和外链原图直连决策 / 代理的
  默认主站 `/static` 资源路径（可配置独立子域）；已解析随机池在一次只读 Redis 原子调用内完成校验、抽样和 rich item
  读取，近期去重与最终排序仍由应用负责；不提供随机、外链或主题专用子域。
- 后台设置保存期间锁定编辑，15 秒请求超时后恢复操作；未保存修改不会被后台刷新覆盖。
  主题、标签和作者卡片按字段保留编辑内容，新词条及时更新共享候选。导入菜单与来源标签支持
  完整键盘操作；原图代理和转码响应取消，已启动的编码收口后停止后续轮次。

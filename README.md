...
- 公开首页完整展示设备、明暗、主题、标签与作者目录及图片数量，十万级图库总量仍能
  ...
  流窗口、固定 32 字符的无状态游标、可恢复 DTO 数据窗口和共享图片解码队列支持
  长列表连续浏览；远页只保留 cursor、ID 与紧凑布局，返回时按权威 cursor 重取。
...
- 后台图片上传 / 链接导入、JSONL 清单导入、公开微博导入、编辑、删除、回收站、最终
  ...
  并可按住 Shift 点击当前页的一头一尾连续选择两端及中间图片，点击列表外空白区域
  可清空选择；首次进入图片列表时，已保存的暗色外观会直接绘制筛选控件，不经过
  白色控件表面；
  上传与下载队列使用单项前瞻的有界流水线，在不增加图片处理并发的前提下重叠素材
  传输与处理；任务缩略图在首次就绪前显示静态图片下载图标，并在暂存 / 正式地址切换时
  等待新图片完成解码后再替换；卡片字段、默认属性、移动折叠与预览样式随上传 / 编辑能力
  自身加载，不依赖预先访问图片编辑器。取消竞态标记按会话代际和所有者在执行收口后立即释放。
- 图片管理员可从公开画廊或后台的图片详情直接编辑可用图片，也可处理常规上传、移入
  回收站和恢复图片，查看、新建、编辑及用精确移动按钮或桌面拖动排序主题、标签和
  作者；单图把一张图片直接交给同一个批量属性编辑器，保存后继续留在窗口并以
  PostgreSQL 权威快照推进基线，复原只撤销当前活动图片尚未保存的属性草稿。单图仅
  隐藏分页、批量默认属性和卡片移出按钮，卡片字段结构与上传 / 导入及批量编辑一致；
  后台图片、词表、上传队列和批量编辑共用可直接输入页码跳转的分页器，并可
  执行数据库、存储、Redis、回收站及全部五项只读检查；检查页默认以一次
...
- 后台导航在鼠标悬浮、键盘聚焦或指针按下内部页面入口时预加载对应路由的 JS、CSS
  与静态依赖；图片详情、编辑和上传等按需能力复用同一套鼠标、键盘与触摸意图覆盖。
  页面仍只在实际导航后挂载和读取数据，同一 SPA 会话复用预加载结果。

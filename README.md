...
- **三档预生成**：超级管理员可从检查页设置参数，后台生成本地 `l/m/s` 图片，支持启停、并发调整、重启恢复、完整核验与对照抽查，详见[预生成指南](docs/guide/normalize-preparation.md)。
- **站点管理**：管理员权限、站点配置、页脚备案信息、配置包、日志、检查与页面嵌入，可设置独立静态资源地址；嵌入页始终按访客展示，首页页脚仅保留自定义内容，并支持[宿主自定义光标协作](docs/guide/embed-cursor.md)与[安全区同步](docs/guide/embed-safe-area.md)。

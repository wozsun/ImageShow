...
- 公开首页、瀑布流画廊、图片详情与可选无导航嵌入页；画廊卡片使用标题或 UUID 短标识，
  ...
  画廊生成的随机图片链接按真实可用宽度截断，并在复制按钮前以贴合文字基线的 ASCII `...`
  标记；复制动作始终使用完整 URL。
  公开与后台详情中的当前展示图保留
...
- local 与多个 S3 兼容对象存储后端并存；支持单图、批量和整后端迁移，以及检查页显式
  ...
  并逐对象确认结果。
- 新入库完整展示图固定使用 `full/<UUID 尾部两位>/<UUID>.<ext>`，缩略图使用
  `thumbs/<UUID 尾部两位>/<UUID>.webp`；设备、亮度、主题、作者和标签只保存在 metadata，
  编辑分类不再搬动对象。5.6.0 的检查页为超级管理员提供一次性旧 `media` 布局升级，按 10 张
  自动分批显示权威进度，断线后可重新读取剩余量继续；确认生产迁移完成后，5.6.1 将删除整条
  升级分支和旧入口。
...
启动与 schema 路径不保留按旧版本、旧字段或旧任务 payload 分流的自动升级代码。5.6.0 的图片
存储布局升级仅由超级管理员在检查页显式执行，并将在生产迁移完成后的 5.6.1 整体删除。非空
数据库必须已经具备当前 readiness 所需的最小结构；缺失或类型不兼容时启动会明确拒绝，应用未

...
v3.10.1 发布前已确认所有生产实例执行过旧 `0002_trash_purge_state`，因此该结构被
压入 `0001_initial.sql` 作为新的全新安装基线，旧实例在 `schema_migrations` 中保留
历史 `0002` 记录即可。`0003_trash_purge_job.sql` 是继续向前执行的兼容迁移，只为
既有数据库扩展 `background_job.type` 的 `trash.purge` 枚举，不重建或清空业务数据。
...
| `final_object_key` | 进入 `committing` 时确定的正式存储键；普通图片对应 `media` 原图，代理图片对应 `link` 缩略图，提交前为空 |
...
| `error` | 失败原因 |
| `expires_at` | 30 分钟空闲过期时间；接收、prepare 和 commit 期间持续续租。普通过期会话由 `import.cleanup` 原子认领；过期的 `committing` 仅在确认其提交 advisory lock 空闲后取消并清理 |
...
Worker 会按保留策略裁剪历史记录：`succeeded` / `ignored` 保留 7 天，耗尽重试且
`next_retry_at IS NULL` 的 `failed` 保留 90 天。
...
清理全部导入会话和 `_uploads` 暂存对象才能删除。后端注册表同时
管理配置快照与按签名复用的 driver/client 生命周期，配置或默认后端变化会关闭并清理
相关实例。S3 的 endpoint / bucket / root_path 与 WebDAV 的 base_url / root_path 是
物理位置；仍有图片、任意导入会话或暂存对象时不允许原地修改。region、凭据、公开 URL
等访问参数不改变物理 identity，可在共享位置锁下验证并轮换。
...
仅在数据库没有 super 时，首次启动才使用 `ADMIN_USERNAME` / `ADMIN_PASSWORD` 创建首个 super；已有 super 的账号和密码始终以 PostgreSQL 为准。

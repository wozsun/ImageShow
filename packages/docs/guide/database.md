...
| 字段 | 含义 |
| --- | --- |
| `id` (UUID, PK) | 服务端按 `image_time` 生成的 UUIDv7；最终图片 id 与对象键复用该值。JSONL 同时间记录会把单批次位置编码到 `rand_a`，保持输入顺序 |
...
| `error` | 失败原因 |
| `expires_at` | 空闲过期时间；接收、prepare 和 commit 期间持续续租，过期后由 `import.cleanup` 原子认领并清理 |

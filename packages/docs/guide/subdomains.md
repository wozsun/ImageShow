...
- `random.img.example.com`：根路径 `/` 提供随机图，可携带 `d`/`b`/`t`/`tag`/`a`/`m` 查询参数，适合直接作为随机图链接分发。

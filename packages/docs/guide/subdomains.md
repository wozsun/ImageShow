...
- `link.img.example.com`：外链（link）图片专用域名。`/thumbs/<设备-明暗>/<主题>/<id>.webp` 提供我们为该外链生成并存储的略缩图；`/media/<id>.<ext>` 由**服务端代理**抓取其外部原图（以图片自身域名作 Referer 绕过防盗链）后转发。外链图的略缩图与原图公开链接都指向该域名，原图永不直链外部主机。

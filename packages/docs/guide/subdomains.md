...
- `link.img.example.com`：外链资源专用域名。`/thumbs/<设备-明暗>/<主题>/<id>.webp` 提供代理链接模式生成并存储的略缩图；`/media/<id>.<ext>` 代理 link 图的外部原图；`/original/<id>` 在详情页原图链接无法无 Referer 直连时提供回退代理。代理请求以图片自身域名作 Referer 绕过防盗链，**缓存遵循源站的 `Cache-Control` / `Expires`，源站未声明才默认 30 天**（略缩图仍走长效 `immutable` 缓存）。

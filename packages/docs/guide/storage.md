...
- **WebDAV 存储**：填写 HTTPS base URL、用户名、密码（HTTP Basic）、根目录与可选 HTTPS 公开访问域名（public base URL）。服务器用标准 WebDAV 动词（PROPFIND/MKCOL/PUT/GET/DELETE/COPY）读写，按需自动创建父目录；PROPFIND 响应用 XML parser 解析，兼容带命名空间和实体转义的 href。所有 WebDAV 请求都有统一 timeout，临时性 429/5xx、初始请求超时或网络失败会有限重试和退避。后端内复制优先使用原生 COPY；新目标只有明确 2xx 且精确目标可读时才判定成功，目标原本存在时还会比较源与目标内容，避免服务端忽略 `Destination` 文件名却返回成功。非成功响应或内容未覆盖都会退化为 GET+PUT。对于“已产生源文件名副本但仍返回失败”的服务端，只清理本次 COPY 新产生的副本，避免留下孤儿对象。还有一个 **Depth: infinity 列举** 开关（**默认关闭**）：关闭时按可移植的递归 `Depth: 1` PROPFIND 列举目录（同级子目录有并发上限）；开启后改用单次 `Depth: infinity` PROPFIND 一把拉回整棵子树，存储检查/清理会快很多——但部分 WebDAV 服务器不支持 `Depth: infinity`，故默认关闭，确认服务器支持再开启。两种列举方式都有最大结果数保护，避免异常大目录拖垮进程。

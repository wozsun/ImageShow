...
数据库 CAS；只有 CAS 成功才删除旧对象。import commit 也取得同一单图锁，因此并发
...
import commit 会先读取 `_uploads` 候选，核对 processed image 的 prepared MD5；正式
同名对象已存在时必须与候选逐字节一致，新写入对象也必须回读通过后才写数据库。不一致
统一返回 `storage_object_conflict`。异常补偿只删除本次实际创建的候选，不碰提交前已经
存在的对象。事务完成后重新读取 PostgreSQL 当前位置，推进图片 cache revision，并仅在

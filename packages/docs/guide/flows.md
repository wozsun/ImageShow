...
选择；常驻的第二导航栏汇总当前选择，并以 `/gallery` 查询参数进入画廊。目录
section 保持透明，只由各筛选卡片提供磨砂表面。主题、标签和作者候选区各自使用
浏览器原生滚动；不在 `touchmove` 中接管手势或按触点增量移动页面，避免取消
Safari 原生惯性并反复触发导航显隐。支持 scroll chaining 的浏览器可在候选区边界
自然交给页面；Safari 若继续锁定当前触点，则需离开卡片重新起手。

...
向下拖动并露出页面背景。导航原有的向下隐藏、向上显示以及两级联动仍只通过
transform 改变可见状态，累计方向距离达到 12px 后才切换，避免微小抖动反复触发。
...
`touchstart`、指针、滚轮或焦点操作才按正常规则关闭。12px 的累计滚动方向和距离
阈值只继续控制工具栏显隐，不参与筛选浮层的关闭判断；软键盘收起时回落到文档

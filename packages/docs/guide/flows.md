...
打开其来源任务的处理结果预览，三个入口不会互相替换。来源移除或重试导致最终 MD5
清空 / 改变时，队列按旧 MD5 重建剩余关联，最后一项恢复普通待提交。来源任务只有在
commit 响应返回正式缩略图和对象 URL 后才拥有可离队保存的持久预览；SSE 提前确认
finalized 以及取消或失败期间会禁用未持久化的来源预览，不把可能失效的 import
preview 当作正式图片地址。

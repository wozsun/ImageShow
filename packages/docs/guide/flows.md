...
立即收起。打开筛选的显式操作会取得交互所有权；打开前已有的惯性滚动尾流在静止前
不会反向关闭刚展开的浮层，但尾流期间一旦出现新的菜单外 `touchstart` / 指针操作，
仍会立即收起。滚动方向和距离阈值只继续控制工具栏显隐，不参与筛选浮层的关闭判断；
筛选更新触发的程序性回顶、面板内部滚动、软键盘收起时回落到文档根节点的焦点及其
视口重排都属于当前内部交互，不能自行收起面板或 Facet 菜单；键盘收起后的下一次
菜单外物理操作再按正常规则关闭。Select / Facet Portal 与父浮层同步关闭，跨越桌面 /
...
- 后台图片列表通过 `/api/admin/images?status=&d=&b=&t=&tag=&a=&cursor=&limit=`
  ...
  滚轮滚动也会立即收起。筛选按钮取得打开所有权后，打开前已有的惯性滚动尾流会被
  消费到静止，不会立即关闭浮层；此后新的外部滚动仍会收起。筛选内部操作及其单次
  程序性列表回顶保持展开。后台不复用公共画廊按页面滚动方向和距离自动隐藏工具栏的
  逻辑。
...
批量编辑只提交实际发生变化的图片和字段；服务端拒绝重复图片 ID、没有 metadata/tags 的空更新项，以及去重后仍超过上限的 tags。不同图片以固定低并发 2 执行，同一图片仍严格保持 metadata 后 tags 的顺序；分类与对象路径变化继续服从存储 mutation lock，不使用覆盖整批的长事务。单项失败不会阻止后续图片，响应包含 `updated`、`failed` 和与请求同序的 `results`；请求数量可由请求项或两项统计直接得出，不重复返回。前端保存后移除成功项、保留失败项草稿并显示 ID 与公开错误信息，再次保存只提交失败或仍有变化的图片。存在改动时批量主按钮显示无空闲图标的“保存N项”，并按三位数字预留宽度；各异步状态在同一内容网格内居中，状态切换不改变按钮外部宽度。

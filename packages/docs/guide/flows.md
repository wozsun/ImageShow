...
并在窗口或 `visualViewport` 高度变化时按帧重建共享观察器，使地址栏伸缩、横竖屏和
不同屏幕尺寸仍得到真实的一屏缓冲。缩略图离开缓冲区后会卸载以释放解码位图和合成
资源，返回时依赖浏览器缓存重新挂载；卡片 DOM、瀑布流高度、焦点入口和详情数据不随
图片回收变化。这样长列表滚动时同时存活的解码图片数量保持有界，避免移动 Safari 因
累计图片内存而终止页面。

...
- 上传 / 导入的“默认属性”和批量编辑的“批量默认属性”在移动端默认折叠；展开后以
  ...
  不再挤压下方列表。图片列表筛选使用独立的浮层布局，不随此处改变。触控、点击、
  键盘焦点或滚轮操作发生在默认属性浮层及其弹出菜单之外时会自动收起。
- “清空已完成”直接移除成功提交的卡片，且在其余任务仍在提交时也可使用；
  ...
  本地 XHR 与服务器下载请求也会中止。移动端清理菜单在触控/笔的原始 `pointerup`
  或鼠标/键盘 `click` 中固定所选动作，并从最新队列快照执行清理或打开确认窗口；
  触控按下与抬起必须来自同一 pointer 且仍位于原按钮内，滑出和 `pointercancel` 不执行，
  兼容 click 只做去重。业务分发不依赖菜单退场、`inert`、动画结束或延迟焦点回调，
  避免 iOS Safari 抑制、丢失或重复本次选择。
...
- 后台图片列表通过 `/api/admin/images?status=&d=&b=&t=&tag=&a=&cursor=&limit=`
  ...
  列表的滚动视口统一延伸到管理区最右侧，卡片仍保留原有右侧留白；移动端分页壳层

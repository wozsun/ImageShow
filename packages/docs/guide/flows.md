...
公共画廊保留已分页取得的卡片与宽高占位，只在当前视口及其上下各一个视口高度的
预加载缓冲区内挂载缩略图元素。所有卡片共享一个 `IntersectionObserver`；由于规范中
百分比 `rootMargin` 按根宽度而非高度解析，前端会以当前可见视口高度换算像素边距，
并在窗口或 `visualViewport` 高度稳定后只重建一次共享观察器，使地址栏伸缩期间不会
反复遍历全部卡片，横竖屏和不同屏幕尺寸仍得到真实的一屏缓冲。缩略图离开缓冲区后会
卸载以释放解码位图和合成资源，返回时依赖浏览器缓存重新挂载；卡片 DOM、瀑布流高度、
焦点入口和详情数据不随图片回收变化。

画廊星空渐变由独立的固定大视口装饰层承载，页面本体只绘制纯色兜底。渐变、星点和
轨道统一使用带 `100vh` 回退的 `100lvh` 高度；Safari 地址栏收放时只改变可见裁切，
不会让图案跟随滚动缩放。装饰层的绘制面积保持在屏幕量级，不随数百张卡片把文档拉长；
它与一屏图片缓冲共同约束长列表的解码与合成资源，避免移动 Safari 因累计 WebContent
内存而终止页面。Chrome 与 Edge 使用相同的标准 `IntersectionObserver`、可选
`visualViewport` 和 CSS 固定层路径。

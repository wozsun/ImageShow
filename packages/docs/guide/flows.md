...
- 应用设置写 `config.json`，存储后端与密钥写 PostgreSQL。物理位置变更在独占位置锁内重新检查全部图片、全部未清理导入会话和 `_uploads` 对象，任一存在即拒绝；凭据、region、公开 URL 等非物理字段只需共享锁，并先验证既有对象读取和临时对象读写。
...
按钮宽度由最长文案一次确定；快速操作的进行态至少展示 500ms，需显示的结果保留五秒，
成功状态仍允许再次点击。
...
负责 `pending | info | success | error` 展示、五秒终态倒计时、悬停或键盘聚焦暂停和
...
Worker 用 `FOR UPDATE SKIP LOCKED` 领取持久任务，按任务类型限制并发。每次 tick 先运行到期的僵尸恢复、过期导入调度和历史裁剪，再并行给每个可运行任务类型一个最多 50 项 / 2 秒的时间片；慢队列不能无限占住 tick。调试日志记录各类型 backlog、最老等待时间、处理数、耗时和预算是否耗尽。存储孤儿清理与物理位置变更使用独占位置锁；导入会话创建、prepared 暂存写入、commit、取消及过期暂存清理持有共享位置锁，分类移动、主题重分配、存储迁移与彻底删除还会取得对应单图锁。物理位置变更因此能在锁内检查全部会话和暂存对象，不会把旧位置文件变成孤儿。人工存储清理仍只把未过期的活动会话视为检查保留引用；过期会话由 `import.cleanup` 先删除暂存与 raw，再删除会话行。

...
JSONL 先请求管理端解析接口，服务端按 `link_image.max_items` 限制数量、逐行严格校验并规范化 `image_time`。合法行才创建 `ImportJob`，错误行保留行号、截断后的原文预览和错误原因。任务创建时的字段优先级为“JSONL 行内字段 > 窗口默认属性 > 系统默认值”；行内 `tags: []` 是显式空标签，`device: auto` 也是显式分类选择，两者都不合并对应默认值。任务进入 ready 前，清单显式提供的设备、明暗、主题、作者和标签受保护；未提供的公共属性继续跟随管理员主动执行的“应用到全部”。ready 后所有来源采用同一编辑规则，清单属性不再受保护。

四种入口在创建任务前先排除确定相同的素材来源。本地选择以目录相对路径（可用时）
或文件名、大小和修改时间识别同一文件；URL、JSONL 和微博图片以规范化后的原图
HTTPS URL 识别同一下载来源，忽略不会参与请求的 fragment，并同时覆盖当前队列与
本次新输入。每个来源只保留首次出现项，因此不会为同一文件重复上传，也不会为同一
原图链接重复下载。

不同来源仍可能在标准化后得到相同最终文件。prepare 返回最终 MD5 后，四种入口统一
进入重复确认：图库重复与当前队列中已完成 prepare 的相同 MD5 都让任务保持 ready，
管理员可选择仍然提交副本或取消，不再自动跳过并取消暂存。任务卡片主缩略图始终显示
并打开本次任务自己的 prepared 预览；下方图库重复项打开既有图片详情，同队列重复项
直接读取仍在队列中的参照任务并打开其实时处理结果，三个入口不会互相替换。队列按
首次得到 MD5 的顺序为每组选择首项，后续相同 MD5 任务才需要批内确认；不在任务中
复制参照任务的属性、预览或离队快照。参照任务移除、重试或改变 MD5 后立即从当前
队列重新计算，最后一项恢复普通待提交。任务提交成功后，commit 响应返回完整管理端
`ImageItem`，相同 MD5 的剩余任务把它作为图库重复项；已提交任务离队后不再以
`ownerId: null` 或失效 import preview 冒充批内来源。队列只额外记录本窗口已经
完成 commit 的 MD5 作为竞态栅栏；晚到的同 MD5 prepare 结果会通过 ready 会话的
幂等 prepare 再读一次服务端图库真值，并合并等待期间直接到达的完整 `ImageItem`，
不会保存参照任务属性或预览快照。

批量提交和单项 commit 重试都只把任务 ID 作为操作意图传到最终提交边界；实际发起
请求前会重新读取实时队列，只有当前仍为可提交状态且不再等待重复确认的任务才会进入
commit。这样，参照提交完成后刚被重新标为待确认的任务不会被旧渲染结果带入并发提交。
完成结果以响应中的 `ImageItem.md5` 作为图库事实；即使来源卡片在响应到达前已经离队，
仍会把该图片传播给当前队列中同 MD5 的任务，而不是依赖已离队任务的快照。
...
保护、时间规范化、批内顺序、来源去重、最终 MD5 确认和错误报告。
...
commit 成功响应返回导入状态与完整管理端图片对象，供队列直接把已经入库的实时参照
转换为图库重复项；响应重放同样读取当前图片及标签，不保留另一套精简结果结构。
...
- 卡片按服务端权威阶段单调前进：未开始或等待 materialize 许可为 `queued`
  “等待中”，实际素材化为 `uploading` / `downloading`，raw 完整落盘及等待 prepare
  许可为 `received`“待处理”，取得许可进入 `preparing` 后才投影为 `processing`
  “处理中”。SSE 与轮询共用同一映射，迟到快照不会把卡片退回更早阶段。
- 前端不读取整文件计算 MD5。本地来源预筛只用 `webkitRelativePath`（可用时）或
  `name + size + lastModified`；普通文件选择中浏览器拿不到完整路径，不假定存在
  未暴露的绝对路径。
- 远端来源先以规范化 HTTPS 原图 URL 排除确定相同的下载；服务端返回最终 MD5 后，
  队列再关联不同来源的相同处理结果，并让每张任务独立等待管理员确认。
- 卡片区分等待、上传/下载、待处理、处理、已就绪、提交、完成、失败、取消；显示存储后端
  显示名、处理前后像素尺寸、处理前后体积、质量或短路状态、失败原因及取消/重试。
  窗口统计固定分成“任务 / 处理中 / 待提交”和“成功 / 失败 / 重复待确认”两行，
  其中“处理中”不计尚在队列等待的任务。
- 上传 / 导入的“默认属性”和批量编辑的“批量默认属性”在移动端默认折叠；展开后以
  与图片列表筛选相同的 `80ms` 浮层过渡覆盖卡片区，不再挤压下方列表。触控、点击、
  键盘焦点或滚轮操作发生在浮层及其弹出菜单之外时会自动收起。
- “清空已完成”直接移除成功提交的卡片，且在其余任务仍在提交时也可使用；
  “清空未提交”会先显示当前任务数量并要求确认，确认后才会取消活动任务、释放
  本地预览并移除卡片，已经成功提交的图片不受影响。“清空重复待确认”仍直接执行。
  批量清理会固定点击或确认时的任务 ID 并立即更新本地队列，后端 cancel 异步收敛，
  本地 XHR 与服务器下载请求也会中止。
- 用户从顶部“更多导入方式”或导入工作流内的“链接 / 清单 / 微博”明确打开输入
  弹窗时，初始焦点落在当前模式唯一的文本区，可直接输入；仅打开通用导入工作流时
  仍使用父窗口自己的初始焦点。模式切换、解析结果和错误更新不会重复抢回文本焦点，
  从顶部直接入口打开时，关闭子窗口后由父窗口接管焦点；从工作流内来源按钮打开时
  则回到对应按钮，父窗口最终关闭后再归还页面入口。
...
| 任务阶段 | 单卡属性 | “应用到全部” |
| --- | --- | --- |
| `queued` / `uploading` / `downloading` / `received` | 锁定 | 本地上传与普通 URL 替换全部五项公共属性，空主题、空作者和空标签也会生效；JSONL / 微博仅替换清单未提供的属性 |
| `processing` | 锁定 | 锁定，避免处理中的会话同时改变语义 |
| `ready` | 可编辑 | 设备 / 明暗应用具体选择，`auto` 恢复本次 prepare 的检测结果；非空主题和作者覆盖，非空标签追加并去重，空值不清空现有值 |
| `committing` / `cancelling` / `done` / `cancelled` | 锁定 | 锁定 |
...
未就绪任务重试时保留当前草稿、清单字段来源和原队列位置，但清除旧会话、MD5、重复项、处理结果与设备/明暗检测结果，从会话创建、传输和 prepare 重新开始；新的检测结果会替换旧值。重试不会自动重新读取窗口默认属性，管理员需要时应先点击“应用到全部”。commit 请求失败后，前端会查询服务端会话状态：仍为 ready 才重新开放属性；期间若出现新的图库重复项，失败卡片直接复用统一重复确认面板，确认前不再提供 commit 重试。已经进入 committing 时继续锁定属性并通过同一幂等请求恢复结果；状态暂时无法确认时不会盲目重提，用户重试会先重新查询服务端，只有确认回到 ready 才重新参加重复判定，确认处于 committing 或 finalized 才继续幂等恢复。finalized、cancelled、missing 等已经不能保证 prepared 对象仍可读取的结果会清除旧预览地址；finalized 只有补取到完整管理端图片对象后才能收敛为完成，补取失败会保留可重试提交状态。
...
`inert` 与 `aria-hidden`。触发器再次关闭时焦点回到触发器；外部点击、焦点、滚轮
和页面滚动手势关闭时保留用户已转移到外部的焦点。Select / Facet Portal 与父浮层
同步关闭，跨越桌面 / 移动断点时也先收尾现有焦点和 Portal，再隐藏移动面板。
...
- 后台图片列表通过 `/api/admin/images?status=&d=&b=&t=&tag=&a=&cursor=&limit=`
  ...
  逻辑。移动浮层关闭的状态变更当帧会添加 `inert` 与 `aria-hidden`；面板内关闭
  将焦点归还筛选触发器，外部交互保留已经转移到外部的焦点。锚定菜单开始关闭前会
  先移走 Portal 内焦点，跨越断点也会同步收起 Portal，避免把仍含焦点的子树标记为
...
导入输入弹窗的 JSONL 与微博解析共用一个异步状态和一个主操作按钮。请求期间按钮是
页面唯一可见的进行中反馈，输入区、模式标签和 `aria-busy` 从同一 `pending` 派生；
解析不设置 500ms 最短等待，也不额外停留成功或失败按钮态，完成后由结果概览、问题
明细或错误区表达结果。普通 URL 仍在浏览器内同步解析，标题栏不再重复显示解析状态。

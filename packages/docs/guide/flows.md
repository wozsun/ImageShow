...
首页与画廊的共享主导航固定在视口顶部，两个页面在正常文档流中统一保留等高空间；
因此 iOS Safari 的顶部弹性滚动不会再把导航连同文档向下拖动并露出页面背景。
导航原有的向下隐藏、向上显示以及第二导航栏联动仍只通过 transform 改变可见状态。
固定主导航不参与弹窗冻结页面时的文档位移补偿；仍在文档中粘附的首页筛选栏和画廊
工具栏继续使用该补偿，避免在深处打开图片详情时主导航被重复向下平移。
...
分割线、按钮和信息面板，后台详情仍使用原浅色边界。移动端详情的图片与面板共享
真实的不透明灰蓝底色，滚动表面直接阻止局部边界回弹露底，不使用额外接缝遮挡条。
图片时间继续复用列表项已有的 `image_time`。若该
...
`touchstart`、指针、滚轮或焦点操作才按正常规则关闭。滚动方向和距离阈值只继续控制
工具栏显隐，不参与筛选浮层的关闭判断；软键盘收起时回落到文档根节点的焦点不会
收起面板或 Facet 菜单，键盘收起后的下一次菜单外物理操作再关闭。Select / Facet

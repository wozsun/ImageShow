...
链接导入输入窗口提供 URL 列表、JSONL 清单和微博链接三个标签，三者都进入 download 生命周期。URL 与 JSONL 按 `link_image.max_items` 限制单次输入数量，微博链接按 `weibo.max_items` 独立限制，微博解析后的合计图片使用 1000 张固定安全上限。同一批任务通过精简控制请求创建会话；单项创建只返回继续流程所需的 ID、`prepare_url`，以及 upload 专用的 `upload_url` 或 download 专用的 `materialize_url`。

本地 upload 与服务器 download 共用同一个两阶段调度器实现。队列按输入顺序分配到现有 N 条 lane；每条 lane 的当前项先 materialize，前端轮询确认服务端权威状态已经进入（或完成）`preparing` 后，才打开一个前瞻槽并素材化唯一后继。准入轮询使用 100ms 起步、最高 1 秒的有界退避，并在等待间隔内同时监听 prepare 请求完成，因此不会用固定 50ms 频率持续请求，也不会因退避延迟已经完成的任务。后继即使先到 `received`，也必须等当前 prepare 请求结束后才能 prepare；失败或取消的后继同样占用这一个槽，当前结束前不会补第三项。连续追加批次和重试进入同一个持久队列，不会另起 worker 绕过上限；页面卸载会停止取项、中止活动传输并清理已创建会话。并发 1 时峰值为 1 个 preparing + 1 个后继 materializing，并发 N 时分别最多 N 个；真正图片标准化仍只有 N 个。
...
prepare 完成后会删除 `data/tmp` 下的 raw 文件；素材化失败、prepare 失败、取消和过期清理同样删除 attempt 隔离的 `.raw.<attempt>` / `.raw.<attempt>.part`（并兼容升级前的 `.raw` / `.raw.part`）与 `_uploads` 候选对象。失败路径先持久化并确认 PostgreSQL 已进入 `failed` / `cancelled` 等可清理状态，再删除 raw 与暂存对象；权威状态无法确认时一律保留。清理只匹配完整 UUID 形态的文件，避免误删其他临时内容。完整分片通过同目录硬链接以“不覆盖本 attempt 目标”的方式原子发布为 `.raw.<attempt>`；随后只有 `execution_token` 仍匹配的执行者才能把同一 token 写入 `raw_token` 并发布 `received`。若进程在文件发布后、`received` 落库前退出，后续 materialize 会按数据库中的执行 token 恢复该完整文件；半成品不会进入恢复路径。升级时旧 `receiving` 统一迁到 `materializing`，遗留共享 `.raw` 会先被认领为 attempt 专属文件再恢复，不会把半成品宣告为已接收。
...
- 应用设置写 `config.json`，存储后端与密钥写 PostgreSQL。物理布局变更在独占位置锁内重新检查全部图片、全部未清理导入会话、未解决 `move.cleanup` 和一次 `_uploads` 快照，任一存在即拒绝；S3 Endpoint 可在同一独占锁内通过新旧 `_uploads` 快照、既有对象的有界 Range 读取和双向随机挑战证明为同一命名空间别名，成功后合并全部相交后端的 identity 集合。凭据、region、公开 URL 等非物理字段只需共享锁，并先验证既有对象读取和临时对象读写。

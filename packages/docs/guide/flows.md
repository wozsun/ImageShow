...
- 应用设置写 `config.json`，存储后端与密钥写 PostgreSQL。仍有图片引用的后端不能原地修改物理位置字段；凭据等可编辑字段会先验证既有对象读取和临时对象读写，再在存储独占维护锁内保存。
...
Worker 用 `FOR UPDATE SKIP LOCKED` 领取持久任务，按任务类型限制并发。每次 tick 先运行到期的僵尸恢复、过期导入调度和历史裁剪，再并行给每个可运行任务类型一个最多 50 项 / 2 秒的时间片；慢队列不能无限占住 tick。调试日志记录各类型 backlog、最老等待时间、处理数、耗时和预算是否耗尽。存储孤儿清理使用独占维护锁；导入持有共享维护锁，分类移动、主题重分配、存储迁移与彻底删除还会取得对应单图锁，防止清理任务删除尚未写入最终数据库引用的候选对象。人工存储清理也会在锁内读取仍处于 `created` / `receiving` / `preparing` / `ready` / `committing` 且租约未过期的导入会话，同时保护对应 `_uploads` 暂存对象和提交中的最终对象。已完成 / 已忽略任务保留 7 天后按批删除，耗尽重试的失败任务保留 90 天后删除；待执行、运行中和仍在重试窗口内的失败任务不会被历史清理删除。

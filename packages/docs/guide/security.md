...
- 登录前置安全验证使用完全自托管的 ALTCHA：服务端签发带 HMAC 的
  ...
  `altcha.enabled` 关闭；
  浏览器单次求解最多等待 90 秒，服务端同时限制
  `cost × counter_max <= 100000000`，挑战有效期最短 120 秒，避免可通过配置校验的
  工作量在客户端必然超时或完成后立即过期；
...
- 全站响应头包含 `X-Content-Type-Options`、`X-Frame-Options`、`Referrer-Policy`、`Cross-Origin-Opener-Policy` 与 CSP；SPA 以 report-only 模式监测脚本 Trusted Types，白名单仅包含构建生成的 ALTCHA PBKDF2 Worker 使用的 `imageshow-altcha-worker`，以及 ALTCHA 内部 Svelte 模板运行时使用的 `svelte-trusted-html`，不提供放行任意脚本 URL 或 HTML 的默认策略；登录页还会在 ALTCHA 首次挂载前预设隐藏 footer 与 logo，避免组件先按默认配置渲染不兼容 Trusted Types 的动态 HTML footer；应用只接受 `site.domain` 及其一级保留/主题子域名，未知 `Host` 直接返回不可缓存的 404。

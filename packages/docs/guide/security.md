...
- 普通 API 请求体在解析前限制为 128 KiB；JSONL 清单和批量图片编辑分别在管理员鉴权与 CSRF 通过后使用独立的 16 MiB、8 MiB 上限，本地图片上传继续使用按配置体积流式限流，避免匿名超大请求占用服务端内存。站点域名只接受 DNS 名称（开发环境可带端口），外部图片、来源、作者、站点资源和远端存储地址只接受 HTTPS。

...
监听器。`port`、`database` 或 `redis` 变化会写入文件，但监听端口、PostgreSQL
连接池和 Redis 客户端需要重启容器后才会按新值建立；`site.domain` 变化会提示
当前访问地址可能失效。完整配置接口和响应均禁止缓存，且仅允许 super 管理员
访问。
...
以防预检之后发生竞态。全部存储后端在同一数据库事务内写入；配置文件应用或
数据库提交失败时会恢复导入前的运行时配置，不保留部分结果。

...
`data/config.json`。编辑器包含 `port`、`database`、`redis`、`site.domain`
及数据库密码等敏感字段，与下方用于跨实例迁移的配置包范围不同。
...
保存继续使用临时文件和原子重命名，写入成功后替换内存配置并通知现有热加载
监听器。`port`、`database` 和 `redis` 可以保存。保存成功不代表已经生效；当前
进程仍使用启动时的监听端口、PostgreSQL 连接池和 Redis 客户端。修改
`database` 或 `redis` 后需要执行 `docker compose restart imageshow`。修改 `port`
时还需先同步 Compose 的容器目标端口映射，再执行 `docker compose up -d` 重新创建
应用容器。

Docker healthcheck 会读取主进程单独记录的当前实际监听端口，不读取 `config.json`
中等待重启的新端口。因此后台保存新端口后，重启前不会提前进入
unhealthy；重启后的主进程会用新端口更新标记。`site.domain` 变化会提示当前访问
地址可能失效。完整配置接口和响应均禁止缓存，且仅允许 super 管理员访问。

### 版本化配置包

super 管理员可在「设置 → 高级配置」导出或导入版本化 JSON 配置包。当前
格式为 `imageshow-config` 版本 1，`application_version` 仅用于识别导出来源；
是否兼容由包格式及其版本决定。配置包最大 1 MiB，单包最多包含 100 个
自定义存储后端。

配置包用于把可迁移的站点行为和存储连接复制到新实例：

- `config` 包含站点展示、上传 / 导入、图片处理、后台、安全、验证码和日志
  等运行时配置，但排除 `port`、`database`、`redis` 和 `site.domain`。这些
  字段属于新实例自己的监听、基础设施连接和访问域名，应在启动实例时配置。

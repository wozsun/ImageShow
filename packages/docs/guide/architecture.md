...
- Redis 是加速层：generation 随机池（`random:<generation>:item`、axis/category/tag/author 集合与计数）、画廊筛选项、公共列表 / 详情缓存、后台概览短缓存、原图直连探测缓存、MD5 判重、对象键 / 缩略图键 / 图片 id lookup、随机去重历史。资源出口按图片 id 读取时先查 `imageshow:image_lookup:id`，miss 或缓存损坏再回退 PostgreSQL。正常 `/random` 不依赖 PostgreSQL；Redis 不可用时随机 API 返回 503，其他读路径可按场景降级到 PostgreSQL。

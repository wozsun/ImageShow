...
export async function restoreImageFromTrash(id: string): Promise<RestoreResult> {
  ...
  try {
    ...
    if (!(await exists("objects", image.object_key, image.storage_backend))) {
      if (await exists("trash", image.object_key, image.storage_backend)) {
        await moveObject("trash", image.object_key, "objects", image.object_key, contentType(image.ext), image.storage_backend);
        movedFromTrash = true;
      } else {
        await client.query("ROLLBACK");
    ...
    await client.query(
      "INSERT INTO category(category_key, device, brightness, theme, count) VALUES($1,$2,$3,$4,0) ON CONFLICT DO NOTHING",
      [image.category_key, image.device, image.brightness, image.theme]
    );
    ...
    if (movedFromTrash && !committed && image) {
      await moveObject("objects", image.object_key, "trash", image.object_key, contentType(image.ext), image.storage_backend).catch(() => {

...
import { cleanupOrphanRawImports, removeRawImport } from "../images/imports/temp-files.js";
...
async function succeed(id: string, result: unknown = {}) {
  await pool.query("UPDATE background_job SET status='succeeded', result=$2::jsonb, updated_at=now() WHERE id=$1", [id, JSON.stringify(result)]);
}

async function ignore(id: string, reason: string) {
  await pool.query("UPDATE background_job SET status='ignored', error=$2, updated_at=now() WHERE id=$1", [id, reason]);
}
...
const taskHandlers: Record<string, (task: Task) => Promise<unknown>> = {
  "thumb.generate": async (task) => {
    ...
    if (!await exists("media", row.object_key, row.storage_slug)) return ignore(task.id, "object missing");
  ...
  "upload.cleanup": async (task) => {
    const rows = (await pool.query(
      `SELECT id, storage_slug
       FROM import_session WHERE status <> 'finalized' AND expires_at < now() LIMIT $1`,
    ...
    for (const row of rows) {
      try {
        // 三种导入模式都用 session id 生成暂存键，因此过期清理只需要按 id 扫 raw 与 _uploads。
        await Promise.all([
    ...
    const deleted = await pool.query(
      "DELETE FROM import_session WHERE id = ANY($1::uuid[]) OR (status = 'finalized' AND expires_at < now())",
      [cleanedIds]
    );
...
function typeConcurrency(type: string): number {
  ...
  switch (type) {
    ...
    case "move.cleanup": return config.background_job.move_cleanup_concurrency;
    default: return 1;
  }
}
...
async function runTick() {
  ...
  if (now - lastExpireUploads >= appConfig.backgroundJob.expireUploadsIntervalMs) {
...
async function expireUploads() {
  // 导入会话过期只入队一个清理任务，避免每次 tick 都插入重复 cleanup。
  await cleanupOrphanRawImports(appConfig.uploadTtlSeconds * 1000);
  ...
  await pool.query(
    `INSERT INTO background_job(id, type, status)
     ...
     WHERE EXISTS (SELECT 1 FROM import_session WHERE expires_at < now())
       AND NOT EXISTS (
         SELECT 1 FROM background_job WHERE type='upload.cleanup' AND status IN ('pending', 'running')
       )`,
    [uuidv7()]
  ).catch(() => undefined);
}
...
export async function drainWorker(timeoutMs = appConfig.backgroundJob.drainTimeoutMs) {

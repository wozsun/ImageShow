import {
  ...
  type ImportQueueActionTypeDto
} from "@imageshow/shared/browser";
import { ApiError } from "../../core/api-error.ts";
import type { AdminSession } from "../../users/admin-session.ts";
import {
  importActionWatermarkHash,
  verifyImportActionWatermark
} from "./action-scope.ts";
import { semanticImportSessionHash } from "./session-projection.ts";
import type {
  ImportTokenEnvelope,
  ImportTokenService
} from "./token-service.ts";

type ImportQueueContinuationClaims = ImportTokenEnvelope & Readonly<{
  action_scope: string;
...
const actionTypes = new Set<ImportQueueActionTypeDto>(importQueueActionTypes);

function isContinuationClaims(
  value: ImportTokenEnvelope
): value is ImportQueueContinuationClaims {
  ...
  return Object.keys(record).length === 14
    ...
    && actionTypes.has(record.action as ImportQueueActionTypeDto)
...
function actionPayloadHash(input: ImportQueueActionInputDto) {
...
export function resolveImportQueueActionCursor(input: Readonly<{
  request: ImportQueueActionInputDto;
  actionScope: string;
  session: Pick<AdminSession, "id" | "username">;
  tokens: ImportTokenService;
}>) {
  const watermark = verifyImportActionWatermark({
  ...
  const watermarkHash = importActionWatermarkHash(
...
export function signImportQueueActionContinuation(input: Readonly<{
  tokens: ImportTokenService;
  nextCursor: number | null;
  request: ImportQueueActionInputDto;
  resolved: ReturnType<typeof resolveImportQueueActionCursor>;
}>) {

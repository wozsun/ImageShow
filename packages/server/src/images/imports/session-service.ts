...
import { importIntentRequestHash } from "./request-hash.ts";
import {
  assertImageIdentity,
  createImportDisplayOrderKey,
...
type UploadCredential = ImportTokenEnvelope & {
  owner: string;
  session_id: string;
...
function isUploadCredential(value: ImportTokenEnvelope): value is UploadCredential {
...
function withSessionSemanticHash(
  session: Omit<ImportSessionSnapshot, "semantic_hash">
): ImportSessionSnapshot {
  const semantic_hash = importSessionSemanticHash(
    session as Omit<ImportSessionSnapshot, "semantic_hash">
  );
...
type ImportItemFailure = Extract<UploadIntentItemDto, { status: "failed" }>;
...
type ImportSessionServiceDependencies = Readonly<{
...
const importSessionServiceDefaults: ImportSessionServiceDependencies = {
...
export class ImportSessionService {
  ...
  readonly tokens: ImportTokenService;
  readonly #dependencies: ImportSessionServiceDependencies;

  constructor(
    repository = new ImportSessionRepository(),
    tokens = new ImportTokenService(),
    dependencies: Partial<ImportSessionServiceDependencies> = {}
  ) {
    this.repository = repository;
    this.tokens = tokens;
    this.#dependencies = {
      ...importSessionServiceDefaults,
      ...dependencies
    };
  }
  ...
  async #canonicalResult(
    ...
    committed: ReadonlyMap<string, CommittedImportResult>,
    lookupError: unknown,
    now: number
  ): Promise<CanonicalAcceptItem | ImportItemFailure> {
  ...
  async createUploadIntents(
    ...
    const results = await this.#dependencies.withStorageReadLock(async (signal) => {
      const created: Array<SettledItem<
        UploadIntentItemInputDto,
        Awaited<ReturnType<ImportSessionRepository["createUploadIntent"]>>
      >> = [];
      for (const item of items) {
        ...
        try {
          ...
          const requestHash = importIntentRequestHash({
            queue: "upload",
            source_type: "upload",
          ...
          const intent: UploadIntentSnapshot = {
            ...
            display_order_key: createImportDisplayOrderKey(
              item.batch_key,
              item.manifest_position,
              sessionId
            ),
            ...
            created_at: now,
            expires_at: now
              + appConfig.importRuntime.uploadIntentTtlSeconds * 1000,
            execution_token: "",
    ...
    let committed = new Map<string, CommittedImportResult>();
  ...
  async acceptRemoteItems(
    owner: string,
    items: readonly RemoteImportItemInputDto[],
    now = Date.now()
  ): Promise<RemoteImportAcceptItemDto[]> {
    const runtime = getRuntimeConfig();
    if (items.length > runtime.link_image.max_items) {
      throw new ApiError(
        400,
        "remote_import_batch_limit_exceeded",
        `单批最多允许 ${runtime.link_image.max_items} 张远程图片`
      );
    }
    const accepted = await this.#dependencies.withStorageReadLock(async (signal) => {
      ...
      for (const item of items) {
        ...
        try {
          ...
          const metadata = {
            ...draftMetadata(item),
            original: runtime.link_image.fill_original_url
              ? item.url
              : item.original
          };
          const requestHash = importIntentRequestHash({
          ...
          const template = withSessionSemanticHash({
            ...
            raw_size: 0,
            discard_at: now
              + appConfig.importRuntime.importSessionIdleTtlSeconds * 1000
          });
          results.push({
            input: item,
            result: (await this.repository.acceptRemoteSession(
              template,
              createImportDisplayOrderKey(
                item.batch_key,
                item.manifest_position,
    ...
    const response: RemoteImportAcceptItemDto[] = [];
  ...
  uploadReceivedTemplate(
    ...
    return withSessionSemanticHash({
      ...
      raw_size: rawSize,
      discard_at: now
        + appConfig.importRuntime.uploadSessionIdleTtlSeconds * 1000
    });
  }

...
import type {
  ...
  ImportQueueEventDto,
  ImportQueueTerminalEventItemDto
...
type ImportQueueEventRepository = Pick<
  ImportSessionRepository,
  "snapshot" | "subscribe"
>;
type ImportQueueActionScopes = Readonly<{
  open: typeof openImportActionScope;
  require: typeof requireImportActionScope;
  sign: typeof signImportActionWatermark;
}>;

const defaultActionScopes: ImportQueueActionScopes = {
  open: openImportActionScope,
  require: requireImportActionScope,
  sign: signImportActionWatermark
};

function terminalEventItem(
  session: Extract<StoredImportSession, { status: "completed" | "discarded" }>
): ImportQueueTerminalEventItemDto {
  return {
...
function completedEventItem(
  session: Extract<StoredImportSession, { status: "completed" }>,
...
export function streamImportQueueEvents(
  context: Context,
  input: Readonly<{
    repository: ImportQueueEventRepository;
    ...
    actionScopes?: ImportQueueActionScopes;
  }>
) {
  ...
  const authenticationHeartbeatMs = input.authenticationHeartbeatMs
    ?? appConfig.importRuntime.sseAuthenticationHeartbeatSeconds * 1_000;
  ...
  const response = streamSSE(context, async (stream) => {
    ...
    const enqueue = (event: string, payload: ImportQueueEventDto) => {
    ...
    try {
      ...
      const buffered: ImportQueueMutation[] = [];
      const mutationPayload = (mutation: ImportQueueMutation) => {
        if (!mutation.session) {
          throw new Error("Import queue mutation omitted its session identity");
        }
        const payload: ImportQueueEventDto = {
          type: "mutation",
          queue: input.queue,
          kind: mutation.kind,
          revision: mutation.metadata.revision,
          last_accepted_order: mutation.metadata.last_accepted_order,
          summary: presentImportQueueSummary(mutation.metadata),
      ...
      const emitMutation = (mutation: ImportQueueMutation) => {
        if (!readySent) {

...
type ImportRetiredCleanupQueueOptions = Readonly<{
  capacity?: () => number;
  concurrency?: () => number;
  lossy?: boolean;
  maxAttempts?: () => number;
 ...
 */
class ImportRetiredCleanupQueue {
  ...
  constructor(options: ImportRetiredCleanupQueueOptions = {}) {
    this.#capacity = options.capacity
      ?? (() => appConfig.importRuntime.queueActionBatchSize);
    this.#lossy = options.lossy ?? false;
    this.#maxAttempts = options.maxAttempts
      ?? (() => appConfig.importRuntime.retiredCleanupMaxAttempts);
...
export const importRetiredCleanupQueue = new ImportRetiredCleanupQueue({
  lossy: true
});

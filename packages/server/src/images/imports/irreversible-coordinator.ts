 ...
 */
export class ImportIrreversibleCoordinator {
  readonly #records = new Map<string, IrreversibleRecord>();
  readonly #tails = new Map<string, Promise<void>>();

  async #critical<T>(pair: ImportSessionPair, work: () => Promise<T>) {
  ...
  registerCancellable(pair: ImportSessionPair) {
    const key = pairKey(pair);
  ...
  unregisterCancellable(pair: ImportSessionPair) {
    const key = pairKey(pair);
  ...
  state(pair: ImportSessionPair) {
    return this.#records.get(pairKey(pair))?.state ?? null;
  }

  settled(pair: ImportSessionPair) {

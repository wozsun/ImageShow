...
function parseCompletedDisplay(value: unknown, queue: unknown) {
  ...
  enumValue(display.source_type, importSourceTypes, "display.source_type");
  if (
...
export function parseStoredImportSession(raw: string): StoredImportSession {
  let parsed: unknown;
  try {
  ...
  if (!importQueueTypes.includes(value.queue as never)) {
    throw new Error("Redis import value has invalid queue");
  }
  if (!importSessionStatuses.includes(value.status as never)) {
  ...
  if (value.status === "discarded") {
    ...
    if (!importSourceTypes.includes(value.source_type as never)) {
  ...
  return value as StoredImportSession;
}
...
export function parseImportQueueMetadata(
  value: unknown
): ImportQueueMetadata {
  ...
  if (!importQueueTypes.includes(metadata.queue as never)) {
  ...
  return metadata as unknown as ImportQueueMetadata;
}
...
export function metadataFromHashReply(values: unknown[]) {
  ...
  return parseImportQueueMetadata(metadata);
}

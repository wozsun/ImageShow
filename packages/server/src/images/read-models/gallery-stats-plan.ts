...
export async function resolveGalleryStatsPlan(
  query: GalleryStatsQuery,
  access: VocabularyReadAccess
) {
  const values =
    query.tag === undefined ? [] : typeof query.tag === "string" ? [query.tag] : query.tag;
  ...
  if (
    scope !== undefined &&
    (!Number.isInteger(scope) || scope < 1 || groups[scope - 1]?.mode !== "all")
  ) {
    throw new ApiError(400, "validation_error", "标签计数范围必须指向已选的且组", {
      field: "tag_scope"
    });
  }
  ...
  const withTags = (tag: TagExpression) =>
    createImageFilterPlan({
  ...
  const plan = withTags(
    normalizeTagExpression(expressions.flatMap((expression) => expression?.anyOf ?? []))
  );
  ...
  const tagCounts: GalleryTagCountPlans = {
    ...
    signature: JSON.stringify([
      ...
      groupPlans.map((group) => group.signature)
    ])
  };

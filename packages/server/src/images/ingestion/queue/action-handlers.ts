...
import { acceptIngestionCommitIntents } from "../commit/intent.ts";
...
function pair(session: StoredIngestionSession): IngestionSessionPairDto {
  return {
...
function metadataForAction(
  ...
  if (patch.brightness !== undefined) {
    metadata.brightness =
      patch.brightness === "auto" && prepared
...
async function commitReadyAction(
  ...
  if (selected.length) {
    input.assertScope();
    const accepted = await acceptIngestionCommitIntents(
      input.repository,
      input.owner,
...
async function clearQueueAction(
  ...
  if (active.length) {
    ...
    for (let attempt = 0; pending.length && attempt < queueActionMutationAttempts; attempt += 1) {
      ...
      const newlyCompleted: CompletedIngestionReceipt[] = [];

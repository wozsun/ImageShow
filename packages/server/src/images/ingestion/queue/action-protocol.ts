...
type IngestionQueueContinuationClaims = IngestionTokenEnvelope &
  Readonly<{
    action_scope: string;
...
export function resolveIngestionQueueActionCursor(
  input: Readonly<{
  ...
  const watermarkHash = ingestionActionWatermarkHash(input.request.action_watermark);
  ...
  if (
    ...
    continuation.redis_connection_epoch !== watermark.redis_connection_epoch ||
    ...
    continuation.captured_queue_revision !== watermark.captured_queue_revision ||
...
export function signIngestionQueueActionContinuation(
  input: Readonly<{
    tokens: IngestionTokenService;
  ...
  try {
    return input.tokens.sign(
      ingestionActionContinuationPurpose,
      {
        ...
        redis_connection_epoch: input.resolved.watermark.redis_connection_epoch,
        ...
        captured_queue_revision: input.resolved.watermark.captured_queue_revision,

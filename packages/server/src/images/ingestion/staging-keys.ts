...
const ingestionStagingKeyPattern = new RegExp(
...
type IngestionStagingKeyIdentity = Readonly<{
  session_id: string;
  image_id: string;
  generation: string;
...
function parseIngestionStagingKey(key: string): IngestionStagingKeyIdentity | null {
  const match = ingestionStagingKeyPattern.exec(key);
...
export type IngestionStagingCleanupKeyIdentity = IngestionStagingKeyIdentity & Readonly<{
  base_key: string;
 ...
 */
export function parseIngestionStagingCleanupKey(
  key: string
): IngestionStagingCleanupKeyIdentity | null {
  const direct = parseIngestionStagingKey(key);
  if (direct) {
    return {
      ...direct,
      base_key: key,
  ...
  const identity = baseKey ? parseIngestionStagingKey(baseKey) : null;
...
export function stagingSessionId(key: string) {
  return parseIngestionStagingCleanupKey(key)?.session_id ?? "";
}

export function ingestionStagingImageKey(input: {
  session_id: string;
  image_id: string;
  generation: string;
  execution_token: string;
...
export function ingestionStagingThumbnailKey(input: {
  session_id: string;
  image_id: string;
  generation: string;
  execution_token: string;

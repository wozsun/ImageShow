...
export function readyImageFilterOperations(
  input: {
    all: FilterSetSource;
  ...
  const combine = (
    ...
    const count =
      kind === "union"
        ? Math.min(
            input.all.count,
            active.reduce((sum, source) => sum + source.count, 0)
          )
        : kind === "intersection"
          ? Math.min(...active.map((source) => source.count))
          : active[0]!.count;
  ...
  if (input.tagClauses.length) {
    components.push(
      combine(
        "union",
        input.tagClauses.map((clause) => combine("intersection", clause))
      )
    );
  }
  ...
  for (const component of components.slice(1))
    result = combine("intersection", [result, component]);
  for (const group of input.exclusions) {
    if (group.some((source) => source.count > 0))
      result = combine("difference", [result, combine("union", group)]);
  }

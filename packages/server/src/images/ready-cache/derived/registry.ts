...
const EVICTION_BATCH_SIZE = Math.floor(REDIS_BATCH_MAX_COMMANDS / MAX_EVICTION_COMMANDS_PER_RESULT);
const MAX_REGISTRY_ENTRIES_DURING_REGISTRATION = READY_IMAGE_DERIVED_CACHE_POLICY.maxResults + 1;
...
export async function evictReadyImageDerivedResults(keys: Iterable<string>) {
  ...
  let modified = false;
  for (let offset = 0; offset < uniqueKeys.length; offset += EVICTION_BATCH_SIZE) {
...
async function assertDerivedRegistryStructure() {
  ...
  const memberSets = memberResults.map((result) => [...((result?.[1] as string[]) ?? [])].sort());
  const expected = JSON.stringify(memberSets[0]);
  if (memberSets.slice(1).some((members) => JSON.stringify(members) !== expected)) {
    throw new Error("Ready-image derived registry has inconsistent field sets");
  }
}
...
export async function registerReadyImageDerivedResultUnchecked(options: {
  ...
  let memberships = retained.reduce(
    (sum, entry) => (entry.kind === "stats-result" ? sum : sum + entry.count),
    0
  );
  ...
  for (;;) {
    const signatures = new Set(
      retained.flatMap((entry) => (entry.signature ? [entry.signature] : []))
    );

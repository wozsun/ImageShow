...
function readyImageStatsDigest(stats: ReadyImageStats) {
  ...
  const entries = [...stats].sort(([left], [right]) => (left < right ? -1 : left > right ? 1 : 0));
...
export async function writeReadyImageStatsAndIntegrity(
  ...
  for (const chunk of chunkHashEntries(statEntries())) {
    ...
    await writer.queue(estimatedRedisBytes(...flat), (pipeline) => {
      pipeline.hset(READY_IMAGE_STATS_KEY, ...flat);
    });
  }
  ...
  for (const chunk of chunkHashEntries(manifestEntries())) {
    const flat = chunk.flat();
    await writer.queue(estimatedRedisBytes(...flat), (pipeline) => {
      pipeline.hset(READY_IMAGE_INTEGRITY_KEY, ...flat);
    });
  }
  await writer.flush();
  signal?.throwIfAborted();
...
export async function readReadyImageIntegrity(client: Redis): Promise<ReadyImageCardinalities> {
  ...
  if (
    ...
    (await client.hlen(READY_IMAGE_INTEGRITY_KEY)) !== entryCount + INTEGRITY_RESERVED_FIELD_COUNT
  ) {
...
export async function validateReadyImageCardinalities(
  ...
  const flush = async () => {
    ...
    queued.forEach(([key, expected], index) => {
      ...
      if (actual !== expected) {
        throw new Error(`Ready-image cache cardinality mismatch for ${key}: ${actual}/${expected}`);
      }
    });
...
export async function updateReadyImageIntegrity(
  ...
  for (const [key, count] of cardinalities) {
    if (count || coreCardinalityKeys.has(key)) {
      await writer.queue(estimatedRedisBytes(READY_IMAGE_INTEGRITY_KEY, key, count), (pipeline) => {
        pipeline.hset(READY_IMAGE_INTEGRITY_KEY, key, String(count));
      });
    } else {
      await writer.queue(estimatedRedisBytes(READY_IMAGE_INTEGRITY_KEY, key), (pipeline) => {
        pipeline.hdel(READY_IMAGE_INTEGRITY_KEY, key);
      });
    }
  }
  await writer.flush();

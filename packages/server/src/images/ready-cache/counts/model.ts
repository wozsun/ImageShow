...
function countRecord(value: unknown): value is CountRecord {
  return Boolean(
    ...
    typeof value === "object" &&
    !Array.isArray(value) &&
    Object.values(value).every((count) => jsonReadyImageCount(count) !== null)
  );
}
...
function validSnapshot(value: unknown): value is ReadyImageCountSnapshot {
  ...
  if (
    ...
    (Object.hasOwn(snapshot, "tagGroups") &&
      (!Array.isArray(snapshot.tagGroups) ||
        ...
        snapshot.tagGroups.some(
          (value) => jsonReadyImageCount(value) === null || value > matching
        ))) ||
  ...
  return (
    safeCountSum(snapshot.axes) === matching &&
...
function assertGlobalStats(stats: Map<string, number>, expectedTotal: number) {
  ...
  const axisTotal = readyImageAxisPairs.reduce(
    (sum, axis) => sum + (stats.get(`axis:${axis.device}:${axis.brightness}`) ?? 0),
    0
  );
  const deviceTotal = devices.reduce(
    (sum, device) => sum + (stats.get(`device:${device}`) ?? 0),
    0
  );
  const brightnessTotal = brightnesses.reduce(
    (sum, brightness) => sum + (stats.get(`brightness:${brightness}`) ?? 0),
    0
  );
  ...
  const validFixedFields = new Set([
    "total",
    ...readyImageAxisPairs.map(({ device, brightness }) => `axis:${device}:${brightness}`),
...
export function isUnfilteredReadyImagePlan(plan: ImageFilterPlan) {
  return (
    plan.axes.length === readyImageAxisPairs.length &&

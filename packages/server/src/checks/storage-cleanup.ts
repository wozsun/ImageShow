...
export async function cleanupStorage() {
  ...
  const uploadRows = (await pool.query(
    "SELECT id, staging_object_key, final_object_key, storage_slug FROM upload_session WHERE status IN ('created','receiving','preparing','ready','committing') AND expires_at >= now()"
  )).rows as Array<{ id: string; staging_object_key: string; final_object_key: string | null; storage_slug: string }>;
  ...
  for (const backend of backends) {
    try {
      ...
      const candidates: Array<readonly [StoragePrefix, string]> = [
        ...(await listStorageKeys("objects", backend)).filter((key) => !ready.has(key) && !deleted.has(key) && !committingOnBackend.has(key) && !knownOnBackend.has(objectKeyId(key))).map((key) => ["objects", key] as const),

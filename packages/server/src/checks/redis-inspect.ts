...
type RedisInspectionOptions = Omit<RedisDeepInspectionOptions, "client" | "signal"> & {
  dependencies?: RedisInspectionDependencies;
};

type RedisInspectionClient = Pick<Redis, "status" | "info" | "dbsize" | "scan" | "pipeline">;
...
const defaultRedisInspectionDependencies: RedisInspectionDependencies = {
  ...
  readRequiredCommands: (signal) => readRequiredRedisCommandCapabilities(redis, signal),
...
function deadlineInspectionResponse(options: {
  ...
  dbsize: number | null;
  requiredCommands: Awaited<ReturnType<typeof readRequiredRedisCommandCapabilities>> | null;
...
export async function inspectRedisState(
  ...
  let requiredCommands: Awaited<ReturnType<typeof readRequiredRedisCommandCapabilities>> | null =
    null;
  let projection: Awaited<ReturnType<typeof readReadyImageCacheAdminStatus>> | null = null;
  ...
  try {
    ...
    if (currentPersistedMeta?.state !== "ready") {
      issues.push(`统一图片缓存状态不是 ready：${currentPersistedMeta?.state ?? "missing"}`);
    }
    if (!currentDeepInspection.complete) {
      issues.push(`Redis 深度扫描未完成：${currentDeepInspection.reason}`);
...
function parseRedisInfo(
  info: string,
  picked = new Set(["used_memory_human", "mem_fragmentation_ratio", "db0"])
) {

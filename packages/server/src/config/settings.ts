...
export async function getSettingsForAdmin() {
  ...
  const { name, domain, icon_url, root_redirect, home, gallery, random_default_method } = settings.site;
  const { login_background, image_page_size, recent_uploads, show_unset_theme_card } = settings.admin;
  return {
    site: { name, domain, icon_url, root_redirect, home, gallery, random_default_method },
...
export function siteConfigPayload() {
  const runtime = getRuntimeConfig();
  const { name, domain, icon_url, root_redirect, home, gallery, random_default_method } = runtime.site;
  return {
    site: {
      name,
      domain,

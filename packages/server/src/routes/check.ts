...
export function registerCheckRoutes(app: Hono) {
  ...
  app.post(`${adminApiBasePath}/check/redis`, async (c) =>
    c.json(apiSuccess(await inspectRedisState(c.req.raw.signal)))
  );
  app.post(`${adminApiBasePath}/check/storage`, async (c) =>
    c.json(apiSuccess(await checkStorage(c.req.raw.signal)))
  );
  app.post(
    ...
    async (c) => c.json(apiSuccess(await maintainStorageAndPurgeTasks(c.req.raw.signal)))
  );
  app.post(`${adminApiBasePath}/check/all`, async (c) =>
    c.json(apiSuccess(await checkSystemState(c.req.raw.signal)))
  );
}

...
export const ingestionSessionIdInput = z
  .string()
...
export const importAcceptInput = z
  .strictObject({
...
export const ingestionSnapshotSelectionInput = z
  ...
  .superRefine((value, context) => {
    ...
    const excluded = new Set(
      value.exclude_items.map((item) => item.session_id + "\0" + item.image_id)
    );
...
export const ingestionSessionUpdateInput = z
  .strictObject({
    items: z
      .array(
        ingestionPairInput
          ...
          .refine(
            (item) =>
              ...
              item.duplicate_decision !== undefined ||
...
export const ingestionDuplicateDetailsInput = z
  .strictObject({
    md5s: z
      .array(z.string().regex(/^[a-f0-9]{32}$/u))
      .min(1)
      .max(ingestionStatusBatchMaxItems)
  })
  .superRefine((value, context) => {
...
export const ingestionCommitIntentInput = z
  .strictObject({
    items: z
      .array(
        ingestionPairInput.extend({
          expected_version: expectedIngestionVersionInput,
...
export const ingestionCancelInput = z
  .strictObject({
    items: z
      .array(
        ingestionPairInput.extend({
...
const ingestionActionMetadataInput = z
  .strictObject({
    ...imageMetadataUpdateFields,
...
export const ingestionQueueActionInput = z
  .strictObject({
    queue: z.enum(ingestionQueueTypes),
...
export const jsonlManifestInput = z.strictObject({
  content: z
    .string()
    .min(1)
    .max(appConfig.ingestion.jsonlManifestMaxBytes)
    .refine(

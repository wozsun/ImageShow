...
import { adminApiBasePath, type AdminPreferencesResponseDto } from "@imageshow/shared/browser";
import { ApiError } from "../core/api-error.ts";
import { apiSuccess, apiSuccessEtag, privateCacheableApiSuccess } from "../core/http/responses.ts";
...
import { readAdminPreferences, updateAdminPreferences } from "../users/preferences.ts";
...
export function registerAdminPreferenceRoutes(app: Hono) {
  ...
  app.patch(`${adminApiBasePath}/preferences`, limitAdminPreferencesBody, async (c) => {
    const preferences = parse(adminPreferencesInput, await readJsonBody(c));
    const savedPreferences = await updateAdminPreferences(authenticatedUsername(c), preferences);
    const response = {

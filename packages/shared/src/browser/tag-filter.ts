...
export function parseTagFilter(
  ...
  for (const value of values) {
    ...
    const terms = (all ? raw.slice(4) : raw).split(",").map((part) => {
      ...
      if (!term || /^[!:]/u.test(term) || /^all:/iu.test(term)
        || /[\u0000-\u001f\u007f]/u.test(term)) {
...
export function resolveTagExpression(expression: TagExpression, terms: ReadonlyMap<string, string>): TagExpression {
  ...
  return normalizeTagExpression(expression.anyOf.map((clause) => clause.map((term) => {
...
export function tagExpressionValues(expression: TagExpression, mode?: TagMatchMode): string[] {
  ...
  return [
    ...
    ...normalized.anyOf.filter((clause) => clause.length > 1).map((clause) => `all:${clause.join(",")}`)
  ];
}
...
export function readableFilterSearch(params: URLSearchParams) {
  return [...params].map(([key, value]) => `${encodeURIComponent(key)}=${encodeURIComponent(value)
    .replace(/%2C/giu, ",").replace(/%3A/giu, ":").replace(/%21/giu, "!")}`).join("&");
}

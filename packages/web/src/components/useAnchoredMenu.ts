...
import { computeAnchoredPosition, type AnchoredMenuSize, type MenuPosition } from "../lib/menu-position.js";

// Shared open/close + positioning machinery for the portal-rendered dropdown
// menus (SelectMenu, ThemeInput, ThemeSelector). It owns the open/closing state,
// keeps a fixed-position box anchored to the trigger (re-anchoring on
// resize/scroll), and closes the menu on outside pointerdown. Escape-to-close,
// close-on-focus-outside and focus-on-open are opt-in so each caller keeps its
// own keyboard/focus feel. `requestClose(afterClose?)` always performs the close
// (plus `onClose` cleanup); the optional callback only adds a side effect such as
...
export function useAnchoredMenu(options: {
  ...
  // Read the per-render callbacks through refs so the stable callbacks and the
  // open-effect below don't have to re-create / re-subscribe every render.

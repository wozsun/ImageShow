...
export function LazyGalleryImage({ src, alt, device, width, height, priority = false }: { src: string; alt: string; device: Device; width: number; height: number; priority?: boolean }) {
  ...
  // Priority (above-the-fold) tiles render their <img> immediately and skip the observer, so the
  // LCP thumbnail isn't gated behind a scroll-in; everything else stays lazy.

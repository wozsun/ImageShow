import { basicTagValue, detectDeviceFromUserAgent, publicTagGroupLimit, type GalleryFacetsDto, type GalleryStatsDto } from "@imageshow/shared/browser";
...
import { publicFilterLabels, publicFilterSections, type PublicFilterDraft, type PublicFilterSection as PublicFilterSectionName, type TagSelection } from "../../../lib/gallery/public-filter-draft.js";
...
function optionCount(stats: GalleryStatsDto | undefined, section: PublicFilterSectionName, slug: string): number | undefined {
...
type Props = {
  ...
  updateTagGroup: (changes: Partial<Pick<TagSelection["groups"][number], "mode" | "selected">>) => boolean;
  finishTagGroup: () => void;
  deleteTagGroup: (id: number) => void;
  toggle: (section: PublicFilterSectionName, slug: string) => void;
};

export function PublicFilterSection({ section, id, draft, tag, tagLocked, options, facets, facetsLoading,
  totals, counts, availabilityUnverified, normalizedQuery, matchName, edit, setTag, updateTagGroup,
  finishTagGroup, deleteTagGroup, toggle }: Props) {
  ...
  return <section id={`${id}-${section}`} data-filter-section={section}
    ...
    <div className="public-filter-section-heading"><h3 id={`${id}-${section}-title`}>
      ...
      {publicFilterLabels[section]} {!isFixed && <small>{options.length}</small>}</h3>
      ...
      {tag.selection.groups.map((group) => {
        ...
        return <div key={group.id} className="public-filter-tag-group">
          <button type="button" aria-pressed={group.id === tag.selection.activeId} disabled={tagLocked}
            ...
            <b>{group.id}</b><small>{group.mode === "all" ? "且" : "或"}</small><OverflowMarqueeText text={text} />
            {value && <small className="public-filter-group-count" title="当前设备、明暗、主题和作者条件下的本组匹配数">{countLabel}</small>}
          </button>
    ...
    {!isFixed && !facets ? <p className="public-filter-muted">{facetsLoading ? "正在读取目录…" : "目录暂不可用，可先调整其他条件。"}</p>
      ...
      : !options.length ? <p className="public-filter-muted">暂无{publicFilterLabels[section]}选项</p>
      : <div className={`public-filter-options ${isFixed ? "is-segmented" : section === "tag" ? "is-tags" : ""}`}>
        {options.map((option) => {
          ...
          return <button key={option.slug} data-filter-option={option.slug} type="button"
            ...
            {section !== "tag" && <span className="public-filter-option-icon"><Icon name={publicFilterIcons[section]} /></span>}

...
import {
  publicFilterLabels, type PublicFilterChip
...
export function PublicFilterChips({ chips, revealChip, emptyLabel, returnFocusRef, onRemove }: {
  ...
  useLayoutEffect(() => {
    ...
    if (revealChip && revealChip !== revealedChipRef.current) {
      ...
      const index = chips.findIndex((chip) => chip.section === revealChip.section && (
        revealChip.groupId !== undefined ? chip.groupId === revealChip.groupId
      ...
      if (box && item instanceof HTMLElement) {
        ...
        if (itemRect.width > availableWidth) {
          target = revealChip.groupId === undefined ? left - leading : right + trailing - box.clientWidth;
  ...
  return <div ref={wrapRef} className="public-filter-chip-control">
    ...
    <div ref={scrollRef} className="public-filter-chips tag-input-scroll-window"
      ...
      {chips.map((chip) => <DirectActivationButton key={chip.groupId === undefined ? `${chip.section}:${chip.value}` : `tag-group:${chip.groupId}`} type="button"
        ...
        pointerFocus="preserve" onActivate={() => onRemove(chip)}>
        <span>{chip.exclude ? "排除" : publicFilterLabels[chip.section]}{chip.groupId !== undefined && ` ${chip.groupId}`}</span> {chip.label}<Icon name="close-line" />
      </DirectActivationButton>)}

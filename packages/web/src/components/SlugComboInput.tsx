...
// A single-select slug box with a styled typeahead dropdown — the shared implementation behind
// ThemeInput and AuthorInput (an image has one theme and one author). Input is restricted to slug
// characters (lowercase a-z, 0-9, -), so admins type/create by slug; a new entry's display name is
// set later in its management page. At rest the box shows the selected option's display name
// (falling back to the slug); focusing reveals the raw slug to edit. The stored value (and what
// onChange emits) is always the slug. `noun` labels the inline hints; `checkReserved` (theme only)
// flags a slug colliding with a reserved subdomain prefix, which the server rejects.
export function SlugComboInput({ value, onChange, options, noun, checkReserved = false, placeholder, disabled = false, ariaLabel, className }: {
  ...
  // At rest the box shows the display name; while focused it shows the editable slug.
  ...
  // Only suggest once something is typed; an empty box shows no dropdown. The 'none' sentinel
  // (管理页里的「未设置」) is never a pickable option — an empty box already means none.
  const matches = query
    ...
    : [];
  // A slug equal to a reserved subdomain prefix collides with the random API / static object
  // host, so the server rejects it; warn inline before submitting (theme only).
  const reserved = checkReserved && (reservedSubdomains as readonly string[]).includes(query);
  // A valid, non-reserved slug that matches no existing option will be created on submit; flag it
  // green so it's clear a new entry is about to be minted. Because the input only accepts slug
  // characters, a non-slug "will create" (e.g. Chinese) can no longer appear.
  const isNew = slugPattern.test(query) && query.length <= 32 && !reserved && !options.some((option) => option.slug === query);

  const choose = (slug: string) => {
    onChange(slug);
    requestClose();
    // Drop focus so the box flips back to showing the display name immediately.
    setFocused(false);

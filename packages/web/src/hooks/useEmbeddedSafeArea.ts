...
export function useEmbeddedSafeArea(enabled: boolean) {
  useEffect(() => {
    ...
    const report = () => {
      ...
      lastReport = key;
      parent.postMessage(
        { channel, version: 1, type: "native-insets", bridgeId, ...native },
        origin
      );
    };
    ...
    const apply = () => {
      for (const edge of edges) {
        ...
        if (insets && !document.fullscreenElement)
          root.style.setProperty(property, `${insets[edge]}px`);
        else root.style.removeProperty(property);
      }
    };
    const announce = (target = origin ?? "*") =>
      parent.postMessage(
        {
          channel,
          version: 1,
    ...
    const onMessage = (event: MessageEvent<unknown>) => {
      if (event.source !== parent || event.origin === "null" || (origin && event.origin !== origin))
        return;
        ...
        if (
          !edges.every(
            (edge) =>
              ...
              candidate[edge] >= 0 &&
              candidate[edge] <=
                (edge === "top" || edge === "bottom" ? window.innerHeight : window.innerWidth) / 2
          )
        )
          return;
        origin = event.origin;
    ...
    return () => {
      listeners.abort();
      if (observer && origin)
        parent.postMessage({ channel, version: 1, type: "disconnect", bridgeId }, origin);
      stopReporting();
      insets = null;
      apply();
    };

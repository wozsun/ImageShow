...
export function usePublicFilterScroll(sectionKey: string, searching: boolean) {
  ...
  const sections = () => Array.from(scrollRef.current?.querySelectorAll<HTMLElement>("[data-filter-section]") ?? []);
  ...
  const rememberPosition = () => {
    ...
    if (!section) return;
    const option = Array.from(section.querySelectorAll<HTMLElement>("[data-filter-option]"))
      .find((item) => item.getBoundingClientRect().bottom > top + 24);
    savedPosition.current = {
      section: section.dataset.filterSection!, option: option?.dataset.filterOption,
  ...
  useLayoutEffect(() => {
    ...
    if (!searching && wasSearching.current && savedPosition.current) {
      ...
      if (target) root.scrollTop += target.getBoundingClientRect().top - root.getBoundingClientRect().top - saved.offset;
    ...
    const measure = () => {
      ...
      if (sectionScrollTarget.current !== null
        && Math.abs(root.scrollTop - sectionScrollTarget.current) < 1) return;
      sectionScrollTarget.current = null;
      const items = sections();
      const top = root.getBoundingClientRect().top + 36;
      const atBottom = root.scrollHeight > root.clientHeight + 2
        && root.scrollTop + root.clientHeight >= root.scrollHeight - 3;
      const anchor = atBottom ? items.at(-1)
      ...
      const row = anchor ? items.filter((item) => Math.abs(item.getBoundingClientRect().top - anchor.getBoundingClientRect().top) < 1) : [];
      const active = row.find((item) => item.dataset.filterSection === preferredSection.current) ?? row[0];
    ...
    for (const event of manualEvents) interactionRoot.addEventListener(event, cancelSectionScroll, { passive: true });
    measure();
    return () => {
      observer.disconnect();
      root.removeEventListener("scroll", schedule);
      for (const event of manualEvents) interactionRoot.removeEventListener(event, cancelSectionScroll);
      cancelSectionScroll();
      if (frame !== undefined) window.cancelAnimationFrame(frame);
    };
  }, [sectionKey, searching, cancelSectionScroll]);

  const goToSection = useCallback((section: PublicFilterSection) => {
    const root = scrollRef.current;
    ...
    const to = Math.max(0, Math.min(root.scrollHeight - root.clientHeight,

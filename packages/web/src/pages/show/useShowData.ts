...
import {
  ...
  type ShowCandidateUsage
} from "./show-data-pool.js";
...
export function useShowData(
  ...
  const requestIdentity = readableFilterSearch(
  ...
  const requestFilters = useMemo(
    () => galleryFiltersFromSearchParams(new URLSearchParams(requestIdentity)),
    [requestIdentity]
  );
  ...
  const publishImages = useCallback((next: ShowImage[]) => {
    ...
    for (const id of confirmedImageEditsRef.current.keys())
      if (!ids.has(id)) confirmedImageEditsRef.current.delete(id);
  }, []);

  const request = useCallback(
    async (replace: boolean, validate = false) => {
      ...
      try {
        ...
        if (!replace && cursorExhaustedRef.current) {
          cursor = "";
          recentImageIdsRef.current = new Set(
            [...recentImageIdsRef.current].slice(-continuationLimit)
          );
        }
        ...
        for (let attempt = 0; attempt < 4; attempt += 1) {
          ...
          const response = await api<PublicImageListResponseDto<"show">>(path, {
            ...
            ...(validate ? { cache: "no-cache" as const } : {})
          });
          ...
          const incoming = eligible.filter(
            (item) => replace || !recentImageIdsRef.current.has(item.id)
          );
          ...
          if (replace || (!previous.length && accepted.length > 0)) {
            const value = {
              sourceKey,
              order,
              dataKey: `${sourceKey}#${++consumptionRoundRef.current}`
            };
        ...
        if (
          ...
          !requestPausedAfterErrorRef.current
        ) {
          completed.resume?.();
  ...
  const loadMore = useCallback(
    (usage?: ShowCandidateUsage) => {
      if (!enabled) return;
      if (usage) {
        if (
          usage.dataKey !== committedRef.current.dataKey ||
          committedRef.current.sourceKey !== sourceKey
        )
          return;
        ...
        const needsRotation =
          !cursorExhaustedRef.current ||
          hasRetiredCandidatesRef.current ||
          imagesRef.current.length > usage.capacity;
  ...
  const removeImage = useCallback(
    (imageId: string, replenish = false) => {
  ...
  return {
    ...
    retry: () => {
      ...
      void request(
        expired || Boolean(failure?.replace) || imagesRef.current.length === 0 || transitioning
      );
    }
  };
}

...
export function GalleryPage({ fixedTheme = "", standalone = false }: { fixedTheme?: string; standalone?: boolean }) {
  ...
  const themeNames = useMemo(() => new Map((options?.themes ?? []).map((option) => [option.slug, displayNameOrSlug(option)])), [options]);
  const tagNames = useMemo(() => new Map((options?.tags ?? []).map((option) => [option.slug, displayNameOrSlug(option)])), [options]);
  ...
  const galleryHoverTitle = (item: ImageItem) => item.title?.trim() || themeLabel(item.theme);
  const galleryHoverTags = (item: ImageItem) => item.tags.map((tag) => tagNames.get(tag) ?? tag).join(" · ");

...
export function adminImageListQuery(
  ...
  return {
    ...
    queryFn: async (context: { signal: AbortSignal; client: QueryClient }) => {
      ...
      const cached = context.client.getQueryData<CachedAdminImageListResponse>(queryKey);
      ...
      const result = await apiWithEtag<AdminImageListResponseDto>(
        ...
        (response) => {
          const candidate = Number(response.headers.get(adminImageListReadStartedAtHeader));

import type { ImportStatusItemDto } from "@imageshow/shared/browser";
import type { ImportJob } from "../../../lib/types.js";
import { completedImportJobPatch } from "./server-import-job.js";

const terminalClientStatuses = new Set<ImportJob["status"]>([
  "done",
  "cancelled"
]);
...
function clientStatusFor(
  status: Extract<ImportStatusItemDto, { status: "present" }>["item"]["status"]
): ImportJob["status"] {
  switch (status) {
...
export function importStatusEventPatch(
  ...
  if (state.status === "completed") {
    return {
      ...
      ...completedImportJobPatch(state.completed_item, state.display)
    };
  }

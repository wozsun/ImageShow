 ...
 * this lifecycle.
 */
export function useImportAuthorityHandoffs(input: Readonly<{
  jobs: readonly ImportJob[];
  jobsRef: Readonly<{ current: readonly ImportJob[] }>;
  ...
  const observeCompletedImportsRef = useRef(input.observeCompletedImports);
  observeCompletedImportsRef.current = input.observeCompletedImports;
  ...
  const clearFences = useCallback((pairKeys: ReadonlySet<string>) => {
    ...
    for (const job of jobsRefRef.current.current) {
      if (
        ...
        && pairKeys.has(serverImportJobPairKey(job))
      ) {
        patches.set(job.id, {
      ...
      try {
        for (
          ...
          observeCompletedImportsRef.current(
            completedImportObservations(statuses)
          );
  ...
  return {
    ...
    pending: fencesRef.current.size > 0 || input.jobs.some((job) => (
      ...
      && importJobAwaitsActionCoverage(job, input.server.revision)
    ))
  };
}

...
const draftSyncMutationAttempts = 8;

function draftSyncTarget(job: ImportJob): DraftSyncTarget | null {
  if (
...
function matchesDraftTarget(job: ImportJob, target: DraftSyncTarget) {
...
function completedDraft(
  item: Extract<ImportStatusItemDto, { status: "completed" }>["completed_item"]
): ImageDraft {
  return {
    device: item.device,
...
function authoritativeDraftFromStatus(
  status: ImportStatusItemDto | undefined
) {
  if (status?.status === "present") {
    return importJobFromServerItem(status.item).draft;
  }
...
async function updateStoredImportsWithResponseRetry(
  items: Parameters<typeof updateStoredImports>[0],
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    ...
    try {
      return {
        response: await updateStoredImports(items),
...
export function useStoredImportDraftSync({
  jobsRef,
  dispatch,
  server,
  reportError,
  ...
  dispatch: (action: ImportQueueAction) => boolean;
  ...
  const ensureSnapshotRef = useRef<(job: ImportJob) => void>(() => undefined);
  ...
  const observeCompletedImportsRef = useRef(observeCompletedImports);
  observeCompletedImportsRef.current = observeCompletedImports;
  ...
  executeBatchRef.current = async (requestedIds: readonly string[]) => {
    ...
    for (
      ...
      chunkOffset += importBatchHardLimit
    ) {
      ...
      for (
        ...
        for (const id of chunkIds) {
          ...
          if (current && !currentTarget) {
            ...
            const serverItem = serverRef.current.items.find((item) => (
              serverImportPairKey(item) === pair
            ));
            if (serverItem) {
              retireSync(
                id,
                sync,
                importJobFromServerItem(serverItem).draft
              );
              outcomes.set(id, false);
              failureMessages.add(
                "导入任务已冻结提交或完成，本地草稿未写入"
              );
              continue;
            }
        ...
        if (frozenEntries.length) {
          try {
            const statuses: Awaited<ReturnType<typeof getImportStatuses>> = [];
            for (
              let offset = 0;
              ...
              const response = await getImportStatuses(statusChunk.map((entry) => ({
                session_id: entry.target.sessionId,
                image_id: entry.target.imageId
              })));
              observeCompletedImportsRef.current(
                completedImportObservations(response)
              );
        ...
        const request = updateStoredImportsWithResponseRetry(
          entries.map(({ target }) => ({
        ...
        if (versionConflicts.length) {
          try {
            ...
            for (
              ...
              const response = await getImportStatuses(
  ...
  ensureSnapshotRef.current = (job: ImportJob) => {
  ...
  const flushPendingUpdates = useCallback(async () => {
    ...
    if ([...results.values()].some((result) => !result)) {
      throw new Error("部分导入草稿未能同步，请刷新后重试");
    }
  }, []);
  ...
  const retryPendingUpdates = useCallback(async () => {
    ...
    try {
      ...
      for (
        ...
        const response = await getImportStatuses(chunk.map(([, sync]) => ({
          session_id: sync.target.sessionId,
          image_id: sync.target.imageId
        })));
        observeCompletedImportsRef.current(
      ...
      if (terminalFailures) {
        reportErrorRef.current(
          `${terminalFailures} 个导入任务已完成、丢失或冻结提交，无法再写入保留草稿`,
          false
        );
      }
  ...
  const updateDuplicateDecision = useCallback((
    ...
    const promise = (async () => {
      ...
      try {
        const update = await updateStoredImportsWithResponseRetry([{
      ...
      dispatch({
        ...
        patch: {
          ...
          message: result.duplicate_decision === "confirmed"
            ? "已确认提交副本"
            : importDuplicateMessage(result.duplicate_count)
        }
      });

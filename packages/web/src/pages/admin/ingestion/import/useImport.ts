...
import { deduplicateImportJobsByDownloadUrl } from "../queue/model/ingestion-job-deduplication.js";
import {
...
export function useImport(options: {
  ...
  const cancellationAcceptOutcomes = useRef(
    new Map<
      string,
      {
        attemptKey: string;
  ...
  const submitImportBatch = useCallback(
    (jobs: IngestionJob[]) => {
          ...
          if (mounted.current && (error as Error).name !== "AbortError") {
            ...
            for (const job of jobs) {
              ...
              if (
                ...
                (!job.importAcceptItemInput || job.importAcceptRejected)
              ) {
    ...
    async (jobs: IngestionJob[]) => {
      ...
      void (async () => {
        for (let offset = 0; mounted.current && offset < acceptedJobs.length; offset += maxItems) {
          const chunk = acceptedJobs
            .slice(offset, offset + maxItems)
            .filter((job) =>
              queue.jobsRef.current.some(
                (current) =>
                  current.id === job.id &&
                  current.attemptKey === job.attemptKey &&

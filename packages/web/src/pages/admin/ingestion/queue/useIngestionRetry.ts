...
} from "./model/ingestion-job-retry.js";
import type { IngestionQueueController } from "./useIngestionQueue.js";

export function useIngestionRetry({
  queue,
  retryBrowserJobs,
  commitJobs
}: {
    ...
    async (target: IngestionJob) => {
      ...
      try {
        ...
        if (
          ...
          latest?.serverVersion === current.serverVersion
        ) {
  ...
  const isRetryPending = useCallback(
    (job: IngestionJob) => pending.has(`${job.id}\0${job.attemptKey}`),
    [pending]
  );
  return {
    retry,
    retryAll,
    canRetryAll,
    retryBusy: pending.size > 0 || retryingAll,
    retryingAll,
    isRetryPending
  };
}

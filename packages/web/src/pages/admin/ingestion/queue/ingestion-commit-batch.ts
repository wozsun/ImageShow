...
async function reconcileUnknownBatch(
  ...
  try {
    const states = await getIngestionStatuses(
      selected.map((job) => ({
        session_id: job.sessionId!,
...
export async function commitSelectedIngestions(options: CommitSelectedIngestionsOptions) {
  let accepted = 0;
  const completed: CompletedIngestionObservation[] = [];
  try {
    for (let offset = 0; offset < options.selected.length; offset += ingestionBatchHardLimit) {

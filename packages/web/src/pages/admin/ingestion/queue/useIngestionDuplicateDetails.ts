...
type DuplicateDetailsItem = IngestionDuplicateDetailsResultDto["items"][number];
...
function selectCachedDuplicateDetails(
  ...
  md5s: readonly string[]
): IngestionDuplicateDetailsResultDto {
  return {
...
export function useIngestionDuplicateDetails({
  jobs,
  updateJobs,
  ...
  const [data, setData] = useState<IngestionDuplicateDetailsResultDto | null>(null);
  ...
  const queueMissingDetails = (query: DuplicateDetailsQuery) => {
    ...
    const activeMd5s =
      activeRequest?.revision === query.revision
    ...
    if (trailingRequest?.revision === query.revision

...
import { createUploadIntents } from "../queue/ingestion-http-client.js";
import {
...
export function useUpload(options: {
  ...
  const { queue, defaults, storageSlug, maxItems, maxBytes, maxLongEdge, browserConcurrency } =
    options;
  ...
  const cancellationIntentOutcomes = useRef(new Map<string, UploadCancellationOutcome>());
  ...
  const transferUploadBatch = useCallback(
    async (jobs: IngestionJob[]) => {
      ...
      const intentConnectionGeneration = queue.captureServerConnectionGeneration();
      try {
        results = (
          await browserLane.current!.run(controller.signal, () =>
  ...
  const transferUploadJobs = useCallback(
    async (jobs: IngestionJob[]) => {
  ...
  const cancelMany = useCallback(
    async (jobs: readonly IngestionJob[]) => {
      ...
      await browserBatchSequencer.current!.run(async () => {
        ...
        for (const target of targets) {
          ...
          if (
            ...
            ingestionJobRetryKind(current) !== "browser-prepare"
          )
            continue;

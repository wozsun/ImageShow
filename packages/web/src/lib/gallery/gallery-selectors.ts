...
export class GallerySelectorError extends Error {
  constructor(
    readonly field: GallerySelectorField,
    message: string
  ) {
    super(message);
...
export function gallerySelectorValue(field: GallerySelectorField, values: readonly string[]) {
  ...
  if (
    tokens.some((value) => value.startsWith("!")) &&
    tokens.some((value) => !value.startsWith("!"))
  ) {
  ...
  if (tokens.length > randomQueryLimits.maxSelectorsPerField) {
    throw new GallerySelectorError(
      field,
      `${label}最多选择 ${randomQueryLimits.maxSelectorsPerField} 项`
    );
  }

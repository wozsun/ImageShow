...
接入摘要的状态桶由 Redis 原子增量维护。切换统计口径前，须在旧版本停止新增接入，让所有
管理员的 Upload / Import 队列完成提交或按需取消，确认未完成数均为零，再关闭旧后台窗口并
升级。不能仅凭“等待中”为零判断已排空；待处理、待确认、待提交及失败项仍是未完成任务。
这保留已提交图片和 completed 回执，不需要清空数据库或整个 Redis。回退旧口径也遵循同一要求。

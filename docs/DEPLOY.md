...
非空库执行下述 6.4.9 限定升级后做只读就绪检查；其他结构与数据维护由维护者在停机维护窗口人工处理。
...
3. 数据库必须满足当前运行时所需的最小结构；6.4.9 按下节自动完成限定升级，其他维护先人工处理。
...
本版在停写、无残留上传 / 导入及后台任务的维护窗口升级；不支持新旧实例混跑。
停止旧实例并取得可恢复的 PostgreSQL、Redis、配置及对象备份后，启动本版镜像即可自动
...
本次入口只保留到下一版。回退必须恢复一致备份或先完成另行核验的逆向维护，不能直接启动
...
隔离恢复方案。除上述 6.4.9 限定升级外，应用启动只读核对非空数据库的最小契约，不会自动补表、回填或对齐完整 schema；

...
非空库除下述 6.4.4 自动升级外，只做只读就绪检查；其他结构与数据维护由维护者在升级前人工处理。
...
6.4.4 启动时自动将 `metadata.purge_job_id` 表示的批量删除意图转入逐图 `background_job`，
校验后删除旧列及其 CHECK。无需手工执行迁移 SQL。完整转换与 readiness 在一个事务内完成；
已有图片、词表、管理员和其他任务不被删除，普通回收站图片不会被新建删除任务。
升级必须先停止旧应用并等待排空，备份 PostgreSQL 及正式存储，保留旧镜像；首次转换要求
连接角色有 metadata 的 ALTER 权限与创建事务临时表的权限。转换期间不得启动旧版本应用。
有效 pending / failed 意图保留原错误、失败次数和退避；遗留 running 按一次僵尸恢复处理。
异常旧引用或校验失败时事务回滚且新应用拒绝启动，按日志定位，并用旧版本检查维护或备份修复后重试。
无旧列的启动跳过转换；后续重启不会重新创建任务。细节与代码位置见
[6.4.4 临时升级兼容](guide/database.md#644-临时升级兼容)。

转换成功后不可直接启动要求旧列的镜像。回退须先停止新应用，并恢复升级前的 PostgreSQL
和与其一致的正式存储备份；新版本运行后可能已经完成永久删除，仅恢复数据库不能恢复对象。
兼容代码计划在 **6.4.5 移除**，从旧结构升级到 6.4.5 或更高版本前须先启动一次 6.4.4 完成转换。

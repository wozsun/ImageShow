...
非空库只做只读就绪检查；结构与数据维护由维护者在升级前人工处理。
...
仍使用 `metadata.purge_job_id` 的旧数据库，必须先启动 **6.4.4** 完成逐图删除任务转换，
确认容器健康、图片数据和删除任务正常，再按上述停机与备份步骤升级到当前版本。
已完成转换的 6.4.4 数据库可直接升级到 6.4.5，无需额外结构变更。
当前版本只核对最小就绪契约，不执行旧删除意图转换；只读就绪检查不替代这项升级前置。
旧结构转换所需权限、事务回滚与数据保留细节见
[6.4.4 部署说明](https://github.com/wozsun/ImageShow/blob/v6.4.4/docs/DEPLOY.md#644-自动升级)。

回退到要求旧列的版本，须先停止应用，再恢复转换前的 PostgreSQL 和与其一致的正式存储备份。
新版本运行后可能已经完成永久删除，仅恢复数据库不能恢复对象。

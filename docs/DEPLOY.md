...
6.1.x 须先使用 6.2.0 镜像完成固定的
[6.2.0 升级说明](https://github.com/wozsun/ImageShow/blob/v6.2.0/docs/DEPLOY.md#620-无主题升级)，
再进入当前版本。已完成迁移的 6.2.0 / 6.2.1 数据库升级至 6.2.2 无需新增 DDL。
全新部署直接使用空数据库初始化。

1. 拉取目标镜像，保留旧镜像与恢复方案。完成需要保留的上传 / 导入，关闭旧后台窗口。
2. 停止唯一应用并等待排空，备份 PostgreSQL、Redis 和 `data/`，保留正式存储对象。
3. 按目标版本要求人工处理数据库结构；从 6.2.0 切换时，须让接入任务与回执结束并到期，
   或在接受丢弃未完成内容时采用下面的 Redis 清理路径。
4. 使用同一 Compose 配置启动新版本，确认容器 `healthy`、图片投影就绪及图片数量和对象键正确。
   重新打开后台并登录，确认图片访问和新接入正常后恢复使用。

**可选 Redis 清理：**在第 2 步停机、备份后，核对实际连接的实例和 `REDIS_DB`，确认该逻辑库
仅供当前应用使用，再由维护者执行 `redis-cli -h <host> -p <port> -n <db> FLUSHDB SYNC`。
它可替代等待接入状态到期，但会丢弃未完成任务、回执、会话及缓存；启动后等待重建并重新登录。
不要在运行中或共享逻辑库中清理，不使用 `FLUSHALL`，不清理 PostgreSQL 或正式图片对象。
清理 Redis 不能替代所需的数据库结构迁移。

空主题输入为 JSON `null`，筛选使用 `theme=~unset` 或 `theme=!~unset`；更新保存的请求与书签。
公开页面或 API 变化时清理受影响的 CDN 缓存。升级失败按备份与旧镜像恢复，恢复演练使用独立环境。

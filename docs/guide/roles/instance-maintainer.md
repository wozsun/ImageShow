...
- 按当前数据库契约人工维护结构与数据，确认停机范围、备份及隔离恢复路径。
...
- 普通停机使用 `docker compose stop`，不清空 `data/`、数据库或存储桶。
- 同一数据库只运行一个应用实例，不提供多实例协调或自动接管。
- 存储对象删除和数据库破坏性结构变更必须先明确范围、停机条件、备份与恢复方案。
- 非空数据库启动时只读核对当前 readiness；应用未消费的额外结构不影响启动，缺失或类型
  不兼容的必需结构会明确拒绝。

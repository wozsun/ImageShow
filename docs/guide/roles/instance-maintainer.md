...
- 部署 5.6.0 后安排停止新图接入的单操作者维护窗口，协助超级管理员完成并保存布局升级证据；
  只有全部旧 `media` 状态归零后才安排 5.6.1。
...
- 启动路径不自动执行版本专用数据迁移；5.6.0 的图片布局升级必须由超级管理员在检查页显式
  运行。它不增加 schema ledger 或持久迁移任务，断线后从 PostgreSQL 当前剩余旧键继续。
  非空数据库仍必须已满足当前 readiness，额外的无消费者结构不影响启动，缺失或类型不兼容的
  必需结构会明确拒绝。

...
强烈建议显式设置 `site.domain`。未设置时使用默认值 `example.com`；域名为空或为 `example.com`
时，应用接受格式合法的访问 Host，资源统一使用 `/static` 同源路径，随访问地址的协议、Host
和端口解析。此基础回退不推测根域名、不持久化请求 Host，也不启用 `static_subdomain`。
以下独立资源子域与未知 Host 限制适用于已配置实际域名的站点。
...
显式域名需要使用合法 DNS 域名，不接受 IP 或单标签 `localhost`；基础回退可通过 DNS Host、
`localhost` 或 IPv4 地址访问，支持端口。基础回退不保证所有代理或特殊主机格式可用。

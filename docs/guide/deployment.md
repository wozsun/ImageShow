...
revision、meta、最后内容更新时间与核心完整性，才能开放缓存读取。v4.1 保留已发布的
`0002_ready_image_revision.sql`：全新安装在 `0001_initial.sql` 后执行它，已升级数据库
保留既有迁移记录且不会重放，尚未执行的旧数据库仍可由 migration runner 正常补齐。
v4.1 不需要管理员凭据 revision 表或相关迁移。应用不在迁移文件之外猜测旧 schema。

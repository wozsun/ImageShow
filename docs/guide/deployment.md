...
当前 `v4.9.5` 的 additions 没有待执行 SQL，只保留注释占位；全部受控生产数据库已经确认
...
后台轻量状态使用固定命令读取连接、revision、图片数量、最后更新时间和完整重建时间，
不会扫描键空间或执行 `MEMORY USAGE`。管理员显式运行 Redis 深检时，才以 10 秒总扫描
期限、最多 100,000 个唯一键和每批最多 128 条测量命令扫描 `imageshow:*`，汇总当前核心 /
派生投影的键数、成员数和内存。超过期限或键上限会返回 `complete: false` 与明确原因，
不得把部分汇总当成总量；浏览器断开会通过请求 `AbortSignal` 停止继续安排扫描或测量批次。
`INFO MEMORY` 卡片仍只描述整个 Redis 实例。

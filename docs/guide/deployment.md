...
当前生产边界是一台主机上的一个 ImageShow 应用容器。PostgreSQL 与 Redis 可以来自
独立的基础设施 Compose，但各自仍是单机单实例。升级时先停止当前应用容器，等待有界
排空完成，再原位启动新容器；不并行运行连接同一数据库的两个应用实例。

应用会在业务启动前取得 PostgreSQL 生命周期 advisory lock。第二个连接同一数据库的
ImageShow 进程会明确退出；锁连接意外断开时，当前进程会停止接收请求、停止 Worker，
排空存储访问并以失败状态退出。这是误部署保护，不是多实例 fencing 或零停机切换协议。
多实例需要补齐的能力见[多应用实例演进待办](./todo-multi-instance.md)。

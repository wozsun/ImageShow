...
复合唯一索引已经并入该基线。5.4.2 的 `schema-additions.sql` 新增可空
`metadata.purge_job_id` 与长期状态 CHECK。空数据库依次执行两者，
非空数据库执行 additions 后，在同一事务运行 5.4.2 一次性 purge 升级，再做只读 readiness；
整个结构过程受同一事务保护。当前启动不再读取
旧作者映射或补齐已有链接；早于 `5.4.0` 的非空部署必须先运行 `5.4.0` 完成一次性迁移，再升级到
`5.4.1`，随后在旧应用进程完全停止并完成备份后运行 `5.4.2`。该版本把可解释的旧删除意图绑定到
retained 替代任务，并物理删除旧四列、旧 CHECK 与旧索引；畸形旧任务或部分旧结构会让事务回滚。
...
限定为干净初始化、单周期 additions 和最小 readiness；5.4.2 的上述相邻版本清理是经明确审阅的
一次性例外，其他破坏性 DDL 与数据整理由维护者另行执行。

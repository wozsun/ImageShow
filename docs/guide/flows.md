...
最终图片完成 `load` 与 `decode()` 后立即用 160ms 渐显；80ms 后导航和首页筛选用
120ms 淡入，再到背景就绪后 160ms 启动 Hero。Hero 的文案、统计卡片与浏览提示分别
用 0/30/60ms 错峰和最长 120ms 的轻微淡入，整组在 180ms 内完成；背景通常在约
250ms 内就绪时，整个首屏会在约 590ms 收口。302 仍属于背景请求耗时，不作为就绪
信号，也不用额外的 fetch、HEAD 或随机图探测。

若背景到 600ms 仍未就绪，则先在现有渐变底色上释放导航，80ms 后释放 Hero；请求
尚未实际开始时由 1000ms 全局期限执行同一兜底。页面从后台恢复时会按绝对期限补齐
被节流的入场阶段。当前背景触发 `error` 后即视为终态，立即显示全部前景且不重试；
同一请求随后迟到的 `load` 或 `decode()` 不会再显示背景。
前景已经开始显示后才就绪的背景改用 360ms 柔和融入，不隐藏或重播任何内容。导航
与 Hero 在隐藏期间都保留最终布局，并使用 `aria-hidden` 与 `inert` 退出可访问树。
背景仍只使用 `site.home.background` 或一次 `/random?m=redirect` 请求。减少动态
效果时立即显示导航和 Hero，并关闭背景与前景的全部入场过渡。

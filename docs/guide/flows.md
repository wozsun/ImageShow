...
网络响应和滚动过程都不会从中段截断流光，连续筛选仍能可靠重触发。
...
元数据的浏览器使用，真实文档背景用于 Safari 26 的地址栏取色；只有顶层“加载中”
占位可见时使用独立纯黑全视口表面。
...
稳定；列数、间距或窗口宽度变化时从同一位置模型重新计算。分页哨兵只负责继续获取
元数据，不再通过只增不减的渲染计数保存历史 DOM。
...
- 图片列表、编辑、批量操作、回收站、存储迁移；图片管理员可以进行常规元数据编辑、
  ...
  删除、取消与保存操作。公开画廊从详情编辑删除图片后，会在详情关闭前刷新并补位
  列表，保持原页面滚动位置，并把焦点交给原位置的

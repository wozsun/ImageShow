...
展映与画廊共用同一个图片筛选工具栏，筛选控件和清空按钮只显示框内文案，不重复显示上方标题；
随机图片链接框内前部显示不可选的“随机API”，链接按剩余宽度截断。首次点击或键盘聚焦链接时
全选完整 URL，不包含标题和省略号；保持焦点后的点击或拖动沿用局部选择，移开焦点再进入时重新全选。
只读链接通过鼠标或触控聚焦后，仅在存在实际文字选区时以该焦点保护导航；取消选区后即使焦点仍在链接上，
也恢复滚动收起，并在其他保护条件解除且展映自动播放时重新计满三秒。键盘可见焦点和桌面导航悬停继续保护导航。
复制按钮仍使用完整 URL。
“清空筛选”只把设备、亮度、主题、标签和作者五项一次
写回缺省路由状态；列表
...
同一个页面实现和清空语义；`/show` 与 `/embed/show` 复用完整 toolbar 外壳、候选查询、菜单 dismiss 和响应式断点。
...
导航菜单、筛选子菜单、
移动筛选面板或图片详情展开期间暂停自动隐藏，关闭后仍在自动播放才重新计满三秒。
已展开的主导航或筛选工具栏内存在鼠标悬停或焦点时，取消空闲计时，并暂停滚动触发的收起。
主导航与筛选栏作为同一交互范围，内部移动鼠标或切换焦点不会恢复计时；只有鼠标与焦点都离开，
且展映仍在自动播放、菜单、面板和详情均未阻止隐藏时，才重新计满三秒。触控设备只以键盘可见焦点或输入框焦点
保护导航，忽略触摸残留的悬停及普通按钮焦点；移动筛选关闭并归还按钮焦点后，展映和画廊恢复滚动收起，展映按播放状态恢复计时。
首页主导航同样暂停滚动收起，
嵌入展映和嵌入画廊的筛选栏复用相同规则；导航以外的鼠标移动仍不重置计时。
两种展映模式的左右底部按钮共用导航显隐状态：导航完全收起时按钮不透明度降至 30%，底部操作
提示使用偏白的浅灰蓝文字与较深的灰色描边，文字与描边保持显示并整体降至 20% 不透明度；任一导航栏出现、鼠标进入任一侧按钮组及其周围 24px 范围，或按钮获得键盘可见焦点时，
两侧按钮与提示一起恢复到 100% 不透明度。鼠标离开该范围且导航仍收起后恢复淡化，过渡为 180ms；系统减少
动态效果时直接切换。按钮保留点击能力。窄屏与粗指针设备同样显示提示，waterfall 为“拖动平移；点按 ± 或双指缩放”，
float 为“上下拖动；点按 ± 调整尺寸”，透明度规则与桌面一致。
模式切换按钮的提示、读屏标签和状态播报将 `waterfall / float` 显示为“瀑布 / 漂浮”；按钮提示同时说明当前模式与点击后的目标模式。
...
改写地址栏。右下角模式控件使用真实链接，手动切换始终显式写入目标 `mode`，包括切回配置的
...
修改配置后重新进入或刷新页面采用新值。减少动态效果、详情弹窗和页面后台状态继续按原规则暂停自动运动。
...
详情在原图按钮开启时沿用列表缓存，关闭时使用 `private, no-cache`，并始终带 `Vary: Cookie`。
关闭状态的详情每次 HTTP 读取都重验证。列表普通读取使用浏览器默认缓存；确认编辑优先，缺少权威公开数据的活动查询
才显式使用 `no-cache`。
...
画廊右下角按随机 → 最新 → 最旧循环切换，保留筛选并从首批开始；排列按钮固定在下，返回顶部出现时位于其上。
...
画廊、嵌入页与展映共用圆形按钮，导航显示 / 隐藏时透明度分别为 90% / 30%，悬停或键盘聚焦为 100%。
...
image_time、description、source、object_url 与 original_url。同图按 ID 组装，有效详情覆盖同名
列表元数据，已确认编辑快照优先。本地会话提示只触发
`/auth/me` 探测；服务端确认已认证后才按需加载管理信息与编辑能力。未登录、过期提示、
401 与 403 路径都不会下载管理 JS 或 CSS。详情标题固定在新标签页打开该图片的直链，
不提供运行时开关或条件分支。

站内画廊与展映的页面批次不携带原图差异布尔值，也不由 Web 比较原始地址和展示地址。详情读取
只有在服务端确认存在另一条合法 HTTPS 原图时才返回非空 `original_url`，否则返回 `null`；加载中、
失败或空链接时不渲染“原图”按钮及禁用占位。“来源”仍固定在动作区左侧并始终渲染，详情尚未完成
或无来源时沿用现有不可用状态与提示；动作区不换行，使右侧“原图”迟到出现时不移动来源按钮。
公开详情、后台图片读取和编辑快照的 `source` 与 `original_url` 均使用 `string | null`，无链接
明确返回 `null`。数据库与编辑 / 接入草稿仍以空串表示未填写来源；读取投影和回填草稿各自完成
...
`site.gallery.public_original_button` 控制未登录访客详情是否返回原图链接，默认关闭。
关闭时详情复用现有 Redis 会话与 PostgreSQL 账号校验，对访客返回 `original_url: null`，
对已登录管理员返回 `/images/original/<id>`；开启时不读会话，访客同样取得该链接。
所有链接都要求 `original` 是与展示图不同的合法 HTTPS URL。后台读取、编辑快照及回收站也
使用同一个公开链接；资源出口不读取会话或按钮开关，直接拼接有效 ID 可以访问。
详情在关闭按钮时为 `private, no-cache`，开启时为 `public, max-age=30, s-maxage=60`，始终带
`Vary: Cookie`；内容 ETag 在链接显示投影后生成，共享查询只复用数据库行。Web 的详情查询按图片 ID
和认证身份隔离，管理员带同源凭据，访客不带凭据；等待已有首次认证探针完成后再读取，不增加
认证请求。编辑和移入回收站按图片 ID 取消或失效各身份的对应详情，迟到私有结果不接管访客界面。
Web 根据服务端返回的非空链接显示入口；加载中、失败或无链接时不渲染按钮或
可聚焦占位，不影响展示图的原生保存。动作 DOM、键盘焦点与视觉顺序仍为“来源”在左、“原图”在右。
唯一原图出口在一次图片解析后完成直连决策或受限代理，直连结果按原图 URL 与 User-Agent 家族缓存。
直连 302 使用公开短缓存，代理继承已校验源站策略或使用 CDN fallback；GET、HEAD、条件请求、
取消及 URL 绑定验证器保持有效，响应带 `Vary: User-Agent`。原图资源可读取正常图片及回收站记录，
公开列表和详情仍只展示 ready 图片；正式展示图和缩略图继续使用稳定公开地址。
...
记录为未采用时直接校验原图并生成，再统一复核当前位置与缩略图。存储对象处理完成后，同一

...
浅色底。图片的有效构图区只为不会消失的首页筛选栏保留顶部空间：桌面 72px、移动端
64px；主导航会随滚动收起，不计入保留高度。主导航收起后，图片顶部正好贴合筛选栏
下缘。剩余视口继续 `object-fit: cover`，发生纵向裁切时以顶部为锚，避免人物照片的
头部被长期固定的筛选栏遮住或优先裁掉。完整视口的深色舞台、遮罩和环境光不内缩，
导航背后仍保持连续背景。承担 72px / 64px 内缩与裁切的外框保持固定；内部图片
使用顶部变换原点，只在入场时从 `scale(1.05)` 收敛到 `scale(1)`。加载完成后不再
缩放或漂移，也不保留持续图片动画。
...
落在首页还是画廊；首页与画廊互相切换时新页面直接保持主导航最终状态。从画廊通过
“首页”或根路径指向首页时的站点品牌进入首页，首页专属筛选栏只执行一次 `160ms`
的纯透明度渐入，不改变位置，也不引入主次导航重叠帧；首次直接进入或刷新首页时，
二者仍由同一个导航阶段一起出现，
不叠加该渐入。画廊属性栏仍单独淡入，后续滚动显隐由既有单一位移栈独占
`transform`。公开移动导航按钮使用独立的 6% 黑色半透明表面；桌面悬浮色与后台
移动导航不受影响。后台导航的暗色当前项以 3.14.3 蓝色为基准小幅提亮，
与既有的更暗悬浮态维持明确层级；当前项与悬浮项共用高对比浅蓝文字。

...
首页与画廊各自只有一个固定在视口顶部的共享导航框架；主导航与首页筛选栏或画廊
工具栏都在框架内相对定位，不再作为两个独立 fixed 图层交给浏览器合成。页面仍以
对应的流内占位元素保留实际高度，两个导航表面也只通过同一共享位移变量保持相邻。
公开路由在 `html` 上设置 `overscroll-behavior-y: none`，阻止根滚动容器的顶部与
底部橡皮筋位移；不影响后台或筛选面板、详情等内部滚动容器。该属性不被旧浏览器
识别时会按 CSS 规则自然忽略，固定框架和下述有界采样仍继续工作，不使用 UA、设备
型号、触摸事件接管或强制重绘。

首页和画廊的导航显隐共用一个有界页面滚动采样 Hook：
每个动画帧最多读取一次位置，弹窗锁定页面时暂停，并统一把 `scrollY` 归一化到文档
真实上下边界。首页主导航以 18px 累计方向距离独立显隐，筛选栏保持常驻；画廊则由
页面级三级状态机统一控制主导航和属性栏，只通过 transform 切换可见状态。向下滚动
超过属性栏自身起始区域后，累计 18px 先收起属性栏，再累计 48px 收起主导航；实际
向上 16px 先显示主导航，再实际向上 18px 显示属性栏。方向反转会清空上一方向的

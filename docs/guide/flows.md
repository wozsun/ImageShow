...
- 卡片区分等待、上传/下载、待处理、处理、已就绪、提交、完成、失败、取消；显示存储后端
  ...
  等待使用低强调中性灰表面；上传/下载和待处理使用最淡蓝色，处理和提交使用中间
  蓝色，已就绪保留最强蓝色。暗色分支的三档蓝色保持既有色值；亮色分支让最淡、处理
  中与已就绪三层形成均匀递进的蓝色辨识度。各档文字均须满足对比度底线，等待
  表面在暗色模式下不得提亮成近白灰。
...
5. 在 Redis generation 随机池中按 axis/category 计数加权选集合；标签和作者筛选通过
   Redis 临时过滤集合完成。合法增量更新期间有界等待 completed revision；Redis 不可用
   或等待超时时返回带 `Retry-After` 的 503。
6. `m=proxy` 从图片所属存储后端代理字节，`m=redirect` 返回 302；`m=json` 用一次
   PostgreSQL 查询补齐所选图片的宽高、时间和标签真值，再生成原图及缩略图 URL，
   返回带实际 `count` 的数组。
...
读取后台外观偏好。
图片列表按 `image_time DESC, id DESC` 使用 `/api/images`
稳定 keyset 游标分页与 Redis 缓存；游标编码边界项的 `(image_time, id)`，下一页
使用严格小于该元组的条件，因此边界项在请求间被删除也不会改变游标含义。响应只包含
卡片数组 `items` 和继续分页所需的
...
受保护接口返回 401，则清除 hint 并保持普通访客展示。列表、详情、facets、后台概览、
...
稳定。当前 `imageQuery` 的会话保存位置数组、列号、列尾高度和总高度作为已提交真值：

...
进入后台时，路由和认证投影等待阶段继续保留启动暗色，`AdminShell` 及后台色板就绪
后才一次性接管已保存的后台外观（缺少偏好时默认自动）。后台可明确选择亮色、暗色或自动；
...
更新瞬间重播。
只有后台路由会挂载这一监听：认证状态就绪但未登录或读取失败时使用默认自动模式，登录后
由完整 `AdminShell` 按保存偏好接管；认证等待阶段仍保持启动暗色。公开首页、画廊及从
画廊打开的详情、编辑和存储迁移二级页面始终由公开颜色域锁定为暗色。认证读取失败使用
已经就绪的后台色域承载重试界面，后台内部换页不会把颜色域重置回启动状态。前两项元数据
供仍读取它们的浏览器
...
- 登录页背景层固定占用当前动态视口并锁住外层文档与自身滚动；登录卡片作为独立
  ...
  效果时关闭过渡，不支持 Visual Viewport API 时回退到 `50dvh`。

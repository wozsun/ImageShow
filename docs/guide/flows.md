...
- 卡片区分等待、上传/下载、待处理、处理、已就绪、提交、完成、失败、取消；显示存储后端
  ...
  青绿色；检查成功卡在亮色分支精确沿用 3.12 的卡片表面与边界，暗色分支与
  上传 / 导入成功反馈共用成功表面和低强调边界，不与完成卡合并为同一强度。
...
导航背后仍保持连续背景。承担 72px / 64px 内缩与裁切的外框保持固定，持续漂移只
作用于内部图片并使用顶部变换原点；纵向位移固定为零，缩放时既不会改变安全区边界，
也不会在图片顶部拉出缝隙。
...
落在首页还是画廊；首页与画廊互相切换时新页面直接保持主导航最终状态。从画廊进入
首页时，首页专属筛选栏仍以一次短距离淡入明确页面层级；首次直接进入首页时它随完整
导航阶段出现，不叠加第二次动画。画廊属性栏仍单独淡入，后续滚动显隐由既有单一位移
栈独占 `transform`。
...
首页背景漂移、环境光与星点，以及画廊的低强度环境光，只持续修改 `transform` 和
`opacity`；文档隐藏或进入 pagehide 时，由原生生命周期事件同步写入根节点暂停标记，
不等待可能被后台标签页节流的 React 提交；恢复时从原位置继续，已完成的入场不重播。
“向下浏览与筛选”提示在入场后保持静止，只响应 hover 与 focus。
`prefers-reduced-motion: reduce` 会立即释放首页全部阶段、settle 目录和画廊卡片，
并关闭持续动画与错峰，但
不改变请求、图片清理、焦点或滚动语义。导航、次导航、目录波次、星轨流光、工具栏和

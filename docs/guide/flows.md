...
float 最多使用 500 张候选，候选总数少于 96 张时请求补充；waterfall 在可用候选少于 96 张或
布局缺图时请求补充，并保留总数小于 800 的条件。reduced-motion 下停在静态构图，不运行路径与速度调整。
...
展映的 `order=random` 使用带当前筛选的 `/random?mode=json&limit=200`；`latest` 与 `oldest` 使用
公开图片 cursor 列表。筛选或顺序切换时取消旧请求、冻结巡航并保留旧平面，首批新结果成功后再
...
编辑完成或快照刷新失败后从同一权威查询重新确认筛选成员。float 随机池增量更新保留原候选顺序，
仅新增候选参与洗牌，避免补图重排已显示卡片。

公开列表省略 `order` 时按 `image_time DESC, id DESC` 使用 keyset cursor；显式
`order=oldest` 时对称使用 `image_time ASC, id ASC` 与正向 cursor；
cursor 是客户端只能透传的 32 字符 Base64URL 值，服务端严格校验编码、时间与 UUID 边界。
公开列表查询是严格对象，只接受公开筛选、`cursor`、`limit`、`order=latest|oldest`
...
在左、“原图”在右，按钮直接请求当前资源根（默认主站 `/static`，可配置独立资源子域）下的
`/link/<id>`；服务端在一次图片解析中复用按原图 URL 与 User-Agent 家族缓存的

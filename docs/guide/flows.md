...
- 存储管理列表由服务端统一删除策略投影为删除、迁移或原因说明：只有当前数据库占用为空、非默认且非内置的后端显示删除，提交前还需二次确认；图片占用改为打开与检查页共用的整后端迁移弹窗，其他已知阻断项逐项说明。列表不为每张卡片远程枚举 `_uploads`，最终删除命令仍在独占位置锁内重新统计并扫描暂存命名空间，拒绝加载后的新引用与暂存对象竞态。

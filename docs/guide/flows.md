...
都会取消确认，提交后禁用并显示 `delete-bin-5-line` 与“删除中”，已知结果收口后恢复初始状态。
单图永久删除、删除已选图片和清空回收站统一使用范围确认弹窗；弹窗中的危险按钮先显示
“永久删除”，第一次点击后变为“确认删除”，两步都使用 `delete-bin-7-line`，第二次点击才提交。
三个永久删除入口只对具有 `image.trash.purge` 权限的超级管理员可见，服务端仍独立强制该权限。
提交后弹窗锁定，以 `delete-bin-5-line` 显示“删除中”，并在正常情况下等待确认范围完全删除，
再关闭并只刷新一次列表。连接中断、有限

...
SPA 首次 HTML 和客户端路由共用集中浏览器表面分支：当前公开页为暗色，后台为浅色，
同时更新 `theme-color`、`color-scheme` 与 `html` / `body` 背景。前两项供仍读取
元数据的浏览器使用，真实文档背景用于 Safari 26 的地址栏取色。顶层路由等待改用
与公开暗色表面一致的旋转加载指示；各次挂载都按同一个 720ms 墙钟相位设置负延迟，
因此路由等待交给首页或画廊等待面时，圆环不会从零度重新起转。首页组件接管后仍在
相同视口位置保留指示；背景完成 decode 并开始渐显时立即隐藏，若背景较慢则在 Hero
开始揭示时用 100ms 收口，因此脚本 / 配置等待与背景 decode / 800ms 截止等待之间
不会出现只有底色而没有反馈的空档，也不会在背景已经出现后继续悬浮。画廊首批列表
尚无图片时复用同一指示并固定在视口正中，后续游标页只在列表末端显示轻量文字状态。
减少动态效果时圆环保持静态，但文字状态仍保留。

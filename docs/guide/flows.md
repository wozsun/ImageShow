...
结果。图片投影区域的角色公共部分只呈现当前状态、PostgreSQL / Redis revision 指纹、
最近错误与刷新；手动重建、存储维护和整后端迁移在服务端确认超级管理员权限后才加载
独立能力模块。深度 Redis 结果复用同一紧凑投影状态，不再复制构建耗时、进度、时间戳、
占用和内部策略。存储维护弹窗使用当前只读检查结果预览缺图、可修复缩略图、孤儿对象和活动导入，

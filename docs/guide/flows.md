...
直接返回稳定的 `subtitle`：已设置主题使用主题显示名，标签使用显示名
...
公开“原图”按钮还受 `site.gallery.show_original_button` 控制；关闭时不渲染按钮或可聚焦占位，
不影响当前展示图的原生保存。开启后，只有 `original` 是另一个 HTTPS URL 时按钮才可用；
按钮直接请求 `static.` 资源域的
...
图片卡片的单图移入回收站仍使用同一个图标按钮内的两次点击确认：第一次进入确认态，第二次才
提交；失焦或点击按钮外会取消确认。单图永久删除、删除已选图片和清空回收站统一使用范围确认
弹窗；弹窗中的危险按钮先显示“永久删除”，第一次点击后变为“确认删除”，第二次点击才提交。
三个永久删除入口只对具有 `image.trash.purge` 权限的超级管理员可见，服务端仍独立强制该权限。
提交后弹窗锁定并在正常情况下等待确认范围完全删除，再关闭并只刷新一次列表。连接中断、有限

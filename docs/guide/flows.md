...
WebKit 一定把延迟 click 派发给原触点元素。指针收起面板后，共享按钮释放仍挂载的开关焦点；
键盘、辅助技术及程序化 `detail=0` 激活继续把焦点留在披露按钮上。聚焦目标、保留编辑器焦点与
激活后释放是同一组件的三种明确指针焦点策略，不由工作流页面补 `blur` 或定时器。折叠面板
自己的捕获阶段 `touchstart` 只负责外部触控关闭，不建立页面入口特判、额外延时器或第二套
展开状态。

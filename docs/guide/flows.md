...
结果。图片投影区域只呈现当前状态、PostgreSQL / Redis revision 指纹、最近错误，以及
刷新和手动重建操作；深度 Redis 结果复用同一紧凑投影状态，不再复制构建耗时、进度、
时间戳、占用和内部策略。存储维护弹窗使用当前只读检查结果预览缺图、可修复缩略图、孤儿对象和活动导入，
...
- 公共与管理 DTO 只包含一个缩略图地址。配置公开 URL 的 S3 直接指向最终对象；local 与
  无公开 URL 的 S3 使用应用 `static.` 入口，回收站使用鉴权管理入口。

...
与 `opacity`，稳定线右端自然衰减，不保留固定高亮圆点。
流光只使用一套关键帧，位移从轨道左缘外到自身 18% 宽度对应的右缘终点，全程使用
单一先慢、居中加速、末端减速的曲线；透明度仅在首尾渐变。标题内部裁切两端，使进入
与淡出的全过程都不越过标题轨道，也不会在中右段切换速度或停顿。
...
与公开暗色表面一致的旋转加载指示；首页组件接管后仍在相同视口位置保留同一指示，
直到 Hero 开始揭示才淡出，因此脚本 / 配置等待与背景 decode / 800ms 截止等待之间
不会出现只有底色而没有反馈的空档。减少动态效果时圆环保持静态，但文字状态仍保留。

...
首页与画廊的共享主导航、首页筛选栏和画廊工具栏均固定在视口；页面以对应的流内
占位元素保留实际高度。因此 iOS Safari 的顶部弹性滚动不会再把任一导航连同文档
向下拖动并露出页面背景。首页主导航仍以 12px 累计方向距离独立显隐；画廊则由页面
级三级状态机统一控制主导航和属性栏，只通过 transform 切换可见状态。向下滚动超过
属性栏自身起始区域后，累计 18px 先收起属性栏，再累计 48px 收起主导航；实际向上
16px 先显示主导航，再实际向上 18px 显示属性栏。方向反转会清空上一方向的累计值，
到达页面顶部或展开任一导航菜单会立即恢复完整导航。已展开的移动导航跨回桌面断点
时会主动关闭并释放完整导航锁定，避免已被 CSS 隐藏的菜单继续阻止滚动收起。

画廊判断滚动手势前会把 `scrollY` 归一化到文档真实上下边界，并为底部保留 4px
容差；Safari 触底后的弹性超滚和回弹始终映射到同一个底部位置，不会伪造向上位移
而弹出菜单。只有离开底部容差后的真实向上滚动才重新累计 16px / 18px 阈值。

...
- 卡片区分等待、上传/下载、待处理、处理、已就绪、提交、完成、失败、取消；显示存储后端
  ...
  等待、上传/下载和待处理共用最淡的蓝色层级，处理和提交使用中间层级，已就绪保留
  最强蓝色层级；亮色分支恢复 3.12 的已就绪与处理中蓝色，暗色分支保留原已就绪
  基准，并让另外两档沿用同一蓝相；处理中与等待中的卡片层分别降至约 75% 与 50%
  亮度，文字仍须满足对比度底线，避免降饱和后偏灰青。三档都必须保持可辨识的蓝色。
...
环境层仍使用带 `100vh` 回退的 `100lvh` 大视口画布，并允许越过外壳覆盖完整底边。
地址栏展开或收起时，内部画布只改变可见裁切，不重新计算 `object-fit: cover`
缩放；横竖屏切换等真实视口变化仍会重新适配。
背景容器从首帧就是深色舞台，并以显式层级依次承载图片、永久深色可读性遮罩、
环境光与星点纹理；横向遮罩由左至右使用 50% / 20% / 12% 不透明度，纵向补充遮罩
由顶部 6% 过渡到底部 28%。图片只在遮罩下方渐显，错误或尚未 decode 时也不会露出
浅色底。图片的有效构图区通常只为不会消失的首页筛选栏保留顶部空间：嵌入桌面为
72px，移动端为 64px；主导航会随滚动收起，不计入构图安全区。非嵌入桌面首页例外地
...
照片的头部被固定筛选栏遮住或优先裁掉。完整视口的深色舞台、遮罩和环境光不内缩，
...
最终图片完成 `load` 与 `decode()` 后用 1100ms 在永久遮罩下渐显并收束缩放；背景
就绪后 100ms 释放主导航，250ms 释放 Hero，800ms 允许目录开始按视口揭示。Hero
内部的 label、标题、统计与浏览提示只用 CSS 做有界错峰，最长标题动画为
780ms；统计面板与首项分别在 Hero 释放后约 180ms、260ms 开始，整组约在
1.2～1.6 秒内收口。302 仍属于背景请求耗时，不作为就绪信号，也不用额外的 fetch、
HEAD 或随机图探测。

若背景到 800ms 仍未就绪，则先在现有渐变底色上释放导航，80ms 后释放 Hero，并在
...
实际显现时以 `30ms` 等待、`180ms` 时长执行一次整栏纯透明度渐入；首次直接进入时
主导航使用与画廊相同的独立透明度渐入，主次导航的固定外壳始终保持最终透明度，
等待阶段只隐藏两个导航表面。显现状态与两条动画在同一次 React 提交中切换，避免先把
整个半透明外壳瞬间显示、再分别淡入内部元素而产生跳变或交界接缝；主导航已经出现
...
外观按钮以两次点击为一组：先切到当前实际明暗的相反模式，再回到自动模式；点击提交
时直接交接当前与目标图标，悬浮动画不会在偏好更新瞬间重播。
...
等距行列；冷色微光分布在左上、右上和下缘，不把视觉重心压在单一角落。装饰层的绘制
...
首页背景图片完成入场后保持静止；环境光与星点以及画廊的低强度环境光继续只修改
`transform` 和 `opacity`。文档隐藏或进入 pagehide 时，由原生
生命周期事件同步写入根节点暂停标记，

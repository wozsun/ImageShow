...
副标题、文案、分页、
批量默认值和移出按钮，快照、草稿、保存、移入回收站、迁移、权威回读与卡片结果使用同一实现。
...
查询，不重放 mutation。编辑窗口内的 trash 同样提交当前活动的 `1..N` 个成员；部分成功时
只移除已确认移入回收站的成员，仍可编辑或结果未知的成员留在原会话等待重试或刷新。
...
移入回收站不是访问撤销：既有直链仍可访问。彻底删除会删除源对象，使未命中缓存的新
源站请求无法回源，但不能召回浏览器或 CDN 已缓存的副本；需要立即吊销缓存时还必须在
应用之外清理相应 CDN，并理解客户端已保存副本无法由服务端收回。

...
真实上下边界。首页和画廊的主导航共用 18px 累计方向距离及同一动画时长；首页筛选栏
保持常驻，画廊则由页面级三级状态机统一控制主导航和属性栏，只通过 transform 切换
可见状态。向下滚动超过属性栏自身起始区域后，累计 18px 先收起主导航，再累计 48px
收起属性栏；实际向上 16px 先显示属性栏，再实际向上 18px 显示主导航。方向反转会
清空上一方向的
累计值，到达页面顶部或展开任一导航菜单会立即恢复完整导航。已展开的移动导航跨回
桌面断点时会主动关闭并释放完整导航锁定，避免已被 CSS 隐藏的菜单继续阻止滚动收起。
...
筛选浮层的关闭判断；导航显隐使用上文的 18px 向下与 16px / 18px

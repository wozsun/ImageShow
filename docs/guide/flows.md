...
`opacity`；文档隐藏时统一暂停，恢复时从当前位置继续，已完成的入场不重播。“向下
浏览与筛选”提示在入场后保持静止，只响应 hover 与 focus。`prefers-reduced-motion:
reduce` 会立即释放首页全部阶段、settle 目录和画廊卡片，并关闭持续动画与错峰，但
...
- 图片列表、编辑、批量操作、回收站、存储迁移；图片管理员可以进行常规元数据编辑、
  ...
  存储清理等其他持久化删除入口继续先展示说明影响范围的确认弹窗；重新排队已耗尽的
  旧对象删除任务也会先说明任务数量和后续删除行为。上传工作区的“移除”和
  “清空已完成”只清理本地任务队列，不删除已经入库的图片；

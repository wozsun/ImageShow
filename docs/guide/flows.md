...
内部的 label、标题、tagline、统计与浏览提示只用 CSS 做有界错峰，最长标题动画为
...
后才一次性接管后台浅色；认证读取失败也使用已经就绪的后台色域承载重试界面，后台
内部换页不会把颜色域重置回启动状态。前两项元数据供
...
分割线、按钮和信息面板，后台详情仍使用原浅色边界。编辑入口只用于 ready 图片，
...
- 标签、主题、作者、用户、设置、检查与账户设置；图片管理员可以查看、新建、编辑和
  ...
  按独立操作权限再次强制校验；
- 后台桌面侧栏与移动导航都在“首页”旁显示外观按钮。当前后台颜色上下文固定为亮色，
  按钮也固定声明 `light` 并保持禁用，不读取或写入管理员偏好；暗色切换在后续版本
  启用。

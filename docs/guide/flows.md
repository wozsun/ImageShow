...
后才一次性接管已保存的后台明暗外观（初始默认暗色）；认证读取失败也使用已经就绪
的后台色域承载重试界面，后台内部换页不会把颜色域重置回启动状态。前两项元数据供
仍读取它们的浏览器使用，真实文档背景用于
Safari 26 的地址栏取色。所有页面和组件颜色均通过语义 token 或按颜色域重映射的
共享别名消费；公开详情按管理员会话加载管理信息或编辑器时，同一按需能力块会携带
后台颜色契约，不要求先访问后台路由，嵌套管理弹窗只在自身范围内重映射共享控件
别名。原始值只允许存在于两份颜色契约及与启动底色一致的初始
`theme-color`。顶层公开路由等待在启动暗色表面中央以低强调度静态显示 `加载中…`；
...
- 标签、主题、作者、用户、设置、检查与账户设置；图片管理员可以查看、新建、编辑和
  ...
  按独立操作权限再次强制校验；
- 后台桌面侧栏与移动导航都在“首页”旁显示同一个外观开关，显式选择亮色或暗色；
  当前不提供跟随系统选项，缺少偏好时默认暗色；图片卡片密度缺少偏好时默认紧凑。

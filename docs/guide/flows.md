...
5.6.0 的一次性布局升级另由可整体删除的 `storage-layout-upgrade` Server / Web 边界懒加载，
同样只对超级管理员开放。
存储维护弹窗使用当前只读检查结果预览缺图、可修复缩略图、孤儿对象和活动 Ingestion，
但必须由超级管理员再次确认。执行端在独占存储位置锁内重新读取 PostgreSQL 和 `full`、旧
`media`、`thumbs`、`_uploads` 四个完整
...
布局升级弹窗独立读取总图片、已符合新布局、剩余旧键、预计传输量、活动旧 Ingestion、待清理
任务和 ready revision。操作者确认已停止新图接入并接受迁移期短时链接失效
后，Web 以每批 10 张连续请求最多 100 张的服务端有界入口，并在每批后用服务端当前计数校正。
断线、关闭或刷新不恢复浏览器游标，只重新读取 PostgreSQL 剩余旧键。数据库已经全部切换但旧
对象、清理任务或投影尚未归零时显示收尾阶段并将进度限制为 99%；本轮无失败且所有完成条件
同时成立后才显示 100%。为避免重复扫描大前缀，物理 `media` 只在数据库、清理任务和投影均已
收敛后执行最终列举。该入口不创建 schema ledger、持久迁移任务或 SSE。
...
- hash 静态资源、稳定的 `/full/*`、5.6.0 迁移期旧 `/media/*` 与可证明存在的 `/thumbs/*` 使用 immutable；非 hash

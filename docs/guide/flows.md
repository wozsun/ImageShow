...
首页装饰背景固定使用带 `100vh` 回退的 `100lvh` 大视口高度，而不是让上下边界
跟随 Safari 动态工具栏。地址栏展开或收起只改变背景的可见裁切，不重新计算
`object-fit: cover` 缩放；横竖屏切换等真实视口变化仍会重新适配。
...
变化时写入一次 `src`，使 Strict Mode 复演和可见状态更新不会重新发起请求。请求开始
即启动 600ms 局部期限，首页控制器另有从进入时计算的 1000ms 全局期限。
...
若背景到 600ms 仍未就绪，则先在现有渐变底色上释放导航，80ms 后释放 Hero；请求
尚未实际开始时由 1000ms 全局期限执行同一兜底。页面从后台恢复时会按绝对期限补齐
被节流的导航、Hero 和目录阶段；若恢复时各阶段的原始绝对时点都已过去，则同次检查
立即全部补齐，不从恢复时刻重新等待。deadline 路径在释放导航后的 360ms 内 armed
目录。
...
目录数据刷新时低频流动，完成后以交替的两套同形 keyframe 强制完整收尾一遍，再
停止并保持亮线；因此快速缓存或网络响应也不会只闪现半段，连续筛选仍能可靠重触发。
标签与作者轨道向标题靠拢，并与下方选项卡片保留独立间距；流光只修改 `transform`
与 `opacity`，稳定线右端自然衰减，不保留固定高亮圆点。
流光位移终点按自身 18% 宽度收回，标题内部为轨道保留等量空间并裁切两端，使进入与
淡出的全过程都不越过标题轨道；末段约三分之一在继续前进的同时平滑渐隐，不会停在
轨道末端后突然消失。
...
不改变请求、图片清理、焦点或滚动语义。导航、次导航、目录波次、完成流光、工具栏和
...
- 图片列表、编辑、批量操作、回收站、存储迁移；图片管理员可以进行常规元数据编辑、
  ...
  回收站只对超级管理员显示，并由服务端再次强制校验。单图编辑弹窗在迁移存储右侧
  提供仅图标的移入回收站按钮：首次点击切换为另一删除图标与强调色，第二次点击才
  提交，焦点移出或在按钮外按下会取消确认；移动端隐藏迁移按钮图标并收拢宽度，保证
  360px 视口仍可同时容纳迁移、删除、取消与保存操作。公开画廊从详情编辑删除图片
  后，会在详情关闭前刷新并补位列表，保持原页面滚动位置，并把焦点交给原位置的

...
移动端的批量默认属性使用共享折叠面板。“应用到全部”只更新仍然挂载的草稿或任务卡片，不会
关闭、移除、禁用或重排按钮自身，因此保留原生 `button` / `click`，不建立跨控件的兼容事件
守卫。折叠开关则会同步隐藏、设为 inert 并重定位面板内容，使用共享
`DirectActivationButton`：触控 / 笔在移动量未超过统一 5px 阈值的 `pointerup` 直接提交开合，
鼠标、键盘与辅助技术继续走原生 click；迟到的同手势兼容事件由该组件已有守卫消费。这样下一次
折叠不再继承“应用到全部”的文档级激活事务，也不依赖 WebKit 一定补发 click。`5.4.5` 增加的
原触点、`touchstart` 与多点 guard 推断已经撤回；折叠面板自己的捕获阶段 `touchstart` 仍只负责
外部触控关闭，不建立页面入口特判、延时器或第二套展开状态。

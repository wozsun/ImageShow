...
`GET /random` 默认从随机池中按各分类的图片数量加权选取一张图片；`m=json` 可一次
...
| `n` | 大于 0 的整数 | 仅显式指定 `m=json` 时有效；缺省为 1，请求多张 JSON 结果 |
...
- 只接受表中八个精确小写键；`d`、`b`、`m`、`n` 各最多出现一次。`d`、`b`
  空值等同未指定，空的 `m` 无效。
...
- selector 可通过逗号或重复参数提交；空白片段忽略，单独的 `!`、控制字符以及同一类
  混用包含和排除均返回 400。
- `id` 只接受带连字符的完整 UUID 或 UUID 最后一组 12 位十六进制字符，不区分大小写；
  可通过逗号或重复参数提交，每次最多 32 个非空值，并按去重前的提交数计数。重复值
  会被归一化去重，空的 `id` 返回 400。
- `n` 接受任意位数、值大于 0 的十进制整数（可有前导零），但只在查询串明确包含
  `m=json` 时有效；仅因站点默认模式为 JSON 并不能省略 `m` 后使用 `n`。服务端单次
  最多处理 200 张，超出时不拒绝请求，而是按 200 张执行。
- 指定 `id` 时只允许同时指定 `m` 和 `n`；`n` 仍须满足显式 `m=json`。即使其他筛选
  参数为空，只要它出现在查询串中也会返回 400。所有格式、数量和互斥校验均先于
  词表、Redis 与存储访问完成。
...
`id` 路径直接从 PostgreSQL 权威数据中查询 `ready` 图片，完整 UUID 与末 12 位匹配到的
图片合并为同一候选集合，再等概率打乱；普通返回模式取一张，JSON 模式最多取 `n` 张。
它不访问 Redis 随机池，也不应用短期最近图片去重；单次响应自身仍会去重。没有任何匹配、
只匹配到回收站图片或候选已不可用时均返回 404，绝不回退到完整随机池。完整 UUID 使用
主键，末 12 位使用 `ready` 图片的部分表达式索引；混合查询先分别命中两个索引分支，
再合并去重候选。例如：
...
完成所请求的词表解析后，不含 `id` 的正常 `/random` 选择路径不依赖 PostgreSQL，不使用
...
`m=proxy` 从图片所属的 local、S3 或 WebDAV 后端读取已入库图片字节，并附带
`X-Image-Info` 头；由于每次请求都会重新抽图，它不声明 `Accept-Ranges`。
`m=redirect` 返回 302 跳转到图片的公开 URL。这里的 `proxy` 只是随机接口的返回
传输方式，与图片导入模式无关。

`m=json` 返回 `application/json`，顶层 `count` 是实际返回数量，`items` 是图片数组：
...
只写 `m=json` 时等同 `n=1`，但仍返回数组。`n` 是期望上限而非数量保证：筛选或 `id`
候选不足、单次 200 张执行上限，或抽取后图片恰好变为不可用时，`count` 都可以小于
请求值。正常随机池批量抽样由 Redis 集合保证单次响应 UUID 唯一，并优先避开该客户端
最近拿到的图片；随后用一次 PostgreSQL 查询读取宽高、时间和标签真值，再生成所属
存储后端的原图及缩略图 URL。GET 与 HEAD 都为 `no-store`；HEAD 返回与 GET 一致的
状态、内容类型和内容长度，但不发送 JSON 正文。

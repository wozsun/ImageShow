...
`GET /random` 默认从随机池中按各分类的图片数量加权选取一张图片；也可用 `id`
限定本次请求的候选图片：
...
- 只接受表中七个精确小写键；`d`、`b`、`m` 各最多出现一次，空值等同未指定。
...
- 指定 `id` 时只允许同时指定 `m`；即使其他筛选参数为空，只要它出现在查询串中也会
  返回 400。所有格式、数量和互斥校验均先于词表、Redis 与存储访问完成。
...
图片合并为同一候选集合，再等概率随机选择一张。它不访问 Redis 随机池，也不应用短期
最近图片去重；没有任何匹配、只匹配到回收站图片或候选已不可用时均返回 404，绝不回退
到完整随机池。完整 UUID 使用主键，末 12 位使用 `ready` 图片的部分表达式索引；混合
查询先分别命中两个索引分支，再合并去重候选。例如：

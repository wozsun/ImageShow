...
`components/image/filter/PublicFilterDialog.tsx` 拥有公开普通筛选的本次草稿、搜索与连续五区目录；
...
`lib/gallery/public-filter-draft.ts` 拥有草稿编解码、输入预算和已选项投影；标签草稿拥有带编号和组内模式的 groups、activeId 与 grouped 展示状态。成组后进入空组或新组，点击组条目切换编辑，删除整组或从已选栏移除所有组中的同一标签。空组不参与查询，最后非空编辑组参与预览与应用；弹窗组条目显示或 / 且关系。路由保存非空组的重复 tag 参数及各组模式，API 与统计投影为规范 TagExpression，同组空集合仍保留本次模式选择。
`useGalleryStats` 提供可禁用订阅：未筛选统计提供标签目录成员，草稿五字段投影即时查询动态候选数量和完整匹配数量；候选数量排除自身维度，标签排除整个标签条件，沿用首页的统计口径；
自动设备与列表采用同一 UA 投影，不发送列表参数，空条件复用总量查询。沿用查询的上一结果占位，弹窗内保留最后成功结果用于失败时展示，更新及失败状态显式提示；“已选”栏下方仅一条刷新光带，减少动态效果时静态显示。未取得数量不伪造 0。
`gallery-query.ts` 的 `galleryStatsSearch` 统一首页与弹窗统计参数：主题 / 作者按集合归一化，标签采用规范表达，自动设备按 UA 投影，并与图片列表共用规范条件投影；页面 URL 仍保留标签组边界与顺序。`public-filter-options.ts` 统一零数量禁用、未验证时锁定新增以及已选 / 不限可取消规则，首页入场项目选择也消费同一规则。`useRefreshGlint.ts` 统一刷新轮次和周期末尾退场，宿主各自拥有入场时机、布局和配色。
...
- `components/` 按稳定 UI 职责保存跨页面组件；`components/image/editor/` 的数量中性
  ...
  公开筛选在成功新增条件后向 `PublicFilterChips` 传递本次定位目标，按滚动区实际宽度平滑定位局部已选栏，不改焦点、不滚动弹窗正文、不因数量更新反复定位。已有标签加入其他组时也定位其唯一已选项，选择超限失败和移除条件不触发新增定位；无需维护自动定位期间的独立箭头状态。
...
- `pages/gallery/` 就近拥有 cursor / ID 数据窗口、typed-array 瀑布流索引、半屏滞回虚拟窗口、
  ...
  摘要按维度以 `·` 分组，仅展示已选设备 / 明暗的具体选项和主题 / 标签 / 作者的已选数量，排除条件保留“排除”标识。标签数量复用已选项的跨组去重口径，未选维度省略，无条件显示“全部图片”，无效标签仍显示待处理提示。

...
- `pages/gallery/` 就近拥有 cursor / ID 数据窗口、typed-array 瀑布流索引、半屏滞回虚拟窗口、
  ...
  编辑、删除、接入或资源配置变更时，历史返回先清除保留的旧 DTO 并水合所需权威页，无变更则直接复用。
  不在标签页间广播失效。列表读取使用 `cache: "no-cache"` 等待 HTTP 条件重验证，继续允许
  304 复用，避免浏览器先交付旧缓存而让主动刷新晚一次生效。临时页查询键包含挂载所有者、导航条目和数据代次，旧访问的在途请求、
  取消与查询清理不接管新访问的读取；前邻页改变 cursor 链时，恢复继续
  重建到锚点原页及下一页的有界范围，待视口权威页收敛后恢复未变锚点，无法定位时落到附近位置。
  编辑快照与页成员确认分开：页面保留未完成刷新意图，直到权威页成功返回；即使同代次离页返回，
  也会接续成员水合，不把已有快照误当作筛选和 cursor 链已经确认。
...
- `pages/show/` 就近拥有公开展映编排、真实图片查询与 `pixi/` 生产运行时；不保留旧实现对照入口。
  ...
  删除成功后取消并隔离较早的读取响应。编辑成功后，同一数据流按 ID 将权威快照原位合入现有
  候选；快照缺失时只通过随机接口的图片 ID 模式回读该图，连续编辑会取消旧回读。分类变化按规范化公开筛选语义决定
  当前成员，退出筛选时只移除目标并从既有 cursor / 随机流补图，不重新抽样或替换累计集合；
  普通补图失败不会被逐帧信号重复触发。
  ...
  与实时边框共用透视投影；保留分档快照复用，不按每个缩放帧重新截图。React 只

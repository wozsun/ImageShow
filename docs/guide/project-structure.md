...
稳定的 `MIMEType.parse()`。Dockerfile 固定 Node 26.10.0 / npm 12.1.0，并通过同一
`NPM_VERSION` 控制构建与容器维护环境；本机可用 `npm install --global npm@12.1.0` 对齐。
...
首页沿用唯一统计查询及现有目录数量、排序规则，不额外请求未筛选统计或保留成员副本。
首页和公开筛选弹窗共用 `usePublicFilterStats` 的显示快照、更新状态与可信度，底层仍由 `useGalleryStats` 拥有查询缓存；每个挂载的消费者各自保存最后成功显示结果，不设全局旧结果。
...
分组与随机接口各自的输入预算；列表 / 统计的 `parseGalleryTagFilter` 允许最多 9 个组内任一 / 全部、组间或的条件，简单选择器仍使用 basic 解析。SQL 标签谓词由 `read-models/image-filter-sql.ts` 唯一生成。
...
`gallery-stats-plan.ts` 解析有序标签组与当前且组的 `tag_scope`，复用图片筛选的词项解析，生成完整条件、标签候选和组计数计划。缓存签名包含三者，保持组切换、或且切换和重排的统计隔离。
统计 HTTP 入口的允许参数由 `galleryStatsQuery` schema 派生，未知参数继续返回 403；参数提取与校验共用同一份字段定义。
...
无上下文时直接执行这七条查询，不补查 revision。标签组与完整条件相同时直接复用 matching，其余组由一条额外聚合 SELECT 在同一快照计算；缓存命中不增加 SQL。
全量 SQL 同时携带词条名称、链接和排序；缓存与混合路径复用词表 owner。标签候选沿
`image_tag` 复合主键连接，筛选继续使用共同 SQL 谓词；主题 / 作者省略自身条件，标签按当前或 / 且组的候选计划统计。Redis 复用相同组计划的筛选索引并批量读取基数，参与原有 revision、索引验证和工作准入。
...
随机链接投影；批量应用一次替换五字段，保留排序与展映模式，等价用户条件不新增导航。
`components/navigation/PublicImageNavigation.tsx` 装配共用 Header / Toolbar 及移动端底部控件插槽。
画廊和展映分别持有视口控制与数据生命周期，共享导航不新增查询或运动状态。
设备与亮度的值遵循 shared 枚举，各页面保留自己的选项顺序及文案；facets 只返回动态主题、标签和作者词表。

`components/image/filter/PublicFilterDialog.tsx` 拥有公开普通筛选的本次草稿、搜索、统计订阅与开关编排；
同目录 `PublicFilterSection.tsx` 渲染五区选项、匹配方式与标签组，不持有独立草稿或查询状态。
主题 / 作者支持包含与排除，标签支持组内任一 / 全部、组间或，最多 9 组。动态选项来自原 facets 查询，不截断完整目录；
空结果、词表失败与非法标签分别展示，未知已选项可移除，错误标签未经显式处理不能应用。
标签选项按已有未筛选统计的正数量成员过滤，主题和作者沿用 facets 的全局成员范围；不按当前草稿匹配数隐藏选项。完整标签词表继续用于路由与草稿合法性解析，零匹配已选条件可保留和移除。未筛选统计初次加载或失败时，标签区域显示加载状态或重试入口，不把未知成员当成空目录。
`lib/gallery/public-filter-draft.ts` 拥有草稿编解码、编辑校验、输入预算和已选项投影；标签草稿拥有带编号和组内模式的 groups、activeId 与 grouped 展示状态。成组后进入空组或新组，点击组条目切换编辑。`publicFilterChips` 生成去重选择项供数量和工具栏摘要使用；`groupPublicFilterChips` 在已选栏呈现每个非空组，以组编号保持身份、按或 / 且用 `/` / `&` 连接显示名，移除时复用整组删除入口。未成组时仍逐标签移除。空组不参与查询，最后非空编辑组参与预览与应用；弹窗组条目显示或 / 且关系及当前其他筛选条件下的组内图片数量。路由和统计保留非空组的重复 tag 参数及各组模式，图片列表 API 投影为规范 TagExpression，同组空集合仍保留本次模式选择。
`useGalleryStats` 提供可禁用订阅：未筛选统计提供标签目录成员，草稿投影即时查询动态候选数量、各组数量和完整匹配数量；主题 / 作者排除自身维度，标签或模式排除所有标签条件，且模式保留当前组已选标签计算候选交集，沿用首页的统计口径；
自动设备与列表采用同一 UA 投影，不发送列表参数，空条件复用总量查询。沿用查询的上一结果占位，弹窗内保留最后成功结果用于失败时展示，更新及失败状态显式提示；“已选”栏下方仅一条刷新光带，减少动态效果时静态显示。未取得数量不伪造 0。
`gallery-query.ts` 的 `galleryStatsSearch` 统一首页与弹窗统计参数：主题 / 作者按集合归一化，自动设备按 UA 投影，与图片列表共用基础投影；统计另保留标签组边界、顺序和或且模式，并由 `tag_scope` 指定非空且组，空组与或组不传 scope。组数量按返回条件匹配当前条目，避免删除或重排时将旧数量套到另一组。页面 URL 保留标签组，但不携带统计专用 scope。`public-filter-options.ts` 统一零数量禁用、未验证时锁定新增以及已选 / 不限可取消规则，首页入场项目选择也消费同一规则。`useRefreshGlint.ts` 统一刷新轮次和周期末尾退场，宿主各自拥有入场时机、布局和配色。
`usePublicFilterScroll.ts` 只管理局部位置、搜索前锚点与五区 scroll-spy，滚动不改变草稿或查询。
`usePublicFilterDialog.ts` 由宿主页面调用，拥有打开时路由身份及关闭后的应用交接；草稿留在弹窗内。
它等待页面锁发布滚动恢复完成事件后才应用，外部路由变化撤销旧草稿与待应用动作。
弹窗复用 DialogFrame 的焦点、滚动锁、安全区与退场；画廊暂停图片交互，展映暂停运动而保留播放意愿。
...
- `components/` 按稳定 UI 职责保存跨页面组件；`components/image/editor/` 的数量中性
  ...
  箭头、键盘和纯纵向鼠标滚轮触发的横移平滑过渡，共享 Hook 累计同向滚轮的未完成目标，
  并在反向或直接操作时让位；原生触摸 / 触控板手势仍保持跟手，减少动态效果时关闭平滑。
  共享弹窗边界消费手指横拖时显式使用即时滚动，避免同一 viewport 的 CSS 平滑延缓手势位移。
  `PublicFilterChips` 只呈现可移除的已选按钮，不包含输入框或点击空白转移输入焦点的行为；移除与导航复用 `DirectActivationButton`，滑动不提交删除，键盘仍可访问这些按钮。
  禁用期间保留标签间距、padding 和删除按钮尺寸与符号，仅降低控件透明度并阻止编辑，避免保存时标签宽度跳动。
  `components/data-display/FacetSelector.tsx` 拥有后台图片主题、标签和作者的搜索式筛选交互；
  `lib/ui/facet-input.ts` 统一筛选与编辑候选的名称 / slug 匹配，公开目录复用匹配规则并保留全部匹配项。
  后台候选按完整、连续、非连续字符匹配分层，同层保留词表顺序，排除已选项后最多返回 50 项。非连续匹配按字符顺序
  前进，每个重复字符占用独立位置；`FacetSuggestionLabel.tsx` 复用匹配区间高亮原文，不另算匹配。
  公开筛选、后台图片筛选 `FacetSelector`、主题 / 作者输入 `SlugComboInput` 与标签输入 `TagInput`
  通过 `useFacetSearchMatcher` 共用拼音能力，覆盖单图 / 批量编辑、上传 / 导入默认属性与任务卡片。
  仅在搜索界面或编辑建议展开时，经页面级模块加载器导入 `lib/ui/pinyin-facet-input.ts` 与 `pinyin-pro`，
  同页共享一次模块请求，关闭或卸载后不提交异步状态。所有入口共用加载中与失败说明，
  失败时保留名称 / slug 搜索，公开筛选提供刷新入口，不把尚未加载的拼音结果当作已确认空结果。
  拼音作为名称匹配的第四层，支持全拼、首字母、混合缩写、多音字、空格与 v / ü；沿用先文字后拼音的排序，
...
- `hooks/` 保存跨页面且主要管理 React 生命周期或交互行为的 Hook。
  ...
  顶部热区排除次级导航内的按钮、链接和输入控件，包含首页“进入展映 / 画廊”与“重置”；鼠标移到控件之外的顶部空白区域仍可唤出，避免操作目标被主导航推走。
  ...
  菜单展开观察均由该 owner 管理。
  `lib/ui/public-navigation.ts` 统一判定导航内的悬停或焦点；精细悬停设备保护导航内的悬停，
  所有设备仅保护 `:focus-visible` 焦点，忽略筛选关闭后归还的普通指针焦点；触控设备同时忽略粘滞悬停。交互期间取消计时并暂停滚动收起，
  首页独立 `AppHeader` 复用该判定保护滚动收起；焦点移出在同一次焦点转移完成后重新读取。
  导航以外的鼠标移动不重置计时；菜单、筛选弹窗或详情展开时同样暂停，所有保护条件解除后
  ...
  移动端底部控件在导航隐藏后先降为 30%，操作提示先降为 20%，五秒后均淡至 10%；提示在次级文字色中混入 30% 白色，
  ...
  画廊回顶按钮按一屏滚动阈值切换可见性，位于右下角并以 `120ms` 淡入或淡出；
  ...
  后台图片筛选据此让清空关闭 Select / Facet，却不改变外层面板状态。
...
- `lib/` 保存无界面代码；HTTP 客户端、query key 和共享查询 Hook 集中在 `lib/api/`。
  ...
  样式的 overscroll 约束仅放在弹窗边界和实际滚动区，不对全部后代统一设置 contain，避免长名称裁切容器阻断卡片到列表的原生滚动传递。
...
- `AppRoutes.tsx` 将普通与嵌入路径映射到同一 `HomePage` / `GalleryPage`，并把 `/show` 与 `/embed/show` 懒加载到
  ...
  和已加载 SPA 都收敛到 404，随机 API 与后台不参与公开页回退。页面参数决定主导航及页脚备案项的展示，
  不能复制公开页实现或以 CSS 隐藏导航。服务端仍独立决定嵌入
  文档是否存在并输出父页面白名单，前端开关只负责已加载 SPA 内的路由收敛。
  普通公开页面与后台共用挂载 `AuthSessionProvider` 的路由布局，唯一持有 `/auth/me` 查询与
  失效恢复监听。提供器卸载时同步关闭对外刷新入口，旧登录确认、恢复调用方与已排队的恢复回调均按取消收口。
  三个嵌入路由不挂载该提供器，因此不探测会话或消费已有认证缓存。
  共享图片详情通过可选会话上下文判断身份；无提供器时使用访客查询键与 `credentials: omit`，
  不挂载管理详情或原图入口。离开嵌入页回到普通页面后，仍按既有会话提示和认证缓存恢复。
...
- `pages/home/HomePage.tsx` 只编排查询、筛选状态和页面生命周期；首屏、筛选摘要栏、
  ...
  `site.icp`、`site.mps`、`site.footer`，普通首页按非空字段展示备案链接与自定义内容，不单独读取配置。
  嵌入首页只展示已配置的 `footer`，未配置时整个页脚不挂载；备案配置仍供普通首页使用。
...
- `pages/gallery/` 就近拥有 cursor / ID 数据窗口、typed-array 瀑布流索引、半屏滞回虚拟窗口、
  ...
  `hooks/useImageBrowseRoute.ts` 复用全局 facets 解析标签词项，阻止非法、未知或超限页面条件
  发出图片请求；公开标签词表包含零图片词条。该 Hook 使用路由 API 写入最终可读查询串，
  页面分享与复制使用相同 slug 条件。`PublicFilterErrorState` 提供错误与逐字段清空入口。
  `lib/gallery/gallery-selectors.ts` 统一首页、公开弹窗与路由的主题 / 作者数量、格式及包含排除校验。
  无效 URL 的原始条件由路由保留，弹窗持有未解析草稿，显式清除前禁止应用、图片查询及分享。
  ...
  查询和随机链接从同一 URL 状态同步更新。共用 `PublicImageToolbar` 提供筛选按钮、已应用摘要、排序和分享，
  摘要按维度以 `·` 分组，仅展示已选设备 / 明暗的具体选项和主题 / 标签 / 作者的已选数量，排除条件保留“排除”标识。标签数量复用已选项的跨组去重口径，未选维度省略，无条件显示“全部图片”，无效筛选条件仍显示待处理提示。
  桌面排序入口显示“随机模式 / 最新优先 / 最旧优先”，三种状态共用 116px 宽度，文字左侧留白为 14px。
  `PublicImageNavigation` 沿用 760px 断点改变控件挂载位置；移动端顶部只留筛选和分享，不挂载摘要。
  `PublicImageOrderControl` 复用同一顺序枚举和路由写入回调，桌面为下拉菜单，移动端为依次切换顺序的图标按钮。
...
- `pages/show/` 就近拥有公开展映编排、真实图片查询与 `pixi/` 生产运行时。
  ...
  筛选入口的“筛选”和“重新筛选｜清空”共用固定总宽度，清空后焦点回到保留的筛选按钮。
  `ShowPlaybackButton` 复用同一播放状态，桌面挂在分享左侧并采用工具栏样式，移动端挂在右下并采用公开悬浮控件样式；系统减少动态效果时禁用播放。URL 和配置枚举保留 `waterfall / float`。
...
- `styles/` 按 base、home、gallery、admin 和 responsive 组织全局样式；首页进一步
  ...
  展映页面、画布和操作区统一使用动态视口，不叠加浏览器工具栏高度补偿。根容器使用 `.gallery-page.show-page` 明确覆盖公共 `.page` / `.gallery-page` 的最小高度、定位及顶部留白，避免路由样式加载顺序把 100dvh 展映区域撑回 100vh，让移动浏览器底部控件落到可视区外。
  ...
  导航隐藏且无交互保护时底部控件为 30%、展映提示文字为 20%，保持 5 秒后淡至 10%。
  重新悬停、获得键盘可见焦点或显示导航取消本次淡出；保护解除后重新计时。
  减少动态效果时保留 5 秒等待，直接切换最终透明度。
  `styles/semantic-colors.css` 拥有启动暗色、公开页源颜色、公开浏览与后台暗色分支共用的固定暗色角色，以及共享组件的公开上下文映射；
  启动画布的普通文字、成功、危险与错误反馈文字都由颜色门禁验证至少 4.5:1 对比度。
  画廊与展映在 `gallery-semantic-colors.css` 中局部降低正文、导航和强调白字的亮度，
  保留高于后台暗色正文的亮度；弹出到页面外的菜单与密度提示使用同一层级，首页继续使用原公开配色。
  浏览工具栏按钮和公开筛选弹窗的背景、边框与操作色复用后台暗色角色；gallery 语义颜色文件映射弹窗表面、文字、边界与动作配色，不加载后台样式块。筛选通过 `DialogFrame` 的 public 颜色上下文避开管理弹窗样式匹配，也不受后台亮色选择影响。
  筛选弹窗标题与正文直接共用公开浏览页面的柔和文字层级，设备、明暗、主题、标签、作者五个分区标题统一为 18px；目录未选中项目的图标和标题使用略亮的 `--public-color-filter-directory-label`（后台辅助文字与主文字按 40% / 60% 混合），不限 / 已选 / 匹配说明与其他目录辅助文字保留原亮度，当前项目继续使用蓝色强调；其他区域局部增强辅助文字对比并使用更深的画布和主表面；目录项目始终左对齐，计数位于右侧。
  筛选弹窗选项悬停边框采用与首页相同的 160ms ease 渐亮效果，颜色消费弹窗自身的 `--public-color-gallery-menu-selection-border` 蓝色边框变量；悬停不改变选项底色，减少动态效果时取消过渡。
  `public-filter-dialog.css` 限定公开筛选的几何与响应式布局。目录和选项区各自复用 `OverlayScrollbar`，在所属容器内定位并支持触屏；已选条件条使用与标签编辑器共用的横向滚动能力，隐藏原生条。
  目录定位在 `usePublicFilterScroll` 内以 160ms 缓出动画完成；点击定位期间及到达目标位置后保留所点目录高亮，避免途经区块或底部截断位置覆盖选择。连续目录点击替换当前目标，手动滚动、拖动滚动条、搜索变化与卸载取消动画并释放点击高亮，随后恢复位置跟随；减少动态效果时直接定位。
  桌面标题与说明同排，窄屏隐藏辅助说明；标题和底栏采用更深的主表面，底部按钮为 38px，主操作蓝色在公开筛选作用域内适度调暗。
  底栏结果区固定为 26px 状态行和 14px 说明行，重试按钮限定在状态行内；加载、数量、错误与重试共用同一占位，长文案单行省略，避免统计状态切换改变底栏高度。大号数量沿用正文基线且不撑高行盒，保持各状态正文的垂直位置一致。
  设备与明暗使用分段单选，选项文字统一为 13px：桌面同组左右排列，760px 及以下上下排列；目录仍保留两项，同一行区块按最后点击的目录项保留高亮，其他区块继续随滚动位置更新。目录按钮同时收紧高度与留白。
  设备提供不限 / 自动判断 / 横版 / 竖版；自动判断的选项计数复用 shared UA 判断，对应页面统计和列表的设备投影。设备分段默认四列，自身容器不超过 280px 时排为两列，避免窄屏文字挤压；页面链接保留 auto，随机链接仍按既有契约省略 device 参数。
  搜索仅匹配主题、标签、作者；已提交的规范化搜索词非空时隐藏设备与明暗区块及对应目录入口，清空后恢复，已选条件不受影响。设备与明暗标题不显示选项种类数，也不参与搜索计数或高亮；输入法组词期间沿用已提交查询的显示状态。词条默认单行显示名称，无显示名时回退 slug；搜索时优先使用显示名匹配结果（含拼音），命中即保持单行，即使同时命中 slug 或存在重名；仅在未命中显示名、命中 slug 时渲染 slug 副行。普通浏览时当前分区可见选项名称重复则显示副行；名称本身已是 slug 时不重复显示。单个选项按实际副行状态使用 has-slug 样式，单双行直接切换并保持卡片高度，悬停仅保留现有边框高亮，键盘聚焦不改变行数；长文本自动滚动继续保留。桌面与窄屏统一为单行名称 14px，双行名称 / slug 为 13px / 9px，行高为 18px / 12px、间距为 2px。主题 / 作者卡片最小高度为桌面 60px、窄屏 58px，标签卡片为 56px；纵向内边距随高度收紧，双行展开仍保留完整文字空间。数量位于右侧、勾选框之前，显示“× 张”；两行均复用首页的 `OverflowMarqueeText`，通过可选 children 保留搜索高亮且不改变字重。共享组件统一管理溢出测量、可见区域动画、悬停 / 聚焦暂停和减少动态效果。
  主题、标签与作者统一使用 `repeat(auto-fill, minmax(min(100%, 240px), 1fr))` 等宽网格，按内容区实际宽度自动增减列数，不为不同屏幕或类别单独指定列数。auto-fill 保留空列，名称长度、词条数量和搜索结果数量不改变列宽；窄于最小卡片宽度时单列收缩至容器。宽屏弹窗上限为 1180px，与后台上传 / 导入弹窗同级。文本容器使用 minmax(0, 1fr) 与 min-width: 0，名称 / slug 继续交由 OverflowMarqueeText 在既定宽度内滚动，并由 ResizeObserver 在宽度变化时重新测量。分区标题与操作区顶部对齐，避免较高的操作按钮使搜索后的首个标题低于原设备标题；匹配方式与标签成组控制区使用自动左外边距保持右对齐，容不下时换行仍靠右。
  桌面采用居中弹窗，760px 及以下铺满可用视口并保留安全区，目录、内容分别滚动，搜索、已选条与底栏固定。
  主题和作者选项默认保留所属图标，标签使用 # 标记；视口宽度不超过 350px 时统一隐藏这三类卡片的前置图标 / 标记，为文字腾出空间，目录图标继续保留。文本在剩余宽度内复用溢出滚动，数量与勾选框保持独立占位。
  `styles/admin/semantic-colors.css` 拥有后台专属源颜色、亮暗分支与后台上下文映射，并只随后台

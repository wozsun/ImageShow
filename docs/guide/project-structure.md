...
- `styles/` 按 base、home、gallery、admin 和 responsive 组织全局样式；首页进一步
  ...
  主题、标签与作者统一使用 `repeat(auto-fill, minmax(min(100%, 240px), 1fr))` 等宽网格，按内容区实际宽度自动增减列数，不为不同屏幕或类别单独指定列数。auto-fill 保留空列，名称长度、词条数量和搜索结果数量不改变列宽；窄于最小卡片宽度时单列收缩至容器。宽屏弹窗上限为 1440px。文本容器使用 minmax(0, 1fr) 与 min-width: 0，名称 / slug 继续交由 OverflowMarqueeText 在既定宽度内滚动，并由 ResizeObserver 在宽度变化时重新测量。匹配方式与标签成组控制区使用自动左外边距保持右对齐，容不下时换行仍靠右，不再强制标签标题纵向左对齐。

...
当前分块规则保留经过排除试验后的最小方案：本地报告默认列出未压缩小于 8 KiB，或生产有效
响应体小于 4 KiB 的全部 emitted JS 与 CSS，并分别统计 512 B、1 KiB、2 KiB、4 KiB、8 KiB 原始体积
档位及有效体积档位；这只是合并候选的发现范围，不是
硬性门槛，完整构建图中的更大共享块仍会参与闭包审计。只有与目标页面必然同行、权限和懒加载
边界不扩大，并且真实完整线路字节下降时才合并。组级 `minSize` 8 / 12 / 16 KiB 在 4.8.19
...
自动合并插件。报告也保留全部入口必达的 Rolldown runtime；该虚拟 runtime 不进入模块 ownership
分组，构建后改写 import 与内容 hash 会复制 bundler 职责，因此不为删除这一个请求另造重写层。
v4.9.11 只显式合并检查与存储页必然共同使用的存储迁移 API 和对话框；该候选
少一次请求且完整线路字节下降。本地 HTTP/2 基准采集口径中，原独立 187 B API 响应同时
记录约 818 B 请求头与 300 B 响应头，远高于合并后的正文波动；因此默认发现窗口扩到
8 KiB，仍以每个候选实际压缩正文和同环境请求开销核算，而不把 8 KiB 当作自动合并线。
该值用于同环境前后对照，不冒充 HPACK 后的物理线路抓包值。正文压缩采用规则与分块采用规则
彼此独立。报告还会发现主构建图之外的 Worker 入口，并把它们纳入体积、命名和重复内容审计；
ALTCHA PBKDF2 Worker 必须由浏览器通过独立 URL 创建，且只在实际出现登录挑战时加载，不能并入
登录页首屏脚本。扩大窗口后，构建图中没有两组 emitted JS 具有完全相同的入口根，也没有两份 CSS
具有完全相同的 owner；尝试把检查面板或导入 API 指定到父路由组时，动态 facade 薄壳仍保留
第二个请求，因此不采用源码反向依赖、构建后重写或跨媒体类型注入来强行减少文件数。

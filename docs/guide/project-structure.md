...
- `components/` 按稳定 UI 职责保存跨页面组件；`components/image/editor/` 的数量中性
  ...
  公开筛选和后台图片筛选 `FacetSelector` 通过 `useFacetSearchMatcher` 共用拼音能力；仅在搜索界面展开时经页面级模块加载器导入 `lib/ui/pinyin-facet-input.ts` 与 `pinyin-pro`，同页共享一次模块请求，关闭或卸载后不提交异步状态。加载失败保留文字搜索并提示刷新；加载中的拼音查询不提前显示空结果。
  拼音作为名称匹配的第四层，支持全拼、首字母、混合缩写、多音字、空格与 v / ü；沿用先文字后拼音的排序，slug 仍按原文字规则匹配。匹配器将 UTF-16 索引合并为原文高亮区间，两端都复用 `MatchedText`，不插入 HTML；后台与公开筛选以各自主题强调色高亮，保持字重。编辑输入的候选继续使用默认文字匹配，不改变新建语义。数据库、DTO、原有 slug 与图片请求均不变。
...
- `styles/` 按 base、home、gallery、admin 和 responsive 组织全局样式；首页进一步
  ...
  筛选弹窗标题与正文直接共用公开浏览页面的柔和文字层级，目录保留后台辅助文字层级，其他区域局部增强辅助文字对比并使用更深的画布和主表面；目录项目始终左对齐，计数位于右侧。
  ...
  设备与明暗使用分段单选：桌面同组左右排列，760px 及以下上下排列；目录仍保留两项，同一行区块按最后点击的目录项保留高亮，其他区块继续随滚动位置更新。目录按钮同时收紧高度与留白。
  ...
  搜索仅匹配主题、标签、作者；已提交的规范化搜索词非空时隐藏设备与明暗区块及对应目录入口，清空后恢复，已选条件不受影响。设备与明暗标题不显示选项种类数，也不参与搜索计数或高亮；输入法组词期间沿用已提交查询的显示状态。词条固定以名称 / slug 两行展示，数量位于右侧、勾选框之前，显示“× 张”；两行均复用首页的 `OverflowMarqueeText`，通过可选 children 保留搜索高亮且不改变字重，避免搜索 slug 时行数或高度变化。共享组件统一管理溢出测量、可见区域动画、悬停 / 聚焦暂停和减少动态效果。
  主题、标签与作者统一使用 `repeat(auto-fill, minmax(min(100%, 240px), 1fr))` 等宽网格，按内容区实际宽度自动增减列数，不为不同屏幕或类别单独指定列数。auto-fill 保留空列，名称长度、词条数量和搜索结果数量不改变列宽；窄于最小卡片宽度时单列收缩至容器。宽屏弹窗上限为 1180px，与后台上传 / 导入弹窗同级。文本容器使用 minmax(0, 1fr) 与 min-width: 0，名称 / slug 继续交由 OverflowMarqueeText 在既定宽度内滚动，并由 ResizeObserver 在宽度变化时重新测量。匹配方式与标签成组控制区使用自动左外边距保持右对齐，容不下时换行仍靠右，不再强制标签标题纵向左对齐。

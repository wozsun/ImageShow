...
3.14.9 以全新空数据库或已经完成 3.14.8 部署的数据库为唯一支持基线。升级到 3.15.1
前的数据库应只记录 `0001_initial`，且实际 schema 与 3.15.0 基线一致。3.15.1 在
advisory lock 内执行一次受校验的基线收口：只有 `0001_initial` 时，若末 12 位索引
缺失则补建并校验；开发期间曾执行 `0002_random_id_suffix_index` 的数据库必须已具有
同一索引，校验通过后删除该临时迁移记录。最终迁移记录只保留 `0001_initial`。

未知迁移记录、缺少 `0001_initial`、临时迁移已登记但索引缺失，或同名索引结构不符
都会拒绝启动，不会猜测或覆盖异常 schema。空数据库直接执行包含该索引的完整
`0001`。迁移器不再识别、折叠或修复 3.14.8 之前的迁移历史；部署 3.14.9 前仍应
先升级至 3.14.8 并完成当时的迁移历史收口，更早版本不能直接跨版本启动 3.14.9。
后续 schema 变化继续从 `0002` 开始新增前向迁移。

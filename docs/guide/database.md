...
等待者，等待 3 秒或执行 7.5 秒即失败。这个预算不作用于管理事务和 Worker，因此主池
仍为二者保留独立余量。调用点为一个公开读取阶段建立显式惰性 reader scope；只有首个缓存 miss
真正执行 SQL 时才借 client，同一阶段后续多条查询都复用它并在 reader 边界按调用顺序执行，
...
中止查询并释放连接。结束后不保留响应副本；共享任务不使用某个访客绑定的 reader。
公开图片列表、详情、资源和随机出口先结束图片读取作用域，再读取存储注册表。
注册表的冷缓存、到期与主动失效加载按 revision 共享独立的有界作用域，保留单个访客独立取消
及全部离开后中止查询的规则，避免持有图片连接再等待共享配置加载。注册表公开结果最多
256 个后端，冷、热缓存均执行该限制；配置快照按[存储说明](storage.md#注册表缓存)缓存。
准入等待和执行期限分别作用于各数据库读取阶段，存储 I/O 使用请求取消信号及 driver 自身时限。
...
等待中的恢复不越过 mutation fence，也不依赖再次发生 Redis 状态变化。
...
新词条在最大值基础上递增，新存储后端在最小值基础上递减；计算结果低于业务下界时取下界，
高于业务上界时取上界。同值按 slug 排序。这一规则适用于单项创建、图片关联时补建词条及
存储后端批量导入，只限定新记录的排序值，不重排或改写已有记录。

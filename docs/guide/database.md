...
3.14.9 以全新空数据库或已经完成 3.14.8 部署的数据库为唯一支持基线。现有数据库的
`schema_migrations` 应只包含 `0001_initial`，且实际 schema 已与该基线一致；启动时
不再识别、折叠或修复 3.14.8 之前的迁移历史，也不根据旧字段形态猜测升级路径。
迁移器仍在 advisory lock 内按文件名顺序执行缺失迁移：空数据库执行完整 `0001`，
未来 schema 变化继续从 `0002` 开始。部署 3.14.9 前应先把现有实例升级至 3.14.8
并完成迁移历史收口；更早版本不能直接跨版本启动 3.14.9。

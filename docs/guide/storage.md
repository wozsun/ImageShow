...
未完成租约，不能破坏部署前 repair 任务仍在收口的候选。检查页维修不调用请求热路径，也不把缩略图字节写入
`background_job`。独占锁
使上传、分类和迁移不能穿过快照边界；客户端中止只停止尚未开始的项目，已经开始的固定低并发
片会先完成写入验证或安全清理。4.8.13 已停止创建新的 repair receipt，`move.cleanup` 暂时只
消费部署前尚未完成的 repair payload；确认生产任务收口后再删除该过渡分支。

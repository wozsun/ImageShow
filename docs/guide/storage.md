...
使上传、分类和迁移不能穿过快照边界；客户端中止会取消后续调度及仍在内部准入排队的项目，已经
...
当前 `storage_slug`、对象键、成品大小、MD5 和分类。存储迁移把数据库大小作为原图与缩略图
的期望值，并在原图传输时同时核对数据库 MD5；兼容且具有同一访问身份的 S3 / COS 后端会在
一次源流校验后发出带源 ETag 条件的 `CopyObject`，其他组合把同一次校验流直接写入目标，
不在内存中保留完整对象。只有传输验证通过才用旧位置和分类做数据库 CAS，并在同一事务中写入
...
整后端迁移与主题重分配直接复用代码内固定 5 项的 Server 逐图搬迁容量，只在

...
当前 `v4.9.10` 的 additions 是注释占位，不执行 DDL 或数据写入。
...
浏览器传输验收按匿名 Home / Gallery / 公开详情、后台 9 图上传、图片编辑、已登录
Home / Gallery 和其他后台页面分组，权重固定为 45 / 25 / 15 / 10 / 5。每个公开路由分别
测量全新持久 profile 冷访问、同一 SPA 连续访问，以及关闭后重开同一 profile 的磁盘缓存；
上传和编辑另测写后状态。报告同时保留浏览器请求、真实网络请求、缓存命中、状态码、header、
body、编码、媒体清单、失败 / 取消和页面可用时间。上传源文件字节始终进入总流量，只在两边
输入完全相同时从可控字节评分中排除。

同一场景与同一缓存状态的基线、候选各有至少 20 个有效样本时，分数使用 70% 可控字节、
20% 网络请求和 10% p95；否则明确把该 p95 标为未评分，并把前两项重分配为 77.78% 与
22.22%。不同缓存状态不混算一个 percentile，冷路径也不能由热缓存收益抵消。媒体身份使用
固定源序号与内容 MD5，而非每轮变化的 UUID；默认随机首页背景单独证明一次请求链路，不进入
固定媒体得分。

`v4.9.10` 以三轮隔离的 450 图完整工作负载重新固定 `v4.9.9` 波动区间。匿名无回退上界
只由同一基线镜像的独立基线与控制样本集合确定，候选自身波动不得放宽门禁。现有匿名路径已经
内联站点公开配置、在没有会话提示时省略 `/auth/me`、对公共读取省略凭据，并为稳定图片使用
不可变 URL；未发现能在固定画质、可见缓冲、并发和动画的同时显著超出控制波动的安全候选，
因此本版不改变匿名生产行为。后续传输候选必须复用同一口径，并先通过匿名各缓存组无回退、
450 张最终 ready、权限闭包、视觉与零容器重启门禁。

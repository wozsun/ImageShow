...
- PostgreSQL 是唯一真相源，承载图片元数据、标签 / 主题 / 作者（含 `image_tag` 关联表）、统一导入会话、后台任务、存储后端注册表与管理员账号及其界面偏好，共 9 张业务表（见[数据库结构](./database.md)）。
- Redis 8 承载两套彼此独立的代际协议。随机池在 `imageshow:random:<generation>:*` 命名空间保存 snapshot 与 axis/category/tag/author 集合；全量与增量更新共享 mutation revision，只有 Lua 原子确认 revision、重建锁 token 和增量锁均允许时才发布新 generation。发布脚本同时把被替换代际登记到持久的退役集合，启动或发布收尾会按 manifest 分批补 TTL；即使进程在切换 current 后退出，旧代际也能继续回收。增量同步由单个 Lua 栅栏原子核对 current generation、mutation revision、update-lock token 与 manifest，并在同一步完成集合 / item / snapshot / 画廊选项写入、manifest 对账和 completed revision 推进；过期执行者不会留下迟到写或永久孤儿键。缺池冷构建由 Redis 固定窗口跨实例限频，相同 pool / filter 构建由进程注册表单飞，调用方中止不取消共享底层 Promise，停机统一中止并排空。构建 generation 与筛选候选从首次写入即带临时 TTL，成功、失败和中止只分批清理本次随机所有权的键。图片读缓存使用统一 `image_cache_revision`：公共列表 / 详情、gallery facets、后台概览、MD5 和对象键 / 缩略图键 / 图片 id lookup 都写入取得的 revision，写入前再次校验，任何图片写操作先推进 revision 再做精确清理。import commit 在事务后重读 PostgreSQL 位置，只使用本次失效返回的 expected revision 预热 lookup；revision 已前进时跳过。Redis 暂时无法确认新 revision 时，本实例的 dirty fence 会让这些读路径直接回源 PostgreSQL 并跳过缓存写入，修复成功后才恢复命中。原图直连探测、后台实体计数、词表和随机去重历史按各自 TTL / revision 管理。正常 `/random` 的 proxy / redirect 选择路径不依赖 PostgreSQL；JSON 模式在 Redis 抽样后用一次 PostgreSQL 批量查询补齐权威元数据。Redis 为空时启动阶段异步重建随机池，固定窗口超额返回带 `Retry-After` 的 429，Redis 不可用仍返回 503，其他读路径按场景降级到 PostgreSQL。

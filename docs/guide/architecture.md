...
当前 `v4.9.13` 的 additions 是注释占位，不执行 DDL 或数据写入。
...
`v4.9.11` 在相同的 50 次 9 图导入与 10 次后台全标签页遍历中，将上传状态订阅从会话
逐项出现和完成都会关闭的连接，收敛为按当前活动世代幂等尝试键建立的稳定 SSE；历史
终态任务不会进入下一批。事件流和失败后的批量状态轮询都使用固定 POST 路径，批量尝试键
或会话 ID 只放在至多 100 项的 JSON 正文中，不形成随任务数增长的 URL。写后失效保持每批
最多一次，并根据提交响应只在新增词条时重读无计数导入词表，详情缓存不因新增图片失效。
三轮候选相对 `v4.9.10` 将上传网络请求均值从
4,283.0 降到 3,709.3（-13.39%），可控线路字节从 21,617,661.3 B 降到
21,250,403.7 B（-1.70%）；50 批状态连接从每轮 577–691 条收敛为固定 100 条，且全部
使用请求目标恒为 25 B 的 `POST /api/admin/imports/events`，导入阶段词表请求从 51 次降到
1 次。长连接请求体、响应头与响应体均进入同一采样口径；九轮仍为 450 / 450 ready，匿名
九个缓存组、固定媒体、零重启与零非取消失败门禁全部通过。完整本地报告保留组合总量、冷 /
热、源文件 payload 及原始逐轮证据。

`v4.9.12` 只在 Home / Gallery 的鼠标悬浮或键盘可见焦点下预取目标路由静态闭包；触摸、
管理入口、API 与媒体不参与预取。冷 Home 到 Gallery 的 6 项 Brotli 响应体为 28,239 B，
冷 Gallery 到 Home 的 2 项为 12,360 B，重复入口意图不新增请求；慢模块仍复用同一 SPA
请求，失败模块由随后正式导航以新文档模块映射恢复。图片元数据保存则用同轮权威快照原位
更新 Gallery 命中卡片，并按真实字段失效其余投影。相同 9 图隔离定向轮的 5 次热编辑相对
`v4.9.11` 将网络请求从 45 降到 30、响应体从 11,723 B 降到 10,225 B，平均可用时间从
3,630.6 ms 到 3,678.6 ms；两边均为 9 / 9 ready、零重启和零非取消失败。该单轮只证明
候选传输方向与功能边界，可用时间差异不作性能结论，也不替代三轮波动区间。

`v4.9.13` 将共享弹窗的动态视口、页面冻结和触摸滚动边界收敛到一个所有权链：根
`DialogLayerPortal` 独占 `100dvh`，移动图片详情只有 article 负责纵向滚动；首锁把应用根
移出命中与可访问树，最后释放再恢复精确滚动和焦点。捕获阶段边界只允许当前顶层弹窗的
实际滚动 owner 消费可用方向，并保留缩放、文本和输入手势。移动关闭按钮位于根层，图片
详情上下文不再创建 article 位移与额外图片合成层。桌面 WebKit 和移动 Chromium 的动态
resize 验证不代表物理 iPhone Safari 工具栏验收，真机结果仍是该缺陷关闭条件。

...
生产部署只支持一个应用进程。PostgreSQL 与 Redis 可以由独立基础设施 Compose 提供，
但都必须是该应用实例明确配置的单一连接目标。部署与停机边界见
[生产部署](./deployment.md)。
...
启动顺序固定为：读取部署配置与运行配置、建立数据库连接、初始化空库或验证非空库、
取得单实例生命周期锁、执行必要清理与管理员初始化、校验 Redis、启动 Worker 和 HTTP。
CLI 入口不会因导入 HTTP 应用而触发这些副作用。

同库第二个应用实例在业务启动前失败；生命周期锁连接丢失会升级为失败停机。正常停机先
停止接收新请求，同时中止 Worker、缓存协调器和存储注册表，再在统一硬期限内排空 HTTP、
driver lease、Redis 与 PostgreSQL。该锁只防止误部署，不构成多实例一致性协议；未来工作
见[多实例待办](./todo-multi-instance.md)。

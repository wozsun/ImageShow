...
只读就绪检查，具体要求见[启动与结构契约](database.md#启动与结构契约)。日常操作见
[启停说明](../DEPLOY.md#日常启停)。
...
- 外部数据库、Redis、存储、备份和恢复见[部署说明](../DEPLOY.md)与[存储指南](storage.md)。
- 上传、分类和回收站操作见[图片管理员指南](roles/image-admin.md)。
- 从源码开发时，运行环境、构建与验证入口见[项目结构](project-structure.md#本地门禁与发布职责)。

...
强烈建议设置实际域名；未设置、为空或为 `example.com` 时接受访问 Host 并使用同源 `/static` 路径，不启用资源子域。显式域名继续约束主站 Host；域名变化可能使当前地址失效。
...
#### site.static_subdomain

- 环境变量：`SITE_STATIC_SUBDOMAIN`
- Compose：显式映射
- 类型、默认值与范围：字符串；默认 `""`；空值或 1–63 字符的合法小写 DNS label

显式域名下，空值使用主站 `/static` 子路径，非空使用独立资源子域；域名为空或 `example.com` 时统一使用主站 `/static`。高级配置保存或热加载后两种出口互斥。同源模式不具备浏览器 Cookie 隔离，详见[资源出口](guide/subdomains.md)。
...
控制主站与资源域 `robots.txt`；首次播种或热加载后影响新请求。

...
本地图片公开 URL 在存储编辑弹窗设置，保存在存储记录中；配置包不会覆盖它，详见[图片公开入口](guide/image-resources.md#本地存储公开-url)。
...
首页底部第一行左侧的 ICP 备案号，填写完整显示文字。非空时链接到工信部备案查询网站，留空隐藏该项。ICP 与公安备案号均非空时，中间显示分隔竖线。
...
首页底部第一行右侧的公安备案号，填写完整显示文字。数字部分用作公安备案查询链接的 `code`，请只填写该备案号及其省份、备案名称，不附带其他数字。留空隐藏该项。只有一个备案项时居中显示，窄屏空间不足时按 ICP、公安备案的顺序换行。
...
留空隐藏该项；页脚不额外显示版权行，三个字段全部留空时整个页脚隐藏。普通首页与嵌入首页使用同一页脚配置。

以上三个字段位于配置文件的 `site.robots_enabled` 之后，不在普通站点设置页提供表单。
修改 `data/config.json` 后手动读取配置文件或重启应用生效；已有配置文件时，环境变量不会覆盖这些值。
